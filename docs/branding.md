...
Each saved profile gets an `id` generated by `newId()` (`client/src/modules/storage/storage.js`)
prepended on save. *(CITATIONS-1, 2026-09-03: this cited `storage.js:158` in a file that has 148
lines — **the one class of stale citation a machine can call wrong today**, since a line past the end
of a file is checkable and a line inside it is not. `newId` is at `:146`. The line number is dropped
rather than replaced, and the path is spelled out because a bare `storage.js` is ambiguous in this
repository.)*
