// ============================================================
// exp-screen-strength.mjs — SCREEN tier: gap-reroll STRENGTH sweep (the visibility knob).
//
...
const S_LIST = argVal('arms', '0.50,1.0').split(',').map((s) => s.trim());
const G_FIXED = argVal('G', '0.75');
...
const TMP_ABS = join(ROOT, 'client/tmp/exp-screen-strength');
...
async function runArmTrack(S, track) {
  const outAbs = join(TMP_ABS, `s${S}-${track.id}`);
  const args = ['scripts/sim-fairness.mjs',
    ...
    '--gapRerollEnabled=true', `--gapRerollThresholdLengths=${G_FIXED}`,
    `--gapRerollStrength=${S}`, '--gapRerollMode=symmetric', '--carouselEnabled=false',
  ...
  const rows = rp.races.map((rec) => {
    ...
    return {
      S, track: track.id, seed: rec.seed,
  ...
  return { S, track, rows, episodes: el.races.map((r) => r.escapeLatency), _secs: (Date.now() - t0) / 1000 };
}

console.log(`\n=== SCREEN — gap-reroll STRENGTH sweep (visibility knob), G fixed at ${G_FIXED} ===`);
console.log(`arms s=${S_LIST.join(' / ')} | tracks ${TRACK_IDS.join(', ')} | N=${RACES} | racers 40 closed / 60 open | seeds paired`);
mkdirSync(OUT_ABS, { recursive: true });

const jobs = [];
for (const S of S_LIST) for (const t of TRACKS) jobs.push({ S, t });
...
async function worker() {
  while (true) {
    ...
    out[i] = await runArmTrack(jobs[i].S, jobs[i].t);
    console.log(`  s=${out[i].S} ${out[i].track.id.padEnd(14)} done (${out[i]._secs.toFixed(0)}s)`);
  }
}
...
const byS = new Map();
for (const r of all) { if (!byS.has(r.S)) byS.set(r.S, []); byS.get(r.S).push(r); }
const epByS = new Map();
for (const o of out) { if (!epByS.has(o.S)) epByS.set(o.S, []); epByS.get(o.S).push(...o.episodes); }
const armAgg = S_LIST.filter((g) => byS.has(g)).map((g) => ({ S: g, ...agg(byS.get(g)), ep: summarizeEpisodes(epByS.get(g) ?? []) }));

// CSVs
const RCOLS = ['S', 'track', 'seed', 'escapeDepthLen', 'escapeDepthCapped', 'maxLeaderGapLen', 'leaderDownCount', 'finaleLead', 'finaleDistinct', 'dead', 'frontGroupAtLine', 'duoEscape', 'gapP1P2', 'gapP2P3', 'runaway', 'parade', 'bandReach'];
writeFileSync(join(OUT_ABS, 'screen-per-seed.csv'), [RCOLS.join(','), ...all.map((r) => RCOLS.map((c) => (typeof r[c] === 'number' ? r3(r[c]) : (r[c] ?? ''))).join(','))].join('\n') + '\n');
const ACOLS = ['S', 'n', 'escapeDepthMed', 'escapeDepthP90', 'escapeDepthMax', 'cappedRate', 'leaderDownPerRace', 'tiltFracMed', 'tiltFracMean', 'tiltSaturatedRate', 'tiltDeltaMed', 'nTiltEvents', 'finaleLead', 'finaleDistinct', 'deadRate', 'duoEscapeRate', 'frontGroupAtLine', 'runawayRate', 'paradeRate', 'bandReach'];
writeFileSync(join(OUT_ABS, 'screen-arms.csv'), [ACOLS.join(','), ...armAgg.map((a) => ACOLS.map((c) => (typeof a[c] === 'number' ? r3(a[c]) : (a[c] ?? ''))).join(','))].join('\n') + '\n');
// per arm x track
const TR = [];
for (const o of out) TR.push({ S: o.S, track: o.track.id, ...agg(o.rows), ep: summarizeEpisodes(o.episodes) });
writeFileSync(join(OUT_ABS, 'screen-arm-track.csv'), [['S', 'track', ...ACOLS.slice(1)].join(','), ...TR.map((r) => ['S', 'track', ...ACOLS.slice(1)].map((c) => (typeof r[c] === 'number' ? r3(r[c]) : (r[c] ?? ''))).join(','))].join('\n') + '\n');

console.log(`\nESCAPE LATENCY (the owner's finding)`);
console.log('s      depth_med  depth_p90  depth_max  capped%  tilts/race  frac_med  saturated%  delta_med  nEvents');
for (const a of armAgg) console.log(`${a.S.padEnd(6)} ${String(r3(a.escapeDepthMed)).padStart(9)} ${String(r3(a.escapeDepthP90)).padStart(10)} ${String(r3(a.escapeDepthMax)).padStart(10)} ${pct(a.cappedRate).padStart(8)} ${a.leaderDownPerRace.toFixed(2).padStart(11)} ${String(r3(a.tiltFracMed)).padStart(9)} ${pct(a.tiltSaturatedRate).padStart(11)} ${String(r3(a.tiltDeltaMed)).padStart(10)} ${String(a.nTiltEvents).padStart(8)}`);
console.log(`\nSTANDING SET (screening numbers — no gate claims at N=${RACES})`);
console.log('s      lead  distinct   dead   duo    front@line  runaway  parade   band');
for (const a of armAgg) console.log(`${a.S.padEnd(6)} ${a.finaleLead.toFixed(2).padStart(4)} ${a.finaleDistinct.toFixed(2).padStart(9)} ${pct(a.deadRate).padStart(7)} ${pct(a.duoEscapeRate).padStart(6)} ${a.frontGroupAtLine.toFixed(2).padStart(11)} ${pct(a.runawayRate).padStart(8)} ${pct(a.paradeRate).padStart(7)} ${pct(a.bandReach).padStart(6)}`);
console.log(`\nper arm x track:`);
for (const r of TR) console.log(`  s=${r.S} ${r.track.padEnd(14)} depth_med=${String(r3(r.escapeDepthMed)).padStart(6)} p90=${String(r3(r.escapeDepthP90)).padStart(6)} tilts/race=${r.leaderDownPerRace.toFixed(2)} sat=${pct(r.tiltSaturatedRate)} lead=${r.finaleLead.toFixed(2)} dead=${pct(r.deadRate)} runaway=${pct(r.runawayRate)}`);
console.log(`\nwall-clock ${(wall / 60).toFixed(1)} min | duo thresholds tight<=${r3(TIGHT)} far>=${r3(FAR)}`);
writeFileSync(join(OUT_ABS, 'meta.json'), JSON.stringify({ arms: S_LIST, G: G_FIXED, races: RACES, seed: SEED, dur: DUR, tracks: TRACK_IDS, racersClosed: RACERS_CLOSED, racersOpen: RACERS_OPEN, tight: TIGHT, far: FAR, wallClockSec: wall }, null, 2));
