...
// RACE-IDENTIFIER-1 (2026-09-05): the field's accessible name became 'Race seed, key or identifier'
...
describe('SEED-REAL-RACE-1 — the seed outlives the session', () => {
  ...
  it('clearing the field REMOVES the key rather than storing an empty string', () => {
    renderStartable();
    openSettingsTab();
    const field = screen.getByLabelText('Race seed, key or identifier');
  ...
  it('the last race seed is offered back in the panel, so he need not have written it down', () => {
    ...
    const filled = screen.getByLabelText('Race seed, key or identifier').value;
