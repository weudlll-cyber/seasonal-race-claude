...
export function analyzeRacerJumps(frames, opts = {}) {
  ...
  if (racerNames.size === 0) {
    return {
      jumps: [],
      summary:
        "No racer data (rc) found in frames — log recorded without racer snapshots.",
    };
  }
  ...
  for (const name of racerNames) {
    ...
    for (let i = 0; i < frames.length; i++) {
      const f = frames[i];
      const rc = Array.isArray(f.rc)
        ? f.rc.find((r) => r.n === name)
        : undefined;
      if (rc)
        racerEntries.push({ globalIdx: i, fi: f.fi, ts: f.ts, st: f.st, rc });
    }

    for (const axis of ["dx", "dy", "dt"]) {
      const factor =
        axis === "dt" ? dtFactor : axis === "dx" ? dxFactor : dyFactor;
      const recentAbs = [];
  ...
  jumps.sort(
    (a, b) => a.frameIdx - b.frameIdx || a.racer.localeCompare(b.racer),
  );
...
function main() {
  ...
  if (!arg) {
    console.error(
      "Usage: node scripts/analyze-camera-log.mjs <path-to-log.json> [options]",
    );
    console.error("");
    console.error("Camera jump options:");
    console.error(
      "  --lerp_threshold=8    Method A: lerp residual threshold in px (default 8)",
    );
    console.error(
      "  --median_factor=5     Method B: rolling-median multiplier (default 5)",
    );
    console.error("");
    console.error("Racer jump options:");
    console.error(
      "  --racer_dx_factor=5   ΔX outlier factor per racer (default 5)",
    );
    console.error(
      "  --racer_dy_factor=5   ΔY outlier factor per racer (default 5)",
    );
    console.error(
      "  --racer_dt_factor=8   Δt outlier factor per racer (default 8)",
    );
    console.error("");
    console.error(
      "  --context=5           Context frames around each flagged frame (default 5)",
    );
    console.error("");
    console.error(
      "Export a log from the race screen (enableFrameLog toggle in DevScreen)",
    );
  ...
  if (!Array.isArray(frames)) {
    console.error(
      'Expected JSON to have a "frames" array (or be a bare array).',
    );
    process.exit(1);
  }
  ...
  if (camJumps.length > 0) {
    ...
    for (const [idx, j] of camJumps.entries()) {
      ...
      console.log(
        `  State:        ${j.state} / lerpPhase=${j.lerpPhase} / obs=${j.observerPhase}`,
      );
      console.log(`  deltaOffsetX: ${j.dox} px    deltaOffsetY: ${j.doy} px`);
      if (j.camTDelta !== null) {
        console.log(
          `  ΔcamT:        ${j.camTDelta}  (large ΔcamT + large Δoffset → track geometry)`,
        );
      }
      if (j.tSpacePin)
        console.log(
          `  T-space pin:  YES — offsetX was pinned to camT this frame`,
        );
      if (j.methodA) {
        console.log(
          `  Method A (lerp residual): expected≈${j.methodA.expected}px, residual=${j.methodA.residual}px`,
        );
      }
      if (j.methodB) {
        console.log(
          `  Method B (median outlier): |dox|=${j.methodB.absDox}px = ${j.methodB.factor}× median (${j.methodB.median}px)`,
        );
      }
      console.log(
        `  Context (${j.context.length} frames, flagged=[${j.flaggedIndex}]):`,
      );
      const header =
        "    fi       ts       st              lp        op          ox       z      dox    lf     ts2 tf  ct       fot     ese";
      console.log(header);
    ...
    console.log(
      "• Method A (lerp residual): actual move ≫ expected from lerp formula",
    );
    console.log(
      "  → Likely cause: T-space→pixel-space switch, target snap, or stale prevFocusT",
    );
    console.log(
      "• Method B (median outlier): spike vs. rolling median, no transition",
    );
    console.log(
      "  → Likely cause: single-frame speed-estimate spike or lap-wrap edge case",
    );
    console.log(
      "• Large ΔcamT alongside large Δoffset: may be normal tToWorld() curvature at corners",
    );
    console.log(
      "• lp=entry + ts2=1 + large dox: camera is T-space pinned — check if camT itself jumped",
    );
    console.log(
      "• lp=entry→tracking transition frame: pixel-lerp target may snap; one-frame artefact",
    );
  }

  // ── Racer jump analysis ───────────────────────────────────────────────────
  const hasRacerData = frames.some(
    (f) => Array.isArray(f.rc) && f.rc.length > 0,
  );
  if (!hasRacerData) {
    console.log(
  ...
  console.log("\n=== Racer Jump Analysis ===");
  const { jumps: racerJumps, summary: racerSummary } = analyzeRacerJumps(
    frames,
    flags,
  );
  ...
  if (racerJumps.length > 0) {
    ...
    for (const [idx, j] of racerJumps.entries()) {
      ...
      console.log(
        `  Axis:   ${j.axis}   value=${j.value}   factor=${j.factor}×median(${j.median})   state=${j.state}`,
      );
      console.log(
        `  Context (${j.context.length} frames, flagged=[${j.flaggedIndex}]):`,
      );
      const header =
        "    fi       ts       st               t         x         y        dx       dy       dt       sp";
      console.log(header);
      for (const [ci, cf] of j.context.entries()) {
        const marker = ci === j.flaggedIndex ? ">>>" : "   ";
        const rc = cf.rc;
    ...
    console.log(
      "• Large dx/dy spike: racer teleported in pixel space — check getPosition() or t-wrap logic",
    );
    console.log(
      "• Large dt spike: sudden t-value jump — check speed-bonus, re-roll, or constSpeed logic",
    );
    console.log(
      "• Correlate racer jumps with camera state (st) — if camera also jumped, shared cause likely",
    );
    console.log(
      "• Multiple racers same frame: likely a global state change (re-roll, lap-wrap, race-start)",
    );
  }
}
