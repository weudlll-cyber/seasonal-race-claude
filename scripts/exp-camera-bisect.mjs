...
const MODE = (
  process.argv.find((a) => a.startsWith("--mode=")) || "--mode=ladder"
).split("=")[1];
if (!DUMP) {
  console.error(
    "usage: node scripts/exp-camera-bisect.mjs <dump.json> [--mode=ladder|tc|transitions]",
  );
  process.exit(1);
}
...
const { DEFAULT_CAMERA_CONFIG } = await import(
  u(join(ROOT, "client/src/modules/cameraConfig.js"))
);
...
const shape = new EditorShape(
  JSON.parse(
    readFileSync(
      join(ROOT, "server/seeds/tracks", `${meta.track}.json`),
      "utf8",
    ),
  ),
);
...
function mkConfig(tc = 0.25) {
  ...
  c.cameraStateProfiles.LEAD_CHANGE = {
    ...c.cameraStateProfiles.LEAD_CHANGE,
    trackingTC: tc,
  };
...
function _replay(CameraDirector, CAM_STATE, tc, want = {}) {
  ...
  for (const f of frames) {
    const racers = f.racers,
      live = racers.filter((r) => !r.finished);
    ...
    const out = cd.update(
      ...
      f.t,
      {
        raceElapsed: f.t,
        finishT,
        finishedCount: racers.length - live.length,
        physicsRacers: racers,
        isOutcomePhase: maxT / finishT >= 0.75,
        winner: null,
      },
      CW,
      CH,
    ...
    if (LEADERFAM.has(st) && live.length) {
      ...
      const ez = cd.zoom * bsX,
        sx = leader.x * ez + out.offsetX,
        sy = leader.y * ez + out.offsetY;
      const outside = sx < MX || sx > CW - MX || sy < MY || sy > CH - MY;
      full.lF++;
      if (outside) full.lOut++;
      if (isEarly) {
        early.lF++;
        if (outside) early.lOut++;
      }
    }
    if (prevOff) {
      const dx = out.offsetX - prevOff.x,
        v = Math.hypot(dx, out.offsetY - prevOff.y);
      full.vels.push(v);
      full.flipDen++;
      if (Math.abs(dx) > FLIP_EPS) {
      ...
      if (isEarly) {
        ...
        early.flipDen++;
        if (Math.abs(dx) > FLIP_EPS) {
          if (prevDxE !== null && Math.sign(dx) !== Math.sign(prevDxE))
            early.flips++;
          prevDxE = dx;
        }
      }
      ...
      if (trans) {
        velTrans.push(v);
        if (isEarly) transEarly++;
      } else velNoTrans.push(v);
    }
    prevOff = { x: out.offsetX, y: out.offsetY };
    prevState = st;
  }
  const pack = (w) => ({
    lOutPct: w.lF ? (100 * w.lOut) / w.lF : null,
    lF: w.lF,
    panVar: variance(w.vels),
    flipsPer100: w.flipDen ? (100 * w.flips) / w.flipDen : 0,
  });
  ...
  if (want.transitions)
    r.trans = {
      earlyFrames,
      transEarly,
      meanVelTrans: mean(velTrans),
      meanVelNoTrans: mean(velNoTrans),
      maxVelTrans: velTrans.length ? Math.max(...velTrans) : 0,
    };
  return r;
}
...
function withRung(sha, fn) {
  ...
  try {
    execFileSync("git", ["worktree", "add", "--detach", "-q", wt, sha], {
      cwd: ROOT,
    });
    return fn(wt);
  } finally {
...
console.log(
  `CAMERA-FOCUS-2 bisect — ${meta.track} seed ${meta.seed}, ${meta.nRacers} racers, ${frames.length} frames, LEADER zoom 3, mode=${MODE}`,
);
...
if (MODE === "ladder") {
  ...
  for (const [sha, label] of COMMITS) {
    const r = await withRung(sha, async (wt) => {
      const m = await import(
        u(join(wt, "client/src/modules/camera/CameraDirector.js"))
      );
  ...
  for (const r of rows) {
    console.log(
      `${(r.sha + " " + r.label).padEnd(29)} | ${f(r.full.lOutPct).padStart(8)}% | ${f(r.full.panVar, 0).padStart(11)} | ${f(r.full.flipsPer100).padStart(9)} | ${f(r.early.lOutPct).padStart(9)}% | ${f(r.early.panVar, 0).padStart(12)} | ${f(r.early.flipsPer100).padStart(9)}`,
    );
  }
  console.log("\nJSON", JSON.stringify(rows));
} else if (MODE === "tc") {
  const TCS = [0.25, 0.15, 0.1, 0.06];
  for (const [sha, label] of [
    ["9e351c02", "pre-clamp"],
    ["60551847", "today (clamp)"],
  ]) {
    await withRung(sha, async (wt) => {
      ...
      for (const tc of TCS) {
        ...
        console.log(
          `  ${tc.toFixed(2)} | ${r.full.lOutPct.toFixed(1).padStart(8)}% | ${r.full.panVar.toFixed(0).padStart(7)} | ${r.early.lOutPct.toFixed(1).padStart(9)}% | ${r.early.panVar.toFixed(0).padStart(8)}`,
        );
      }
    });
  }
  ...
  console.log(
    `  EARLY window: ${t.earlyFrames} frames, ${t.transEarly} state-transitions`,
  );
  console.log(
    `  mean pan-vel AT transition = ${t.meanVelTrans.toFixed(1)}px  vs  non-transition = ${t.meanVelNoTrans.toFixed(1)}px  (max transition cut = ${t.maxVelTrans.toFixed(0)}px)`,
  );
}
