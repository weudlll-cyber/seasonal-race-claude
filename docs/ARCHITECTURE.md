...
**Start phase as setup for race phase:** The start phase determines the lateral distribution of racers entering the race phase. If `startPhaseAvoidanceFactor` is too low, all racers collapse toward physicalY=0 before the race phase starts; in the race phase a dense cluster causes symmetric force cancellation (avoidance from both +Y and -Y neighbors cancels), trapping racers at center despite strong per-pair forces. Default `startPhaseAvoidanceFactor = 0.6` (tuned via force decomposition trace, see `docs/diagnose/avoidance-force-decomposition.md`) makes avoidance 2.4× stronger than home-force at start, spreading racers to near-maximum width before the race phase begins. See Lesson 72.
