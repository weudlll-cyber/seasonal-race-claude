...
const TRACKS = [ { id: 'searound', racer: 'manta' }, { id: 'luger-hill', racer: 'luge' }, { id: 'seatrack', racer: 'dolphin' }, { id: 'space-sprint', racer: 'rocket' } ];
const BE = [5, 15, 25, 40]; const zi = (r) => { for (let i = 0; i < BE.length; i++) if (r <= BE[i]) return i; return BE.length; };
const results = [];
for (const N of NLIST) {
  for (const t of TRACKS) {
    const out = join(OUT, `${LABEL}-${t.id}-N${N}`); const t0 = Date.now();
    execFileSync(process.execPath, ['scripts/sim-fairness.mjs', `--track=${t.id}`, `--racer=${t.racer}`, '--seed=1', `--races=${N}`, '--track-defaults', '--hero-map', '--runaway-parade', `--out=${out}`], { cwd: ROOT, maxBuffer: 512 * 1024 * 1024 });
    ...
    let runaway = null; try { const rp = JSON.parse(readFileSync(join(out, 'runaway-parade.json'), 'utf8')).races; runaway = rp.length ? rp.filter((r) => r.runawayParade?.runaway).length / rp.length : null; } catch { /* */ }
    const rec = { label: LABEL, track: t.id, racer: t.racer, N, bandReach: hm.fairness?.bandReach ?? null, rowMin, holm: hm.fairness?.startRowUnfair ? 'UNFAIR' : 'ok', holmP: hm.fairness?.startRowMinPHolm ?? null, runaway, secs: Math.round((Date.now() - t0) / 1000) };
    results.push(rec);
    console.log(`[${LABEL} N=${N}] ${t.id}/${t.racer}: band=${(rec.bandReach * 100).toFixed(1)}% rowMin=${(rowMin * 100).toFixed(0)}% ${rec.holm}(p=${rec.holmP == null ? '?' : rec.holmP.toFixed(3)}) runaway=${runaway == null ? '?' : (runaway * 100).toFixed(0) + '%'} (${rec.secs}s)`);
  }
}
