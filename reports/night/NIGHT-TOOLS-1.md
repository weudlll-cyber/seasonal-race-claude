...
The inputs to the cost table only move when a guard's _work_ changes — more tracks, more frames, more
...
against commit X" stamp could not be _generated_ — verification is a human reading, and a generator
could only stamp _when a file was last touched_, which is not the same claim and would be worse than
...
- **CAMERA_DIRECTOR.md** — accurate, _but_ you cannot run a camera check from it alone: it names the
...
- **SIM.md** does _not_ completely describe the shipped world: it never names 8 of the 19 files the
