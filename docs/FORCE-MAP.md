...
- **Realism envelope** ([`raceGovernor.js` → `brakeLoBound`](../client/src/modules/raceGovernor.js#L420-L443)): a **`±pulkEnvelopeMaxEffect`** clamp on `|governorMult − 1|`, a per-frame **`pulkEnvelopeMaxStepPerFrame`** slew limit, and an optional **naturalness ceiling cap** (`pulkCeilingCap` **true** → `computeDirectorCeiling`, hard-capped at `NATURALNESS_CEILING` = **1.2**, `pulkBoostHeadroom` **0.1** additive headroom above the band max).
