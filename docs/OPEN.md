...
| ★★ **Three production-arm specs FAIL — `arrival-shape`, `comeback-precedence`, `garden-path-finishes`** | **triaged 2026-09-25; one needs HIS WORD, one needs a plan dump, one needs a suite run** | ★ **Each run THREE TIMES ALONE** (THREE-FAILING-SPECS-1). `garden-path-finishes` **passes 3/3 alone** — its failure belongs to SUITE CONTEXT. `arrival-shape` fails 3/3 at `:155`, received **0** where > 0.5 is required — and whether that is a regression or a dead premise is **already item 1 on his own "needs your word" list**. `comeback-precedence` fails 3/3 at `:111`: the race's only comeback cut comes **from OVERVIEW**, which has no hold to cut through, so the signature never occurs in that fixture. ★ Nothing fixed, no assertion changed, still in no gate. → [BACKLOG.md](BACKLOG.md) · [THREE-FAILING-SPECS-1.md](../reports/evolution/THREE-FAILING-SPECS-1.md) |
