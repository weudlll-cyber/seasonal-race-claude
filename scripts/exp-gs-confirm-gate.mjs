...
const OUT_ABS = (() => {
  const r = argVal("out", "reports/parity/gs-confirm-data");
...
const SCALE_TRACK_IDS = [
  "luger-hill",
  "mountainstreet",
  "searound",
  "dirt-oval",
]; // the 4 standard scaling tracks
const trackSeed = (id) =>
  JSON.parse(
...
console.log(
  `CONTROL G=0.75/s=0.5 (shipped)  vs  CANDIDATE G=0.5/s=1.0 | N=${RACES}/arm/track | ${TRACKS.length} tracks × 2 arms = ${TRACKS.length * 2 * RACES} races | paired seeds | jobs=${JOBS}`,
);

const tasks = [];
for (const arm of ARMS)
  for (const t of TRACKS) tasks.push(() => runTrack(t, arm, RACES));
...
for (const t of TRACKS) {
  ...
  const ac = aggregate([rc], allRows),
    ad = aggregate([rd], allRows);
  const dBand =
    rd.bandReach != null && rc.bandReach != null
      ? (rd.bandReach - rc.bandReach) * 100
      : null;
  console.log(
  ...
  perTrack.push({
    ...
    closed: t.closed,
    control: {
      bandReach: rc.bandReach,
      holm: rc.startRowUnfair,
      holmP: rc.startRowMinPHolm,
      servoSat: rc.servoSatRate,
      ...ac,
    },
    candidate: {
      bandReach: rd.bandReach,
      holm: rd.startRowUnfair,
      holmP: rd.startRowMinPHolm,
      servoSat: rd.servoSatRate,
      ...ad,
    },
    dBandPp: dBand,
  });
...
function printPooled(label, a) {
  ...
  console.log(
    `  band-reach (racer-row weighted): ${pct(a.bandReachPooled)}   ${a.bandReachPooled >= 0.7 ? "CLEARS 70%" : "BELOW 70%"}`,
  );
  console.log(
    `  Holm-unfair tracks: ${a.holmFlaggedTracks}/${a.holmTracksTotal} | dead ${pct(a.deadRate)} | front@line ${a.frontGroupAtLine.toFixed(2)} | leadChanges ${a.finaleLead.toFixed(2)} | distinct ${a.finaleDistinct.toFixed(2)}`,
  );
  console.log(
    `  runaway/parade/duo ${pct(a.runawayRate)}/${pct(a.paradeRate)}/${pct(a.duoRate)} | escDep med/p90/max ${r3(a.escapeDepthMed)}/${r3(a.escapeDepthP90)}/${r3(a.escapeDepthMax)} | servoSat ${pct(a.servoSatPooled)}`,
  );
}
...
const scaleTracks = SCALE_TRACK_IDS.map((id) =>
  TRACKS.find((t) => t.id === id),
);
const candArm = ARMS.find((a) => a.key === "candidate");
const scaleTasks = [];
for (const d of DUR_VARIANTS)
  for (const t of scaleTracks)
    scaleTasks.push(() => runTrack(t, candArm, SCALE_RACES, { durSec: d }));
...
for (const d of DUR_VARIANTS) {
  ...
  console.log(
    `${String(d).padStart(3)}s  ${pct(a.bandReachPooled).padStart(10)}  ${pct(a.runawayRate).padStart(8)} ${pct(a.paradeRate).padStart(7)} ${pct(a.deadRate).padStart(5)} ${pct(a.servoSatPooled).padStart(9)}`,
  );
  scaleRows.push({
    dur: d,
    ...
    deadRate: a.deadRate,
    servoSatPooled: a.servoSatPooled,
    perTrack: dRuns.map((r) => ({
      track: r.track.id,
      bandReach: r.bandReach,
      servoSat: r.servoSatRate,
    })),
  });
}
...
writeFileSync(
  join(OUT_ABS, "gs-confirm-gate.json"),
  JSON.stringify(
    {
      normalSpeed: NORMAL_SPEED,
      races: RACES,
