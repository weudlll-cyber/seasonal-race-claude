...
const ARM_KEYS = argVal("arms", Object.keys(ALL_ARMS).join(","))
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
async function run(armKey, flags, track) {
  ...
  for (const r of rp) {
    ...
    for (const [row, pr] of Object.entries(r.chaosSteer.perRow)) {
      ...
      a.n += pr.n;
      a.steered += pr.steered;
      a.ticks += pr.ticks;
      a.multSum += pr.multSum;
      a.inBandEnd += pr.inBandEnd;
    }
  }
  ...
  const flPct = (pred) =>
    fl.length ? fl.filter(pred).length / fl.length : null;
  const flAvgNonNull = (f) => {
    const v = fl.map(f).filter((y) => y != null);
    return v.length ? mean(v) : null;
  };
  const pulk = fl.length
    ? {
        // CHAOS window [0, chaosEnd]
        maxGapChaos: flA((x) => x.maxGapP1P2_chaos),
        gapAtChaosEnd: flAvgNonNull((x) => x.gapAtChaosEnd),
        ...
        leaderDrawnB1_025Pct: flPct(
          (x) =>
            x.leaderDrawnRankAtChaosEnd != null &&
            x.leaderDrawnRankAtChaosEnd <= 5,
        ),
        // PULK window [chaosEnd, 0.60]
        maxLeadHoldShare_mid: flA((x) => x.maxLeadHoldShare_mid),
        distinctLeaders_mid: flA((x) => x.distinctLeaders_mid),
        leadChanges_mid: flA((x) => x.leadChanges_mid),
        meanGapP1P2_mid: flA((x) => x.meanGapP1P2_mid),
        maxGapP1P2_mid: flA((x) => x.maxGapP1P2_mid),
        earlyBreakawayPct: flPct((x) => x.earlyBreakaway),
        leaderIsDrawnB1_mid: flA((x) => x.leaderIsDrawnB1_mid),
        firstLeadChange: flAvgNonNull((x) => x.firstLeadChangeFromChaosEnd),
        // BRAKE (addendum): trigger-armed share + actual fires + the design gate fraction
        brakeArmed_mid: flAvgNonNull((x) => x.brakeArmed_mid),
        brakeFires_chaos: flA((x) => x.brakeFires_chaos),
        brakeFires_mid: flA((x) => x.brakeFires_mid),
        gapCapGateFrac: fl[0]?.gapCapGateFrac ?? null,
        // LAW
        LAW_full: flA((x) => x.LAW_full),
        LAW_last50: flA((x) => x.LAW_last50),
      }
    : null;
  ...
  for (const r of rp) {
    ...
    if ((r.frontBattle?.frontContestFraction ?? 0) < 0.3) boring++;
    else thriller++;
  }
  ...
  const inBandEnd = mean(
    cs.map((c) => (c.nField ? c.inBandEnd / c.nField : 0)),
  );
  const mm = cs.map((c) => c.meanMult).filter((x) => x != null);
  const steerTele = {
    inBandEnd,
    steeredShare: mean(
      cs.map((c) => (c.nField ? (c.steeredRacers ?? 0) / c.nField : 0)),
    ),
    steeredTicks: mean(cs.map((c) => c.steeredTicks ?? 0)),
    meanMult: mm.length ? mean(mm) : null,
    maxTickDelta: cs.length
      ? Math.max(...cs.map((c) => c.maxTickDelta ?? 0))
      : 0,
  };
  return {
    arm: armKey,
    track: track.id,
    arrival: hm.fairness?.bandReach ?? null,
    ...
    closed: track.closed,
    fc,
    distinctLead: mean(rp.map((r) => r.frontBattle?.distinctLeaders ?? 0)),
...
for (const [k, f] of Object.entries(ARMS))
  for (const t of TRACKS) tasks.push(() => run(k, f, t));
...
for (const armKey of Object.keys(ARMS)) {
  ...
  for (const t of TRACKS) {
    const r = get(armKey, t.id),
      s = get("ship", t.id);
    const dArr = s ? (r.arrival - s.arrival) * 100 : 0,
      dFc = s ? (r.fc - s.fc) * 100 : 0,
      dDB = s ? (r.deadBoring - s.deadBoring) * 100 : 0;
    const sgn = (x) => (x >= 0 ? "+" : "") + x.toFixed(0) + "pp";
    console.log(
      `  ${t.id.padEnd(13)}${r.closed ? "C" : "O"} | ARRIVAL ${pct(r.arrival)} (${armKey === "ship" ? "—" : sgn(dArr)}) rowMin ${pct(r.rowMin)} ${r.holm} | fC ${pct(r.fc)} (${armKey === "ship" ? "—" : sgn(dFc)}) | DEAD ${pct(r.dead)} BORING ${pct(r.deadBoring)} (${armKey === "ship" ? "—" : sgn(dDB)}) thrill ${pct(r.deadThriller)} | dLead ${r.distinctLead.toFixed(2)} maxHold ${pct(r.maxLeadHold)} p1s ${r.p1MultiSec.toFixed(1)}`,
    );
    // CHAOS-STEER-1 scorecard line: in-band-at-chaos-end (the direct Part-1 number) + delta vs ship, and steer telemetry.
    const st = r.steerTele,
      ss = s?.steerTele;
    const dIB = ss ? (st.inBandEnd - ss.inBandEnd) * 100 : 0;
    console.log(
      `     └chaos-end in-band ${pct(st.inBandEnd)} (${armKey === "ship" ? "—" : sgn(dIB)}) | steer: share ${pct(st.steeredShare)} · ticks/race ${st.steeredTicks.toFixed(0)} · meanMult ${st.meanMult == null ? "n/a" : st.meanMult.toFixed(3)} · maxTickΔ ${st.maxTickDelta.toFixed(4)}`,
    );
    // PULK-SPECTACLE-1: chaos / pulk / brake windows (the owner's mid-race-flat finding).
    const p = r.pulk;
    if (p) {
      const f1 = (x) => (x == null ? "n/a" : x.toFixed(1)),
        f2 = (x) => (x == null ? "n/a" : x.toFixed(2));
      console.log(
        `     └CHAOS | maxGap ${f1(p.maxGapChaos)}L · handover@end ${f1(p.gapAtChaosEnd)}L · fieldSpread ${f1(p.fieldSpreadAtChaosEnd)}L · leaderSteered ${pct(p.leaderSteered025Pct)}(mult ${f2(p.leaderSteerMeanMult)}) · leaderDrawnB1 ${pct(p.leaderDrawnB1_025Pct)}`,
      );
      console.log(
        `     └PULK  | maxHold ${f2(p.maxLeadHoldShare_mid)} · distinctLead ${f1(p.distinctLeaders_mid)} · earlyBreak ${pct(p.earlyBreakawayPct)} · leaderDrawnB1_mid ${f2(p.leaderIsDrawnB1_mid)} · meanGap ${f1(p.meanGapP1P2_mid)}L · 1stLeadChg ${f2(p.firstLeadChange)}`,
      );
      console.log(
        `     └BRAKE | armed_mid ${f2(p.brakeArmed_mid)} · fires chaos/mid ${f1(p.brakeFires_chaos)}/${f1(p.brakeFires_mid)} · gate ${f2(p.gapCapGateFrac)} · LAW ${f2(p.LAW_full)}/${f2(p.LAW_last50)}`,
      );
    }
    // ROW-SKEW DIAGNOSIS: per-start-row band-reach (all arms) + per-row steer exposure (steer/combo arms).
    const rows = [...new Set([...r.rowBandReach.keys()])].filter(
      (i) => r.rowBandReach[i] != null,
    );
    const brLine = rows.map((i) => `r${i}:${pct(r.rowBandReach[i])}`).join(" ");
    console.log(`     └row band-reach | ${brLine}`);
    if (Object.keys(r.rowSteer).length) {
      const exLine = Object.keys(r.rowSteer)
        .sort((a, b) => a - b)
        .map((row) => {
          const a = r.rowSteer[row];
          const share = a.n ? a.steered / a.n : 0,
            mm = a.ticks ? a.multSum / a.ticks : null;
...
if (ARM_KEYS.includes(GATE_ARM)) {
  ...
  let arrivalOK = 0,
    failBoth = 0,
    belowShip = 0;
  const failEverywhere = { R: [], F: [], B: [], P: [], P2: [] },
    holmWorse = [];
  const rows = [];
  for (const t of TRACKS) {
    const s = get("ship", t.id),
      c = get(GATE_ARM, t.id);
    const aDelta = c.arrival >= s.arrival + 0.1,
      aAbs = c.arrival >= 0.88,
      notBelow = c.arrival >= s.arrival - 1e-9;
    const A = (aDelta || aAbs) && notBelow;
    if (A) arrivalOK++;
    if (notBelow && !aDelta && !aAbs) failBoth++;
    if (!notBelow) belowShip++;
    const R = c.rowMin >= s.rowMin - 1e-9;
    if (!R) failEverywhere.R.push(t.id);
    const F = c.fc >= s.fc - 0.02;
    if (!F) failEverywhere.F.push(t.id);
    const B = c.deadBoring <= s.deadBoring + 0.02;
    if (!B) failEverywhere.B.push(t.id);
    ...
    const P = pHold && pDist && pGap;
    if (!P) failEverywhere.P.push(t.id);
    const P2 = pHold && pDist && pGap2;
    if (!P2) failEverywhere.P2.push(t.id);
  ...
  console.log(
    `  arrival: OK-form ${arrivalOK}/10 (need ≥8), failBoth ${failBoth} (need 0), belowShip ${belowShip} (need 0) → ${arrivalGate ? "PASS" : "FAIL"}`,
  );
  console.log(
    `  rowMin≥ship: ${everywhere("R") ? "PASS" : "FAIL " + failEverywhere.R.join(",")} | fC≥ship−2: ${everywhere("F") ? "PASS" : "FAIL " + failEverywhere.F.join(",")} | DEAD-BORING≤ship+2: ${everywhere("B") ? "PASS" : "FAIL " + failEverywhere.B.join(",")}`,
  );
  console.log(
    `  PULK watchdog v1 (abs ship+1.0L): ${everywhere("P") ? "PASS" : "FAIL " + failEverywhere.P.join(",")} | v2 (ratio ship×1.5, PREREG): ${everywhere("P2") ? "PASS" : "FAIL " + failEverywhere.P2.join(",")} | Holm worsened: ${holmWorse.length ? holmWorse.join(",") : "none"}`,
  );
}
console.log(
