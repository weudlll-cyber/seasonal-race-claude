...
//
// THE THRESHOLD IS A DEFECT LINE, NOT A TUNING KNOB, and the measurement says so. A healthy run-in
// keeps the centre within 0.1 track widths of the spine; the defect reached 10.9. Two track widths
// sits 20x above the healthy maximum and 5x below the observed fault, so every value between about
// 1 and 5 gives the identical verdict — which is exactly the property a defect line should have and
// a tuned number should not. It is deliberately NOT set close to the healthy value: this guard is
// here to catch a camera pointed at nothing, not to police framing taste.
//
...
// THE CONTROL ARM. Runs the same two races with `finishLineFraming` off, which is the behaviour the
// guard's threshold was chosen against. It exists so the threshold can be shown to DISCRIMINATE
// rather than merely to be satisfied — a limit nobody has seen fail and pass is a guess.
