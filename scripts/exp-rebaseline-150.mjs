...
const OUT_ABS = (() => { const r = argVal('out', 'reports/parity/rebaseline-data'); return isAbsolute(r) ? r : join(ROOT, r); })();
...
function derivedDuration(track) {
  ...
  const requestedSeconds = track.closed ? 0 : trackDefaultSeconds(track.seed, track.pathLengthPx, pace, runout);
  const m = deriveRaceDuration({ isOpen: !track.closed, pathLengthPx: track.pathLengthPx, laps, requestedSeconds, normalSpeedPxPerSec: NORMAL_SPEED, speedMultiplier: M, runoutZone: runout });
  return { durSec: m.realizedDurationSec, paceScale: m.paceScale, slowdown: !!m.slowdownActive };
}
...
function servoSaturatedRate(ssJson) {
  ...
  for (const race of ssJson.races ?? []) {
    ...
    for (const prog of Object.keys(samples)) {
      for (const rec of samples[prog]) { tot++; if (rec.servoSaturated) sat++; }
    }
  }
...
async function runTrack(track, races, { durSec = null } = {}) {
  ...
  try { ssRate = servoSaturatedRate(JSON.parse(readFileSync(ssPath, 'utf8'))); } catch { /* speed-source optional */ }
  ...
  const dd = durSec == null ? derivedDuration(track) : { durSec, paceScale: null, slowdown: null };
  return {
    track, rows, durSec: dd.durSec, paceScale: dd.paceScale, slowdown: dd.slowdown,
...
console.log(`tracks ${TRACK_IDS.join(', ')} | N=${RACES}/track => ${RACES * TRACKS.length} pooled races | racers ${RACERS_CLOSED} closed / ${RACERS_OPEN} open | canonical per-track defaults | gap-reroll ON default`);
...
const perTrack = runs.map((r) => {
  ...
  console.log(`${r.track.id.padEnd(15)} ${(r.track.closed ? 'closed' : 'open').padEnd(6)} ${String(a.nRaces).padStart(3)}  ${pct(r.bandReach).padStart(8)}  ${(r.startRowUnfair ? 'UNF' : 'ok').padStart(4)}  ${pct(a.deadRate).padStart(5)} ${a.frontGroupAtLine.toFixed(2).padStart(11)} ${pct(a.runawayRate).padStart(8)} ${pct(a.paradeRate).padStart(7)} ${pct(a.duoRate).padStart(6)} ${String(r3(a.escapeDepthMed)).padStart(10)} ${String(r3(a.escapeDepthP90)).padStart(10)} ${pct(r.servoSatRate).padStart(9)} ${r.durSec.toFixed(1).padStart(7)}`);
  return { track: r.track.id, closed: r.track.closed, bandReach: r.bandReach, startRowUnfair: r.startRowUnfair, startRowMinPHolm: r.startRowMinPHolm, servoSatRate: r.servoSatRate, durSec: r.durSec, ...a };
});

console.log(`\n── POOLED (${agg.nRaces} races) ──`);
console.log(`band-reach (racer-row weighted) : ${pct(agg.bandReachPooled)}   ${agg.bandReachPooled >= 0.70 ? 'CLEARS 70%' : 'BELOW 70%'}`);
console.log(`Holm-unfair start-row tracks    : ${agg.holmFlaggedTracks}/${agg.holmTracksTotal}`);
console.log(`runaway / parade / duo          : ${pct(agg.runawayRate)} / ${pct(agg.paradeRate)} / ${pct(agg.duoRate)}`);
console.log(`dead finales                    : ${pct(agg.deadRate)}`);
console.log(`front group @ line              : ${agg.frontGroupAtLine.toFixed(2)}`);
console.log(`finale lead-changes / distinct  : ${agg.finaleLead.toFixed(2)} / ${agg.finaleDistinct.toFixed(2)}`);
console.log(`escape depth med / p90 / max    : ${r3(agg.escapeDepthMed)} / ${r3(agg.escapeDepthP90)} / ${r3(agg.escapeDepthMax)}`);
console.log(`saturated-correction rate       : ${pct(agg.servoSatPooled)}`);
console.log(`mean derived duration           : ${agg.durMean.toFixed(1)} s`);

// 70% verdict
const verdictPass = agg.bandReachPooled != null && agg.bandReachPooled >= 0.70;
...
const scaleTasks = [];
for (const d of DUR_VARIANTS) for (const t of TRACKS) scaleTasks.push(() => runTrack(t, SCALE_RACES, { durSec: d }));
...
for (const d of DUR_VARIANTS) {
  for (const t of TRACKS) {
    ...
    console.log(`${String(d).padStart(3)}s  ${t.id.padEnd(15)} ${pct(r.bandReach).padStart(8)}  ${pct(a.runawayRate).padStart(8)} ${pct(a.paradeRate).padStart(7)} ${pct(a.deadRate).padStart(5)} ${pct(r.servoSatRate).padStart(9)}`);
    scaleRows.push({ dur: d, track: t.id, bandReach: r.bandReach, runawayRate: a.runawayRate, paradeRate: a.paradeRate, deadRate: a.deadRate, servoSatRate: r.servoSatRate });
  }
  const dRuns = scaleRuns.filter((x) => x.durSec === d);
  const dAgg = aggregate(dRuns, scaleAllRows);
  console.log(`      ${('POOLED ' + d + 's').padEnd(15)} ${pct(dAgg.bandReachPooled).padStart(8)}  ${pct(dAgg.runawayRate).padStart(8)} ${pct(dAgg.paradeRate).padStart(7)} ${pct(dAgg.deadRate).padStart(5)} ${pct(dAgg.servoSatPooled).padStart(9)}`);
}

// ── write machine-readable output ────────────────────────────────────────────────────────────
writeFileSync(join(OUT_ABS, 'rebaseline-150.json'), JSON.stringify({
  normalSpeed: NORMAL_SPEED, races: RACES, seed: SEED, scaleRaces: SCALE_RACES,
  pooled: agg, perTrack, verdictPass, durationScaling: scaleRows,
}, null, 2) + '\n');
