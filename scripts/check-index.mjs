...
export const GUARD = {
  id: "report-index",
  covers:
    "reports/ and its INDEX files, both directions: no unindexed report, no index link to a missing file",
  blind: [
    "it does not descend into subdirectories, so reports/night/captures/ is deliberately unchecked",
    "it does not read what an index entry SAYS - only that the file it names exists",
  ],
  dirs: ["reports/", "docs/"],
  files: [],
  reach: [],
  cmd: ["node", "scripts/check-index.mjs"],
  also: [["node", "scripts/check-index.mjs", "--dir=reports/night", "--index=reports/night/INDEX.md"], ["node", "scripts/check-index.mjs", "--dir=reports/parity", "--index=reports/parity/INDEX.md"]],
};
