...
const R = (index, t) => ({ index, t, finished: false, isHeroChoreographed: false, trajectoryMult: 1.0 });
...
test('CLOSED: link is the SHORT arc and is CONTINUOUS across the 0/1 lap seam', () => {
  ...
  for (const lt of leaderTs) {
    const { links } = consecutiveLinks([R(0, lt), R(1, chaser)], /*isOpen*/ false, LEN);
    ...
    if (prev != null) assert.ok(Math.abs(len - prev) < 20, `discontinuity across the seam: ${prev} → ${len}`);
    prev = len;
  }
});
...
test('CLOSED: arcT picks the short way round (0.98 vs 0.02 = 0.04 lap, not 0.96)', () => {
  ...
  assert.ok(Math.abs(links[0].len - 4.0) < 1e-6, `expected 4 lengths (short arc), got ${links[0].len}`);
});

test('OPEN: link is the raw monotonic difference (no wrap)', () => {
  const { links } = consecutiveLinks([R(0, 0.60), R(1, 0.50)], /*isOpen*/ true, LEN);
  assert.ok(Math.abs(links[0].len - 10.0) < 1e-6, `expected 10 lengths, got ${links[0].len}`);
});
...
test('observer reads-only and produces per-cap duty + hero adjacency', () => {
  ...
  const field = [R(0, 0.50), R(1, 0.42), R(2, 0.415), R(3, 0.41)];
  ...
  const obs = makeCohesionObserver({ isOpen: false, lenScale: LEN, finishT: 1.0 });
  const before = field.map((r) => ({ ...r }));
  obs.onFrame(field, 0.5);
  // READ-ONLY: the observer must not mutate any racer field.
  for (let i = 0; i < field.length; i++)
    assert.deepEqual({ index: field[i].index, t: field[i].t, finished: field[i].finished }, { index: before[i].index, t: before[i].t, finished: before[i].finished });
  const res = obs.result();
  const cap3 = res.perCap.find((p) => p.cap === 3);
  assert.equal(cap3.dutyMaxCandidates, 1, 'exactly one over-3L link (the 8L front gap)');
  assert.equal(res.heroFrontmost.heroHero, 1, 'the frontmost over-3L link is hero→hero');
});
