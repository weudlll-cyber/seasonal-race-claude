...
- **Speicher:** localStorage-Key `racearena:playerGroups` (`KEYS.PLAYER_GROUPS`), **Array von `{ id, name, players: string[] }`**. `DEFAULT_PLAYER_GROUPS = []` (KEINE ausgelieferten Defaults). Quelle: `storage/storage.js:10`, `defaults.js:195`, `PlayerGroupsManager.jsx:22, 39-44`. ~~**[SUPERSEDED]** `DEFAULT_PLAYER_GROUPS` was removed (A-series cleanup); player-groups are now managed server-side, not in localStorage.~~
