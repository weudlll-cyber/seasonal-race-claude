...
const protocolLaps = lapsForApproxSeconds(
  60,
  pathLengthPx,
  paceSpeedPxPerSec(NORMAL_SPEED, spd),
);
...
function run(seed, laps, model) {
  ...
  const result = runSingleRace({
    ...
    racePlanController: controller,
    racerTargetRankMap: plan._racerTargetRank,
    frameHook,
  });
  const finish = new Array(N);
  for (const r of result)
    finish[r.racerIndex] = { rank: r.finalRank, timeMs: r.finishTime };
  return { cps, finish, result };
}

function orderByFinish(finish) {
  return finish
    .map((f, idx) => ({ idx, rank: f.rank, timeMs: f.timeMs }))
...
console.log(
  "=== searound / manta / 40 — canonical duration model, A/B arm diff ===",
);
console.log(
  `geometry: pathLengthPx=${pathLengthPx.toFixed(1)} width=${geometricTrackWidth} rows=${totalRows} isOpen=${isOpen}`,
);
console.log(
  `normal track speed: ${NORMAL_SPEED} px/s   (legacy 60 s protocol input would map to ${protocolLaps} laps here — a different race by design)`,
);
console.log(
  `Arm A (sim call site)     : laps=${LAPS}  finishT=${modelA.finishT}  realizedDur=${modelA.realizedDurationSec.toFixed(6)}s  raceBaseSpeed=${modelA.raceBaseSpeed.toExponential(9)}`,
);
console.log(
  `Arm B (browser call site) : laps=${LAPS}  finishT=${modelB.finishT}  realizedDur=${modelB.realizedDurationSec.toFixed(6)}s  raceBaseSpeed=${modelB.raceBaseSpeed.toExponential(9)}`,
);
...
for (const seed of SEEDS) {
  ...
  const ordA = orderByFinish(A.finish),
    ordB = orderByFinish(B.finish);
  const sameOrder = JSON.stringify(ordA) === JSON.stringify(ordB);
  if (!sameOrder) anyOrderMismatch = true;
  const gridSame =
    JSON.stringify(
      A.result.map((r) => [r.racerIndex, r.startRowIndex, r.indexInRow]).sort(),
    ) ===
    JSON.stringify(
      B.result.map((r) => [r.racerIndex, r.startRowIndex, r.indexInRow]).sort(),
    );
  const marginA = A.finish[ordA[1]].timeMs - A.finish[ordA[0]].timeMs;
  console.log(
    `--- seed ${seed} ---  t0 grid identical:${gridSame}  finish order identical:${sameOrder}`,
  );
  console.log(
    `  top5: ${ordA
      .slice(0, 5)
      .map((i) => NAMES[i])
      .join(", ")}   (winner margin ${marginA.toFixed(2)}s)`,
  );

  // Checkpoint stream diff (per-racer t at each 5 s of physicsTs)
  const cpsAll = [...new Set([...A.cps.keys(), ...B.cps.keys()])].sort(
    (a, b) => a - b,
  );
  let seedWorst = 0;
  let mismatchedCps = 0;
  for (const ts of cpsAll) {
    const a = A.cps.get(ts),
      b = B.cps.get(ts);
    if (!a || !b) {
      mismatchedCps++;
      continue;
    ...
    const ra = rankVectorAt(a),
      rb = rankVectorAt(b);
    if (JSON.stringify(ra) !== JSON.stringify(rb) || d > 0) {
      console.log(
        `   ts=${ts}  max|Δt|=${d.toExponential(3)}  Spearman=${spearman(ra, rb).toFixed(6)}`,
      );
      mismatchedCps++;
    }
  }
  ...
  console.log(
    `  checkpoints compared: ${cpsAll.length}   non-identical: ${mismatchedCps}   max |Δt| over all: ${seedWorst.toExponential(3)}`,
  );
  console.log("");
}
