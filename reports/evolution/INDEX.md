...
- [BOARD-PORTRAIT-FIT-1.md](BOARD-PORTRAIT-FIT-1.md) — **the beetle is SEVENTH. The board sized one
  ...
  MINTED on his acceptance — the mint and its reason live in `docs/fingerprints.json`, which is
  their one home, and no second report restates them**). ★ **THE RENDER FINGERPRINT MOVED,
  DELIBERATELY**: it samples the STARTERS board, so a visible board change must move it. **World and camera re-run and UNMOVED** — the R17 pairing. ★ **THE CAUSE.**
...
- [CEREMONY-SKIP-1.md](CEREMONY-SKIP-1.md) — **END THE CURRENT CEREMONY BEAT WITH A CLICK — a test aid, default OFF. NOT MERGED: the merge waits for the owner's eye on the running build** (served as `index-B5rAtZzc.js` on 4173; the bundle was READ to confirm the switch compiles to false). **ONE NUMBER MOVES:** the ceremony has a single clock, `st.countdownStart`, and every consumer derives its elapsed from it — so a click never names a beat, it moves that origin BACKWARDS by the remainder and every consumer follows by construction. No second clock, no counter, no per-beat state, no `switch` over beats. **`nextBeatStart` lives beside `ceremonyAt`** reading the same schedule fields — a second list of boundaries anywhere else is the defect CEREMONY-TRUTH-1 was written about — and **zero-length beats FALL OUT rather than being special-cased**: a zero-length beat makes its boundary equal to the previous one, so strictly-greater steps over it and no branch says so. The handler sits on the WRAPPER, not a canvas, because the brand card covers the canvas exactly where the first skip is wanted. **ALL FOUR FINGERPRINTS UNCHANGED**, with verify's routing as the authority (PASS 18, SKIP 5); WORLD-OFF is not in verify's set and was run by hand to complete the four. **The tracking-lag stamp was RE-STAMPED DELIBERATELY** — not re-measured, not bypassed: a ceremony beat boundary is not an input to a measurement of the TRACKING phase, which begins after the ceremony is over. **TWO HONEST GAPS RECORDED:** three of the spec's own test cases do NOT exist — switch-off, wrong-phase, and a DOM test that skips the brand beat — so the handler's guards are written but unproven; and the handler is `onMouseDown` rather than `onClick`. **A TOOL-TRUST FINDING, recorded and NOT fixed:** `engine-reach --check` reported "none of 0 paths" on the staged diff while the pre-commit hook's own invocation reported "1 of 6" correctly — its selection cannot be trusted for uncommitted work. Also kept: a fixture that silently built an EMPTY schedule because `ceremonySchedule` is POSITIONAL, caught only by the one test that walks the whole sequence. 3 proposals.

- [BACKLOG-TRUTH-1.md](BACKLOG-TRUTH-1.md) — **THE ACTION CONTROL IS RECORDED; THE BACKLOG AUDIT STOPPED EARLY AND SAYS WHERE.** **STAGE 1 COMPLETE:** the owner order of 2026-08-22 — one control over how much race action, for a NORMAL HOST, not a developer knob — is now the first live section of BACKLOG.md, with UNVERIFIED candidates by file:line, both surfaces addressed (the host-facing Race Settings panel is 86 lines carrying exactly ONE control today), and four open questions including what the dial does to the band-fairness gate. Design deliberately absent. **STAGE 2 DID NOT COMPLETE — 4 of 26 verified, 22 named by line in the report** rather than half-verified, per the spec own rule. minRacersVisible CLOSED by 46736d81 (the only BACKLOG edit beyond Stage 1); worktree stubs STILL OPEN and WORSE (51, not the 47 the item claims); deploy.yml.disabled and the missing build identifier STILL OPEN. **STAGE 3 NOT STARTED.** **THREE SPEC-SUPPLIED CLAIMS WERE NOT REPRODUCIBLE** and are recorded as unconfirmed rather than inherited — the finish-knob relabel could not be read where expected, the eye-test item could not be located, and the sprite-route correction was not re-established (and its file is outside the two paths the spec permits, so it was not edited). **One spec conflict flagged, not silently resolved:** the spec asks for shipped DEFAULTS in a document; CONFIG-TRUTH-1 forbids exactly that, so addresses were recorded instead. 3 proposals, including making verify: lines a GUARD inside check-doc-facts rather than a convention. Documents only; no fingerprint can move.

- [FAIRNESS-PROMISE-2.md](FAIRNESS-PROMISE-2.md) — **THE OWNER SETTLES IT, 2026-08-25: the NARROW reading, and the director's speed correction is INTENDED.** In his words: originally there was only the re-roll, the start bonus and the finish bonus; since the DIRECTOR exists, steering by the director is also possible — to make the drawn ranks actually reachable AND to keep the race exciting. **So L184 forbids a positional force that DRAGS a racer to a place — wall, spring, authored curve — and not a speed correction serving the drawn outcome. The code does not change.** **`docs/FAIRNESS.md` CORRECTED** to describe the race that ships: it names BOTH mechanisms — the draw bias, and the director's `trajectoryMult` **plus `areaBonusMult`, the second planner-written term the audit found** — states that no positional force is used, carries his reading dated, and **states plainly that the split between the two has never been measured** (47 reports mention `trajectoryMult`; none mentions it beside `bandBias`), with an instruction not to quote either as *the* reason for the headline. That absence is now IN the document rather than smoothed over. **`docs/LESSONS.md`** gains a dated note beside L184 recording which reading is operative and why the ambiguity mattered — **its EVIDENCE is untouched**: every arm it measured was positional and that remains the whole of what it proved. **Deliberately NOT done:** the code, the controller, a fairness sweep, or any rewriting of L184's arms. **The standing gap is named so it is not lost — it is still NOT established that the draw bias alone reaches 85–90%**, since FAIR-ARRIVAL's ARM B posted 89% with the controller already in the tree. Documents only; no fingerprint can move (R15).

- [FAIRNESS-PROMISE-1.md](FAIRNESS-PROMISE-1.md) — **DOES THE RACE CORRECT THE MOTION AFTER THE DICE? — investigation only; nothing changed, not the code, not FAIRNESS.md, not the lesson.** **THE CONTROLLER CAME FIRST BY 70 DAYS:** `trajectoryMult` entered on 2026-05-20 (`596a1b29`, Phase 3A), and `FAIRNESS.md`'s "not by any positional force" sentence was written 2026-07-29 (`e06a6af1`, DOCS-1 STAGE 4) — **the sentence was written about a race that already contained the controller.** **L184 IS INTERNALLY INCONSISTENT IN SCOPE:** its TITLE forbids correcting "the Motion After the Dice" and its INSIGHT says any post-dice correction is an opponent force — both catch a speed multiplier — while its CONSEQUENCE forbids only a post-dice **positional** force (wall, spring, authored curve), which a speed multiplier is not, and **every arm it measured was positional** (ARM C was "a literal band wall"). **The lesson PROVES a claim about positional forces and ASSERTS one about motion.** **THE 85–90% HEADLINE HAS NEVER BEEN ATTRIBUTED:** 47 reports and 9 docs mention `trajectoryMult`, `FAIRNESS.md` mentions it **0** times, and **0 reports mention both `bandBias` and `trajectoryMult`** — the split has never been measured, and that absence is the finding. **TWO TERMS ARE PLAN-DERIVED, NOT ONE:** `trajectoryMult` AND `areaBonusMult` are both written by the planner from the intended outcome; `boost`/`brake` are race state, `rowEnvMult` is start-row context (its status under the broad reading NOT ESTABLISHED). Verdict: **primarily a DOCUMENTATION defect with a real design question open underneath.** The corrected sentence is written in the report and deliberately NOT applied to `FAIRNESS.md`. 3 proposals: change the document (cheapest, makes the gap visible); MEASURE the split first (`bandStrictness` already exists as the dial); or retire the controller (moves the WORLD fingerprint and invalidates the COMBO15 baseline — and it is not established the draw bias reaches 85–90% alone, since FAIR-ARRIVAL's ARM B posted 89% with the controller already in the tree). Sheet/gate/fingerprints skipped per R15.

- [CAMERA-CURVE-1.md](CAMERA-CURVE-1.md) — **HOW THE CAMERA DRIVES THE WHOLE TRACK — investigation only, every bend, whole race.** **THE CORNER-CUTTING HYPOTHESIS IS REFUTED on his track.** Measuring the PERPENDICULAR distance from the delivered centre to the target's own PATH (not to the target point — a camera late but round scores 0, correctly), every bend on space-sprint deviates **1–8 world px, at most 1.0% of the frame**; the LAST BIG BEND ranks 6th of 10 at **1 px = 0.2%**; the longest bend (544 px) deviates 2 px. **The straights are the control and the bends beat them — worst bend 8 px against worst straight 51 px, a ratio of 0.16.** **WHAT HE IS SEEING IS THE ZOOM, NOT THE PAN:** at every one of the fastest moments the centre moves ~10 world px while the picture moves ~35 screen px. The fast moment is most likely **the run-in's opening glide — 1.37 → 6.59 corridors, a factor of 4.8, in 1250 ms, with 8454 px of offset travel** — the only SUSTAINED fast move outside the photo finish. Through every bend the delivered centre turns SLOWER than the target (0.65 vs 0.96 on the last bend), which is lag along the path, not departure from it. **NOT GENERAL:** city-circuit (closed, projection read from the harness's own field-size rule rather than assumed) has 11 of 14 bends at 1–6 px but **three at 6.5–17.5% of the frame** — cause NOT ESTABLISHED, and the deviation's sign convention was not independently validated, so it is not leaned on. **A shorter time constant would not fix it and would cost the whole race** — it cannot touch the zoom, which is what moves the picture. 3 proposals, each naming its term: measure `d ln(width)/dt` as a first-class quantity (none exists); price `runInOpenMs` against the distance it opens; give city-circuit's three bends their own pass. **(c) is EXCLUDED for him by his own measurement** — 1200 frames, zero over 20 ms — and is not offered again. New: `scripts/diag/camera-curve.mjs`. Sheet, gate and fingerprints skipped per R15: nothing in their reach touched.

- [RACE-JUDDER-1.md](RACE-JUDDER-1.md) — **WHERE THE JUMPS AND THE JUDDER COME FROM — investigation only, whole race, real browser.** **ALL THREE causes are present and they are NOT independent.** **(b) IS THE DOMINANT CAMERA-SIDE DEFECT and nothing looks for it:** one-second windows in which the picture travels **757 px and ends 4 px from where it started**, changing direction **42 times in 60 frames** — every step ~12 px, three orders of magnitude inside invariant 4's bound, because a maximum-step bound cannot see the shape of a second BY CONSTRUCTION. On space-sprint it sits in PHOTO_FINISH at 0.98; on city-circuit the same shape sits at progress 0.0000 in OVERVIEW. Reversals per second p50 0, p90 6, max 42. **(c) IS REAL AND LARGE AND IT MAGNIFIES (a):** real-clock delivery is BIMODAL — 16.7 ms or exactly two vsyncs, **28.6% of frames doubled**, runs of 228, 44.9 fps average — and **69 of the 100 worst-moving frames were doubled frames against a 33.5% base rate**. In 33 ms the racers advance twice as far, so the camera's step doubles; whether it OVER-responds beyond that is NOT ESTABLISHED. Headless pacing does not transfer to his machine — `?perfprobe=1` already exists to settle it there. **(a) IS SMALLER THAN THE RAW NUMBERS SAY:** worst on-canvas movement 53 px (space-sprint) / 127.6 px (city-circuit), p50 3 px. **INVARIANT 4 MEASURES THE WRONG QUANTITY:** `4-panstep` grades the OFFSET — the world origin's screen position, usually off canvas — not the picture. On the worst frame the offset moved **742 px while the picture moved 53**. Worst PASSING width step 0.1276 ln = **81.7 px at the frame edge**, passing with a factor of five to spare. **THE VIRTUAL CLOCK IS STRUCTURALLY BLIND TO (c)** — 16/17 ms and nothing else — which is why a real-clock arm was added. 3 proposals, each naming its term: grade the picture not the offset; add a sequence invariant; settle (c) on HIS machine before touching `rawDt`/`dtSmoothingAlpha`. New: `scripts/diag/judder-census.mjs`, and two opt-in harness flags. **The browser gate was NOT skipped** — this edits the gate's own script — and passes. No product file, no key, no fingerprint measured (R15).

- [CHECK-COST-POLICY-1.md](CHECK-COST-POLICY-1.md) — **A CHECK IS RUN ONLY IF THE CHANGE COULD HAVE ALTERED ITS ANSWER.** `VERIFY-RULES.md` R15 and five sub-rules: fingerprints unmoved ⇒ no 80-race sheet; an identical merge tree ⇒ no re-measurement on the merge commit; a docs-only change pays neither the browser gate nor the client suite; re-measuring a number in a document is its OWN block; and **every report names its skips and what determined each answer**. **THE BRIEF'S PREMISE WAS WRONG AND THE MEASUREMENT SAYS SO:** the four heavy checks already skip. Routed over the last ten merges through the guards' own declarations they spent 1710 s and skipped 3690 s — **68% of the heavy cost** — with 6 of 10 merges running none of them. The lost hour was in what `verify` does NOT control: a 4473 s sheet run confirming what four byte-identical fingerprints had already proved, two more sheet runs destroyed by being started alongside other work, and re-measurements of identical merge trees and of MEASURED stamps mid-ship. **THE BROWSER GATE REDUCED FROM 10 RACES TO 2** (space-sprint + city-circuit), 671–885 s → **200 s**, the owner's decision: those two hold both extremes of every column the sheet reports — worst single-frame step, widest frame, longest standstill — while the other eight sit strictly inside them and produced ZERO events in both 80-race sweeps on record. **PROVED IN BOTH DIRECTIONS:** clean rc=0, `--sabotage-corner` rc=1 (154 violations), `--sabotage-noline` rc=1 (4). **With the LIMIT stated rather than assumed away:** the sabotage arms drive the CROSSING check only, so the five window invariants remain a net whose red has never been demonstrated at this scope, and the eight dropped tracks' geometry now reaches the nightly sweep rather than the pre-merge gate. **ONE ROUTING REFACTOR MEASURED AND THEN NOT BUILT:** extending `isInertChange` from the world fingerprint to CAMERA / RENDER / `check-runin-frame` would have saved **0 seconds** across those ten merges — every selection was live code, not comments — so it is not built and the rule records why. No check was weakened, narrowed or made unable to fail. NOTHING MINTED; no file in any fingerprint's reach was touched.

- [RETIRE-RUNIN-LEGACY-1.md](RETIRE-RUNIN-LEGACY-1.md) — **THE CEILING-AND-HOLD RUN-IN IS RETIRED, COMPLETELY — a removal with NO behaviour change** (archive tag `archive/runin-ceiling-hold`). `runInSchedule` chose between the schedule-driven endgame the owner accepted and the old path; the old arm had **no Dev Screen control**, so it was unreachable in the shipped product AND from the UI, while costing a second implementation every later repair had to be kept consistent with. His decision, 2026-08-25. **ALL FOUR FINGERPRINTS BYTE-IDENTICAL** — that table IS the argument: a removal is a claim that nothing reached the code, and four byte-identical fingerprints over ten tracks are what settles it. Re-measured after every step of the chain, not once at the end. **THE CHAIN:** `_runInHoldCeiling`, `_runInReleaseProgress`, `_runInShouldRelease`, `_runInStartProgress`, `_runInStartTs` (write-only after the removal — dead in the way hardest to see), `_runInWindowOpen`, and the `line` re-clamp in `_setTargets`, **provably unreachable** because `ceilings.line` is Infinity on every frame with one arm. Two diagnostics went with the mechanism they measure (`runin-close-rate`, `runin-pace-table`); `line-ceiling-terms` survives minus its hold lines. **STOPPED DELIBERATELY at the `line` probe key**: also always Infinity, but four diagnostics read it and **no fingerprint could tell a correct removal from a broken one** — the same reasoning that kept the old arm alive too long, so it is written down rather than done quietly. **TESTS — 10 deleted**, each named with its case (5 RUNIN-HOLD-1, 1 window gate, 4 RUNIN-LINE-1); no requirement lost its last guard. Two fixtures RE-EXPRESSED not deleted: the corridor-cap block moves before the threshold (where its composition is live), and RUNIN-BACK-1's mirror assertion is now stated against the travel's own parameter — **stricter** than what it replaces. **SIZE: camera module 23904 → 23502 lines**; `CameraDirector.js` 5009 → 4831; test file 8398 → 8176; the endgame path 496 → 414. The justification measurements are PRESERVED in `CAMERA_DIRECTOR.md` §3b (arrival 48%→6%, widest 6.1→4.4, smoothness 78.3→13.3, 9/9 monotone, fewer racers cut) with a warning that the smoothness figure is a SMOOTHED metric of the shape Lesson 218 is about; `DEAD-ENDS.md` §Q records the retirement. **AND A DEFECT OF MY OWN, REPAIRED HERE:** both MEASURED stamp ids on master were corrupted by ENDGAME-REWRITE-1's stamp script writing its regex source into the documents; the guard never noticed because it never compares an id to anything. NOTHING MINTED.

- [ENDGAME-REWRITE-1.md](ENDGAME-REWRITE-1.md) — **THE ENDGAME, WRITTEN AS IT SHOULD HAVE BEEN WRITTEN — a clean re-implementation with NO behaviour change, proved byte for byte.** **LEAD WITH THE DESIGN PAGE**: the endgame as ONE mechanism in one page — when it opens, what sets the width, who is framed, what closes it, where it arrives, and six invariants. Its living home is `CAMERA_DIRECTOR.md` §3b, with a table mapping every heading to the method that carries it. **FIVE CHANGES.** `_finishBandNearestPoint` REMOVED (attempt A1, measured backwards, 0 call sites, 48 lines); `_lineCeiling`'s 40-line specification REATTACHED — inserting that dead helper had stranded it above a method that never runs, so the function carrying the endgame's width rule had no documentation at all; **one name for "the schedule owns the zoom"** replacing five inline copies in four spellings, which is the MECHANISM by which the endgame acquired five authors; the 385-line schedule split into a 24-line sequence plus `_scheduleEngaged` / `_scheduleFittedProgress` / `_scheduleWiden` / `_scheduleClose`. Largest function **375 -> 120**. **PROVED FOUR WAYS:** all four fingerprints BYTE-IDENTICAL (CAMERA `0434cd0385eacc7b`, RENDER `57b2eb101d806b22`, WORLD/WORLD-OFF unmoved); the twelve-item sheet **960 of 960 verdicts identical** over 80 races with the pooled line identical item for item; the browser gate 0 violations on all five invariants; `verify` 15/0. **THE FIVE FOUND FIXES WERE PINNED WITH TESTS BEFORE THE REWRITE** — four had none — and each proved by removing its fix and watching it go red. **THE FINGERPRINT CAUGHT TWO REAL LOSSES**: a dropped `return z;` whose own check was written from the same slice expression it was verifying, and a missing `raceState` parameter that eslint does not flag in this file. **FOUND, NOT TOUCHED:** `runInSchedule` has no Dev Screen control and gates a whole second mechanism unreachable in the shipped product — an owner decision the fingerprints CANNOT settle, since they only run the default config; `_lfEntryByState` is NOT vestigial (all six states differ by 3x). NOTHING MINTED — nothing moved.

- [ENDGAME-LAND-CLEAN-1.md](ENDGAME-LAND-CLEAN-1.md) — **SHIPPED: THE ACCEPTED ENDGAME, RE-IMPLEMENTED ONTO MASTER ONCE** (tag `v-ship-endgame-land`). The owner judged the served production build with both switches ON and accepted it; this block wrote that design onto master as a DECISION rather than merging eight nights of excavation. **THE EQUALITY IS PROVED FOUR WAYS:** every production file BYTE-IDENTICAL to `exp/endgame-schedule@bbc63869`, `CameraDirector.js` differing by **two COMMENT lines** (citations repointed from dropped diagnostics) — so a behavioural difference is impossible from the diff; clean-forced-OFF reproduces the branch's OFF pair exactly; branch-forced-ON reproduces the shipped pair exactly; and the twelve-item sheet agrees on **960 of 960 verdicts** across 80 races, ten tracks, both field sizes, both configs, seeds 1/2/3/9 (pooled: worst frame 0.0371 ln, widest 13.572, frames with NO band 3, winner cut 0 — identical on both). **THE SHEET'S FIGURES DIFFER ON 79 OF 80 RACES AND THAT WAS MEASURED, NOT EXPLAINED AWAY:** the SAME build run twice moves the same fields by the same amounts (0/132 verdicts, 11/11 races, garden-path 21%), so the browser instrument does not reproduce itself digit for digit — noise floor ~1 frame per race, now written down. **`contentionWatch` and `bandFloor` SHIP ON**, both with Dev Screen controls. **The `verify` blocker closed on the defaults with `check-runin-frame` UNTOUCHED** (9/9 tracks, 0 frames outside the region). Browser gate at step 0a clean: 0 violations on all five invariants, **PHOTO_FINISH owns 9/9 crossings** with the winner on canvas on every one. Six superseded diagnostics dropped, `endgame-spec.mjs` kept; dead-lever census 0 of 99. CAMERA + RENDER **MINTED**; WORLD/WORLD-OFF measured and unmoved. Both MEASURED stamps re-run with the switches ON for the first time — straggler-truth identical, tracking-lag moved on PHOTO_FINISH alone (3.54 -> 3.08 pp). Lessons 218 (the proxy law) and 219 (the fixed-seed law) written; `f0cb5179`'s false merge message corrected in three findable places.
