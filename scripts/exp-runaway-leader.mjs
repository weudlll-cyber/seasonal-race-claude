...
} from "./sim/observers/outcome-front-battle.mjs";
...
function summarize(label, type, rows) {
  ...
  const gaps = rows
    .map((r) => Number(r.leaderGapAt090Len))
    .filter((x) => isFinite(x));
  const grpSizes = rows
    .filter((r) => r.paradeFinish === 1)
    .map((r) => r.paradeGroupSize);
  const spreads = rows
    .filter((r) => r.paradeFinish === 1 && r.paradeSpeedSpread !== "")
    .map((r) => Number(r.paradeSpeedSpread))
    .filter((x) => isFinite(x));
  // Group-size histogram (2,3,4,5+) for the parade groups.
  const grpHist = { 2: 0, 3: 0, 4: 0, "5+": 0 };
  for (const g of grpSizes)
    grpHist[g >= 5 ? "5+" : g] = (grpHist[g >= 5 ? "5+" : g] || 0) + 1;
  return {
    track: label,
    type,
    N: n,
    runawayWinnerRate: r4(runaway / n),
    runawayWinners: runaway,
    paradeFinishRate: r4(parade / n),
    paradeFinishes: parade,
    gap090_mean: r4(mean(gaps)),
    gap090_p50: r4(percentile(gaps, 0.5)),
    gap090_p90: r4(percentile(gaps, 0.9)),
    gap090_max: r4(gaps.length ? Math.max(...gaps) : 0),
    ...
    paradeGrp_2: grpHist[2],
    paradeGrp_3: grpHist[3],
    paradeGrp_4: grpHist[4],
    paradeGrp_5plus: grpHist["5+"],
    paradeSpeedSpread_mean: r4(mean(spreads)),
    paradeSpeedSpread_max: r4(spreads.length ? Math.max(...spreads) : 0),
  };
}
...
const SUM_COLS = [
  ...
  "paradeGrp_4",
  "paradeGrp_5plus",
  "paradeSpeedSpread_mean",
  "paradeSpeedSpread_max",
];
...
if (argv.includes("--speed-source-diag")) {
  ...
  async function runTrackS(track) {
    ...
    const ss = JSON.parse(
      readFileSync(join(outAbs, "speed-source.json"), "utf8"),
    );
    const runawayByIdx = new Map();
    let runawayCount = 0;
    for (const r of rp.races) {
      const isR = classifyRace(r.runawayParade, D).runawayWinner;
      runawayByIdx.set(r.raceIdx, isR);
      if (isR) runawayCount++;
    }
  ...
  async function workerS() {
    while (nextS < jobsS.length) {
      const i = nextS++;
      perTrackS[i] = await runTrackS(jobsS[i]);
      doneS++;
      console.log(
        `  [${doneS}/${jobsS.length}] ${jobsS[i].id.padEnd(15)} runaway=${perTrackS[i].runawayCount}/${perTrackS[i].nRaces}`,
      );
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(JOBS, jobsS.length) }, workerS),
  );
  const allRecs = perTrackS.flatMap((p) => p.recs);

  // ── Consistency gate (runaway rates = baseline) + cross-check (product == effSpeed) ──
  const BASELINE_RUNAWAY = {
    "luger-hill": 18,
    mountainstreet: 18,
    searound: 30,
    "dirt-oval": 28,
  ...
  for (const p of perTrackS) {
    if (
      RACES === 100 &&
      BASELINE_RUNAWAY[p.track.id] != null &&
      p.runawayCount !== BASELINE_RUNAWAY[p.track.id]
    ) {
      gateFail.push(
        `${p.track.id}: runaway ${p.runawayCount}/100 (baseline ${BASELINE_RUNAWAY[p.track.id]}/100)`,
      );
    }
  }
  const totalRunaway = perTrackS.reduce((s, p) => s + p.runawayCount, 0);
  if (RACES === 100 && TRACKS.length === 4 && totalRunaway !== 94)
    gateFail.push(`OVERALL: runaway ${totalRunaway}/400 (baseline 94/400)`);
  ...
  for (const r of allRecs) {
    if (!r.finishClamp) {
      xCk++;
      if (Math.abs(r.product - r.effSpeed) > 1e-9) xBad++;
    }
  }
  if (xBad > 0)
    gateFail.push(
      `CROSS-CHECK: ${xBad}/${xCk} records where product != effSpeed (>1e-9) — decomposition incomplete`,
    );
  ...
  const toCsvS = (rows) =>
    [
      S_COLS.join(","),
      ...rows.map((r) =>
        S_COLS.map((c) => (r[c] == null ? "" : r[c])).join(","),
      ),
    ].join("\n") + "\n";
  for (const p of perTrackS)
    writeFileSync(
      join(OUT_S, `speed-source-${p.track.id}.csv`),
      toCsvS(p.recs),
    );

  if (gateFail.length) {
    const msg = [
      "# Speed-Source Diagnostic — GATE FAILED (STOPPED)",
      "",
      "A verification gate failed — NOT reported. Investigate before trusting any number.",
      "",
      ...gateFail.map((g) => `- ${g}`),
      "",
    ].join("\n");
  ...
  const pick = (cls, trackId, rankSel) =>
    allRecs.filter(
      (r) =>
        r.runaway === cls &&
        (trackId ? r.track === trackId : true) &&
        rankSel(r.rank),
    );
  const isP1 = (rk) => rk === 1;
  ...
  const decompRows = (cls) => {
    const p1 = pick(cls, null, isP1);
    const ch = pick(cls, null, isChaser);
    return VAR_FACTORS.map((f) => {
      const mp1 = med(p1.map((r) => r[f]));
      const mch = med(ch.map((r) => r[f]));
      return {
        factor: f,
        p1: mp1,
        chaser: mch,
        ratio: mch !== 0 ? mp1 / mch : 0,
        delta: mp1 - mch,
      };
    });
  };
  const decompRun = decompRows(1);
  const decompNon = decompRows(0);
  // Headline: the variable factor with the largest P1/chaser ratio among runaway races.
  const topFactor = [...decompRun]
    .filter((d) => d.chaser !== 0)
    .sort((a, b) => b.ratio - a.ratio)[0];
  const p1TrajRun = med(pick(1, null, isP1).map((r) => r.trajectoryMult));
  const p1SatShareRun = (() => {
    const p1 = pick(1, null, isP1);
    return p1.length
      ? p1.filter((r) => r.servoSaturated === 1).length / p1.length
      : 0;
  })();
  ...
  const satRows = (cls) =>
    Array.from({ length: 15 }, (_, i) => {
      const rk = i + 1;
      const rows = pick(cls, null, (x) => x === rk);
      return {
        rank: rk,
        n: rows.length,
        satShare: rows.length
          ? rows.filter((r) => r.servoSaturated === 1).length / rows.length
          : 0,
  ...
  const at090 = (cls) =>
    allRecs.filter((r) => r.runaway === cls && Math.abs(r.sample - 0.9) < 1e-9);
  const gapStruct = (cls) => {
    const rows = at090(cls);
    const byRank = Array.from({ length: 15 }, (_, i) =>
      med(rows.filter((r) => r.rank === i + 1).map((r) => r.gapAhead)),
    );
    ...
    for (const r of rows) {
      const k = `${r.track}:${r.raceIdx}`;
      if (!byRace.has(k)) byRace.set(k, {});
      byRace.get(k)[r.rank] = r.gapAhead;
    }
    const within3 = [];
    for (const [, ranks] of byRace) {
      let cum = 0,
        cnt = 0;
      for (let rk = 2; rk <= 15; rk++) {
        if (ranks[rk] == null) break;
        cum += ranks[rk];
        if (cum <= 3.0) cnt++;
      }
      within3.push(cnt);
    ...
    return {
      byRank,
      within3Median: med(within3),
      within3Mean: within3.length
        ? within3.reduce((s, x) => s + x, 0) / within3.length
        : 0,
      n: byRace.size,
    };
  };
  const gapRun = gapStruct(1);
  const gapNon = gapStruct(0);
  ...
  md.push("");
  md.push(
    `Read-only, unmodified baseline (all gap-servo config absent, shipped defaults). All 4 tracks, **N=${RACES} per track**, seed=${SEED} (same seeds as the f40a7a6 baseline). Top-15 live ranks decomposed at samples ${SPEED_SOURCE_SAMPLES.map((s) => s.toFixed(2)).join("/")}.`,
  );
  md.push(
    "Consistency gate PASSED (runaway = baseline). Cross-check PASSED: on all " +
      xCk +
      " non-finish-clamped records, effSpeed == product of the recorded factors (within 1e-9) — the decomposition is complete, no factor missed.",
  );
  md.push("");
  md.push(
    "**Speed chain (from `raceStep.js advanceRacerT`):** `effSpeed = baseSpeed · boost · brake · rowEnvMult · trajectoryMult · areaBonusMult · governorMult` (× dt), then a FINISH clamp only. `rowEnvMult` = rowBonusPost; `areaBonusMult` = areaBonusPost. baseSpeed = const · **spreadFactor** · speedBonusMult. There is **no single pre-finish speed clamp** — each factor is clamped at its own source (trajectoryMult∈[0.85,1.10]; spreadFactor∈natural band ≤" +
      (perTrackS[0] ? "" : "") +
      " ~1.081; governorMult∈pulk envelope).",
  );
  md.push("");

  md.push("## Headline");
  md.push(
    `Among runaway races, the largest P1-vs-chaser speed-delta source in [0.70, 0.95] is **${topFactor.factor}** (median P1 ${f6(topFactor.p1)} vs P2–P15 ${f6(topFactor.chaser)}, ratio ${topFactor.ratio.toFixed(4)}). P1's median trajectoryMult is **${f3(p1TrajRun)}** — ${p1TrajRun < 1.0 ? "BELOW 1.0, i.e. the servo is BRAKING the leader, not boosting it" : "at/above 1.0"}; P1 runs servo-clamped (traj ≥ maxMult) in ${pctS(p1SatShareRun)} of samples. So the escape speed is ${topFactor.factor === "spreadFactor" ? "the leader's NATURAL re-roll draw, not the servo or areaBonus" : "led by " + topFactor.factor}.`,
  );
  md.push("");
  ...
  const decompTable = (rows, label) => {
    ...
    for (const d of rows)
      lines.push(
        `| ${d.factor} | ${f6(d.p1)} | ${f6(d.chaser)} | ${d.ratio.toFixed(4)} | ${(d.delta >= 0 ? "+" : "") + d.delta.toFixed(6)} |`,
      );
    return lines;
  };
  md.push(
  ...
  md.push("|---|---|---|---|---|---|");
  for (const s of satRun)
    md.push(
      `| P${s.rank} | ${s.n} | ${pctS(s.satShare)} | ${f3(s.servoHead)} | ${f6(s.bandHead)} | ${f6(s.effSpeed)} |`,
    );
  md.push("");
  md.push(
  ...
  md.push(
    "| pos gap | " +
      Array.from({ length: 14 }, (_, i) => `P${i + 1}→P${i + 2}`).join(" | ") +
      " |",
  );
  ...
  md.push("");
  md.push(
    `Racers within 3.0L of P1 at 0.90 (of P2..P15): RUNAWAY median **${gapRun.within3Median}** (mean ${gapRun.within3Mean.toFixed(2)}, n=${gapRun.n}); non-runaway median ${gapNon.within3Median} (mean ${gapNon.within3Mean.toFixed(2)}, n=${gapNon.n}).`,
  );
  md.push("");
  ...
  console.log(
    `Cross-check PASSED (${xCk} records, 0 mismatch). Headline factor: ${topFactor.factor} (ratio ${topFactor.ratio.toFixed(4)}); P1 median traj=${f3(p1TrajRun)}.`,
  );
  console.log(
    `Within 3.0L of P1 at 0.90: RUNAWAY median ${gapRun.within3Median} vs non-runaway ${gapNon.within3Median}.`,
  );
  process.exit(0);
}
...
if (argv.includes("--formation-diag")) {
  ...
  async function runTrackF(track) {
    ...
    return races.map((rec) => {
      const rp = rec.runawayParade;
      const f = rp.formation || {};
      ...
      return {
        ...
        firstCross15: f.firstCross15,
        sustained15: f.sustained15 == null ? "" : f.sustained15 ? 1 : 0,
        firstCross30: f.firstCross30,
        sustained30: f.sustained30 == null ? "" : f.sustained30 ? 1 : 0,
        gapAt030: f.gapAt030,
        gapAt060: f.gapAt060,
        leaderStable: (() => {
          const s = formationLeaderStable(
            f.leaderIdxAtCross30,
            rp.finalRankByIndex,
          );
          return s == null ? "" : s ? 1 : 0;
        })(),
  ...
  async function workerF() {
    while (nextF < jobsF.length) {
      const i = nextF++;
      perTrackRows[i] = await runTrackF(jobsF[i]);
      doneF++;
      const run = perTrackRows[i].filter((r) => r.runaway).length;
      console.log(
        `  [${doneF}/${jobsF.length}] ${jobsF[i].id.padEnd(15)} runaway=${run}/${perTrackRows[i].length}`,
      );
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(JOBS, jobsF.length) }, workerF),
  );
  ...
  for (const t of TRACKS) {
    ...
    if (
      RACES === 100 &&
      BASELINE_RUNAWAY[t.id] != null &&
      run !== BASELINE_RUNAWAY[t.id]
    ) {
      gateFail.push(
        `${t.id}: runaway ${run}/100 (baseline ${BASELINE_RUNAWAY[t.id]}/100)`,
      );
    }
  }
  ...
  if (RACES === 100 && TRACKS.length === 4 && overallRun !== 94)
    gateFail.push(
      `OVERALL: runaway ${overallRun}/400 (baseline 94/400 = 23.5%)`,
    );
  ...
  for (const t of TRACKS)
    writeFileSync(
      join(OUT_F, `formation-${t.id}.csv`),
      toCsvF(all.filter((r) => r.track === t.id)),
    );
  writeFileSync(join(OUT_F, "formation-races.csv"), toCsvF(all));

  if (gateFail.length) {
    const msg = [
      "# Formation Diagnostic — CONSISTENCY GATE FAILED (STOPPED)",
      "",
      "The runaway rates deviate from the recorded f40a7a6 baseline — the comparison basis is broken, so the diagnostic is NOT reported. Investigate before trusting any formation number.",
      "",
      ...gateFail.map((g) => `- ${g}`),
      "",
    ].join("\n");
  ...
  const hist = (rows, field) => {
    const h = Object.fromEntries(BUCKETS.map((b) => [b, 0]));
    for (const r of rows) h[formationBucket(r[field])]++;
    return h;
  };
  ...
  md.push("");
  md.push(
    `Read-only, unmodified baseline (leash absent, shipped defaults). All 4 tracks, **N=${RACES} per track**, seed=${SEED} (same seeds as the f40a7a6 baseline). Consistency gate PASSED (runaway/parade rates = baseline). Facts only — the mechanism decision is the owner's.`,
  );
  md.push(
  ...
  md.push(
    `Runaway races: **${runawayN}/${all.length}** (${((100 * runawayN) / all.length).toFixed(1)}%). Non-runaway: ${nonRunRows.length}.`,
  );
  md.push("");

  // Headline
  const before060 = runRows.filter(
    (r) => r.firstCross30 != null && r.firstCross30 < 0.6,
  ).length;
  const in060_075 = runRows.filter(
    (r) =>
      r.firstCross30 != null && r.firstCross30 >= 0.6 && r.firstCross30 < 0.75,
  ).length;
  const at075plus = runRows.filter(
    (r) => r.firstCross30 != null && r.firstCross30 >= 0.75,
  ).length;
  md.push("## Headline");
  md.push(
    `Of the ${runawayN} runaway races, the 3.0L lead was first crossed **BEFORE 0.60 in ${before060} (${((100 * before060) / runawayN).toFixed(1)}%)**, in [0.60, 0.75) in ${in060_075} (${((100 * in060_075) / runawayN).toFixed(1)}%), and at 0.75 or later in ${at075plus} (${((100 * at075plus) / runawayN).toFixed(1)}%).`,
  );
  md.push("");
  ...
  const histTable = (field, rows) => {
    const lines = [
      "| scope | " + BUCKETS.map((b) => BUCKET_LABEL[b]).join(" | ") + " |",
      "|---|" + BUCKETS.map(() => "---").join("|") + "|",
    ];
    const overall = hist(rows, field);
    lines.push(
      "| OVERALL | " + BUCKETS.map((b) => overall[b]).join(" | ") + " |",
    );
    for (const t of TRACKS) {
      const h = hist(
        rows.filter((r) => r.track === t.id),
        field,
      );
      lines.push(`| ${t.id} | ` + BUCKETS.map((b) => h[b]).join(" | ") + " |");
    }
    return lines;
  };
  ...
  const fc30 = runRows
    .filter((r) => r.firstCross30 != null)
    .map((r) => r.firstCross30);
  const q30 = quart(fc30);
  md.push("## firstCross30 among runaway races — quartiles (progress)");
  md.push(
    `p25 = ${q30.p25.toFixed(3)}, **median = ${q30.p50.toFixed(3)}**, p75 = ${q30.p75.toFixed(3)} (n=${q30.n}).`,
  );
  md.push("");

  // gapAt060 distribution: runaway vs non-runaway
  const g60run = quart(
    runRows.filter((r) => r.gapAt060 != null).map((r) => r.gapAt060),
  );
  const g60non = quart(
    nonRunRows.filter((r) => r.gapAt060 != null).map((r) => r.gapAt060),
  );
  md.push(
  ...
  md.push("|---|---|---|---|---|");
  md.push(
    `| RUNAWAY | ${g60run.p25.toFixed(2)} | **${g60run.p50.toFixed(2)}** | ${g60run.p75.toFixed(2)} | ${g60run.n} |`,
  );
  md.push(
    `| non-runaway | ${g60non.p25.toFixed(2)} | ${g60non.p50.toFixed(2)} | ${g60non.p75.toFixed(2)} | ${g60non.n} |`,
  );
  md.push("");
  md.push(
  ...
  md.push(
    `In ${lsTrue}/${lsRows.length} (${lsRows.length ? ((100 * lsTrue) / lsRows.length).toFixed(1) : "0"}%) of runaway races, the racer leading when 3.0L was first crossed is the one that finishes rank 1 (the eventual winner already leads at gap formation).`,
  );
  md.push("");
  md.push(
  ...
  console.log(
    `Headline: 3.0L crossed BEFORE 0.60 in ${before060}/${runawayN} runaway races (${((100 * before060) / runawayN).toFixed(0)}%); gapAt060 median RUNAWAY=${g60run.p50.toFixed(2)}L vs non-runaway=${g60non.p50.toFixed(2)}L.`,
  );
  process.exit(0);
}
...
if (argv.includes("--gapreroll-confirm")) {
  ...
  const TRK = (ONLY ? [ONLY] : ALL10).map((id) => {
    const s = trackSeed(id);
    return { id, racer: s.defaultRacerTypeId, closed: !!s.closed };
  });
  const KNOWN = {
    ...
    searound: 30,
    "dirt-oval": 28,
  }; // baseline first-100-seed counts
  ...
  const bandReach = (rawData, b) => {
    const rows = (rawData || []).filter((r) => r.sollBereich === b);
    return rows.length
      ? rows.filter((r) => zoneIdxOf(r.finalRank) === b - 1).length /
          rows.length
      : null;
  };
  const ARMS = [
  ...
  async function runArmTrack(arm, track) {
    ...
    if (arm.gr)
      args.push(
        `--gapRerollThresholdLengths=${arm.G}`,
        `--gapRerollMode=${arm.mode}`,
        `--gapRerollStrength=${arm.s}`,
      );
    ...
    const fd = JSON.parse(
      readFileSync(join(outAbs, "fairness-data.json"), "utf8"),
    );
    let hm = {};
    try {
      hm = JSON.parse(readFileSync(join(outAbs, "hero-map.json"), "utf8"));
    } catch {
      /* opt */
    }
    const bySeed = {};
    ...
    for (const rec of rp.races) {
      ...
      bySeed[rec.seed] = {
        ...
        parade: c.paradeFinish,
        within3: rpr.within3P1At090,
        gap090: rpr.leaderGapP2At090Len,
        leaderIdx: rpr.leaderIdxAt090,
        finalRank: rpr.finalRankByIndex,
      };
      if (c.runawayWinner) runaway++;
      if (c.paradeFinish) parade++;
      within3All.push(rpr.within3P1At090 ?? 0);
    }
    ...
    return {
      ...
      agg: {
        ...
        runaway,
        parade,
        runawayRate: runaway / rp.races.length,
        paradeRate: parade / rp.races.length,
        within3Med: med(within3All),
        b1: bandReach(fd.rawData, 1),
        b2: bandReach(fd.rawData, 2),
        holmUnfair: hm.fairness?.startRowUnfair ?? null,
        top5Action: nat.outcomeTop5SwapsMean ?? null,
        gapWindowRolls: nat.gapWindowRolls ?? null,
        gapBiasedRolls: nat.gapBiasedRolls ?? null,
        gapLeaderDutyCycle: nat.gapLeaderDutyCycle ?? null,
      },
    };
  }
  ...
  async function runArm(arm) {
    ...
    async function w() {
      while (nx < jobs.length) {
        const i = nx++;
        const r = await runArmTrack(arm, jobs[i]);
        store[arm.name][jobs[i].id] = r;
        dn++;
        console.log(
          `  ${arm.name.padEnd(13)} [${dn}/${jobs.length}] ${jobs[i].id.padEnd(15)} runaway=${r.agg.runaway}/${r.agg.n} winRolls=${r.agg.gapWindowRolls != null ? r.agg.gapWindowRolls.toFixed(0) : "-"}`,
        );
      }
    }
  ...
  await runArm(ARMS[0]);
  const knownPresent = Object.keys(KNOWN).filter((k) =>
    TRK.some((t) => t.id === k),
  );
  if (knownPresent.length === 4 && RACES >= 100) {
    ...
    for (const k of knownPresent) {
      ...
      let sub = 0;
      for (let s = 1; s <= 100; s++) if (bs[s]?.runaway) sub++;
      if (sub !== KNOWN[k])
        fails.push(`${k}: first-100 runaway ${sub} (baseline ${KNOWN[k]})`);
    }
    if (fails.length) {
      writeFileSync(
        join(OUT_C, "SUMMARY.md"),
        `# Confirmation STOPPED — continuity gate failed after A1 (V0)\n\nV0's first-100-seed subset must reproduce the baseline EXACTLY. It did not — comparison basis broken; A2/A3 NOT run.\n\n${fails.map((f) => `- ${f}`).join("\n")}\n`,
      );
      console.error(
        "\n❌ STOP GATE 1 FAILED:\n" + fails.map((f) => "  " + f).join("\n"),
      );
    ...
    console.log(
      "  STOP gate 1 PASSED (V0 first-100-seed = baseline on all 4 known tracks).",
    );
  } else {
    console.log(
      `  STOP gate 1 SKIPPED (only ${knownPresent.length}/4 known tracks in this run).`,
    );
  }

  // ── A2: SYM-1.5-s075, then STOP GATE 2 (window rolls > 0 on all tracks) ──
  await runArm(ARMS[1]);
  const zeroWin = TRK.filter(
    (t) => (store["SYM-1.5-s075"][t.id].agg.gapWindowRolls ?? 0) <= 0,
  ).map((t) => t.id);
  if (zeroWin.length) {
    writeFileSync(
      join(OUT_C, "SUMMARY.md"),
      `# Confirmation STOPPED — window-eligible rolls still 0 (A2)\n\nThe closed-track regression: A2 must have window-eligible rolls > 0 on ALL tracks. Still 0 on: ${zeroWin.join(", ")}. A3 NOT run.\n`,
    );
    console.error(
    ...
    for (const t of TRK) {
      ...
      const rows = Object.keys(bs)
        ...
        .map((s) => ({
          ...
          track: t.id,
          seed: s,
          runaway: bs[s].runaway ? 1 : 0,
          parade: bs[s].parade ? 1 : 0,
          within3: bs[s].within3 ?? "",
          gap090: bs[s].gap090 ?? "",
          leaderIdx: bs[s].leaderIdx ?? "",
        }));
      writeFileSync(
        join(OUT_C, `races-${arm.name}-${t.id}.csv`),
        [
          RCOLS.join(","),
          ...rows.map((r) =>
            RCOLS.map((c) =>
              typeof r[c] === "number" ? +Number(r[c]).toFixed(4) : r[c],
            ).join(","),
          ),
        ].join("\n") + "\n",
      );
  ...
  const armAgg = (name) => {
    ...
    const N = ar.reduce((s, r) => s + r.n, 0),
      runaway = ar.reduce((s, r) => s + r.runaway, 0),
      parade = ar.reduce((s, r) => s + r.parade, 0);
    return {
      name,
      N,
      runawayRate: runaway / N,
      maxTrack: Math.max(...ar.map((r) => r.runawayRate)),
      paradeRate: parade / N,
      b1min: Math.min(...ar.map((r) => r.b1 ?? 0)),
      b2min: Math.min(...ar.map((r) => r.b2 ?? 0)),
      holmTracks: ar.filter((r) => (r.holmUnfair ?? 0) > 0).length,
      action: mean(ar.map((r) => r.top5Action ?? 0)),
      biased: mean(ar.map((r) => r.gapBiasedRolls ?? 0)),
      duty: mean(ar.map((r) => r.gapLeaderDutyCycle ?? 0)),
      within3Med: med(ar.map((r) => r.within3Med)),
  ...
  const v0a = AA[0];
  for (const a of AA) {
    a.actionDelta = v0a.action ? a.action - v0a.action : 0;
    a.pass =
      a.runawayRate < 0.1 &&
      a.maxTrack <= 0.15 &&
      a.paradeRate <= 0.02 &&
  ...
  const pairArm = (name) => {
    ...
    for (const t of TRK) {
      const v0bs = store.V0[t.id].bySeed,
        abs = store[name][t.id].bySeed;
      const pt = {
        conv: 0,
        tot: 0,
        w3: [],
      ...
      for (const s of Object.keys(v0bs).map(Number)) {
        ...
        if (!ar.runaway) {
          ...
          pt.resid++;
          all.resid++;
          if (v0bs[s].gap090 != null)
            (pt.residV0Gap.push(v0bs[s].gap090),
              all.residV0Gap.push(v0bs[s].gap090));
          if (ar.gap090 != null)
            (pt.residArmGap.push(ar.gap090), all.residArmGap.push(ar.gap090));
        }
      }
      perTrack[t.id] = pt;
    }
  ...
  const paired = {
    "SYM-1.5-s075": pairArm("SYM-1.5-s075"),
    "SYM-1.5-s10": pairArm("SYM-1.5-s10"),
  };
  ...
  md.push("");
  md.push(
    `Measurement only, feature frozen at master ${"`"}b1f6617${"`"}. All 10 tracks (default racer each), N=${RACES}, seed set 1–${RACES} (paired), scheduled rolls only. STOP gates PASSED: V0 first-100-seed = baseline on the 4 known tracks; window-eligible rolls > 0 on all 10 tracks in A2. Facts only — the arm decision is the owner's.`,
  );
  md.push("");
  ...
  for (const a of AA)
    md.push(
      `| ${a.name} | ${pctS(a.runawayRate)} | ${pctS(a.maxTrack)} | ${pctS(a.paradeRate)} | ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} | ${pctS(a.b1min)} | ${pctS(a.b2min)} | ${a.holmTracks}/10 | ${a.within3Med} | ${a.biased.toFixed(1)} | ${a.duty.toFixed(2)} | ${a.pass ? "✅" : "❌"} |`,
    );
  md.push("");
  ...
  for (const t of TRK)
    md.push(
      `| ${t.id} | ${t.closed ? "closed" : "open"} | ${pctS(AA[0].perTrack[t.id])} | ${pctS(AA[1].perTrack[t.id])} | ${pctS(AA[2].perTrack[t.id])} |`,
    );
  md.push("");
  ...
  for (const name of ["SYM-1.5-s075", "SYM-1.5-s10"]) {
    ...
    md.push(
      `| ${name} | ${a.tot} | ${a.conv} | ${a.tot ? pctS(a.conv / a.tot) : "-"} | ${q(a.w3, 0.25)}/${med(a.w3)}/${q(a.w3, 0.75)} | ${a.escWin}/${a.escTop3}/${a.escDrop} | ${a.resid} | ${med(a.residV0Gap).toFixed(2)}→${med(a.residArmGap).toFixed(2)} |`,
    );
  }
  md.push("");
  md.push(
  ...
  md.push("");
  // Topology section
  const grp = (arm, closed) => {
    const ar = TRK.filter((t) => t.closed === closed).map(
      (t) => store[arm][t.id].agg.runawayRate,
    );
    return ar.length ? mean(ar) : 0;
  };
  md.push(
  ...
  const newClosed = TRK.filter((t) => t.closed && !KNOWN[t.id]).map(
    (t) => t.id,
  );
  md.push(
    `New OPEN tracks (${newOpen.join(", ")}): V0 mean runaway ${pctS(mean(newOpen.map((id) => store.V0[id].agg.runawayRate)))} (known open ≈18–20%).`,
  );
  md.push(
    `New CLOSED tracks (${newClosed.join(", ")}): V0 mean runaway ${pctS(mean(newClosed.map((id) => store.V0[id].agg.runawayRate)))} (known closed ≈24–30%).`,
  );
  md.push(
    `Mechanism (SYM-1.5-s10) mean runaway: open group ${pctS(grp("SYM-1.5-s10", false))}, closed group ${pctS(grp("SYM-1.5-s10", true))} (V0: open ${pctS(grp("V0", false))}, closed ${pctS(grp("V0", true))}).`,
  );
  md.push("");
  md.push("## The three open questions (N=200 power)");
  md.push(
    `- **searound vs the 15% cap:** s075 ${pctS(AA[1].perTrack["searound"] ?? 0)}, s10 ${pctS(AA[2].perTrack["searound"] ?? 0)} (cap 15%).`,
  );
  md.push(
    `- **band-reach ≥70% on the strength arms:** s075 B1min ${pctS(AA[1].b1min)} / B2min ${pctS(AA[1].b2min)}; s10 B1min ${pctS(AA[2].b1min)} / B2min ${pctS(AA[2].b2min)} (all-track minima).`,
  );
  md.push(
    `- **s075 vs s10 action:** Δ vs V0 = ${(AA[1].actionDelta >= 0 ? "+" : "") + AA[1].actionDelta.toFixed(2)} (s075) vs ${(AA[2].actionDelta >= 0 ? "+" : "") + AA[2].actionDelta.toFixed(2)} (s10).`,
  );
  md.push("");
  md.push(
  ...
  for (const a of AA)
    console.log(
      `  ${a.name.padEnd(13)} runaway=${pctS(a.runawayRate)} maxTrack=${pctS(a.maxTrack)} actionΔ=${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} B1min=${pctS(a.b1min)} PASS=${a.pass}`,
    );
  for (const name of ["SYM-1.5-s075", "SYM-1.5-s10"]) {
    const a = paired[name].all;
    console.log(
      `  ${name} converted ${a.conv}/${a.tot} (${a.tot ? pctS(a.conv / a.tot) : "-"}); escapee win/top3/drop ${a.escWin}/${a.escTop3}/${a.escDrop}`,
    );
  }
  process.exit(0);
}
...
if (argv.includes("--release-sweep")) {
  ...
  async function runOne(arm, track) {
    ...
    if (arm.gr)
      args.push(
        `--gapRerollThresholdLengths=${GR.G}`,
        `--gapRerollMode=${GR.mode}`,
        `--gapRerollStrength=${GR.s}`,
      );
    else args.push("--gapRerollEnabled=false"); // shipped default is now ON — an OFF arm must say so
    ...
    for (const rec of rp.races) {
      ...
      bySeed[rec.seed] = {
        ...
        gap090: raw.leaderGapP2At090Len,
      };
    ...
    for (const r of fd.rawData || []) {
      ...
      if (!ranks) continue;
      driftRows.push({
        sollBereich: r.sollBereich,
        rankAtRelease: ranks[r.racerIndex] ?? null,
    ...
    return {
      ...
      agg: {
        ...
        runawayRate: runaway / rp.races.length,
        paradeRate: parade / rp.races.length,
        p1Swap: swapDenom ? swaps / swapDenom : null,
        swaps,
        swapDenom,
        leadChangeMean: mean(leadChanges),
        b1Exit: drift[1].rate,
        b1ExitN: drift[1].inside,
        b2Exit: drift[2].rate,
        b2ExitN: drift[2].inside,
        b1: bandReach(fd.rawData, 1),
        b2: bandReach(fd.rawData, 2),
        holmUnfair: hm.fairness?.startRowUnfair ?? null,
        top5Action: nat.outcomeTop5SwapsMean ?? null,
        biased: nat.gapBiasedRolls ?? 0,
        duty: nat.gapLeaderDutyCycle ?? 0,
      },
    };
  }
  ...
  async function runArm(arm) {
    ...
    async function w() {
      while (nx < jobs.length) {
        const i = nx++;
        const r = await runOne(arm, jobs[i]);
        store[arm.name][jobs[i].id] = r;
        console.log(
          `  ${arm.name.padEnd(8)} ${jobs[i].id.padEnd(15)} runaway=${r.agg.runaway}/${r.agg.n} swap=${r.agg.p1Swap != null ? (100 * r.agg.p1Swap).toFixed(0) + "%" : "-"} b1Exit=${r.agg.b1Exit != null ? (100 * r.agg.b1Exit).toFixed(0) + "%" : "-"}`,
        );
      }
    }
    await Promise.all(Array.from({ length: Math.min(JOBS, jobs.length) }, w));
  ...
  console.log(
    RACES === 100 && SEED === 1
      ? "  STOP gate PASSED (V0 = known baseline on all 4 tracks)."
  ...
  const armAgg = (name) => {
    ...
    for (const t of TRK) {
      ...
      for (const s of Object.keys(v0bs).map(Number)) {
        ...
        const a = abs[s];
        if (!a || a.swap == null) continue;
        exDenom++;
        if (a.swap) exSwap++;
        exLead.push(a.leadChanges);
      }
    }
    const w = (f) => {
      const v = ar.map(f).filter((x) => x != null);
    ...
    return {
      ...
      paradeRate: parade / N,
      p1Swap: swapDenom ? swaps / swapDenom : null,
      exSwap: exDenom ? exSwap / exDenom : null,
      exDenom,
      exLeadMean: exLead.length ? mean(exLead) : null,
      ...
      holmTracks: ar.filter((r) => (r.holmUnfair ?? 0) > 0).length,
      action: mean(ar.map((r) => r.top5Action ?? 0)),
      biased: mean(ar.map((r) => r.biased)),
      duty: mean(ar.map((r) => r.duty)),
  ...
  for (const a of ARMS)
    for (const t of TRK) {
      ...
      writeFileSync(
        join(OUT_R, `races-${a.name}-${t.id}.csv`),
        [
          RCOLS.join(","),
          ...Object.keys(bs)
            .map(Number)
  ...
  const md = [];
  md.push(
    "# Release Sweep — choreoReleaseProgress 0.97→0.93 × gap-reroll (4 known tracks, N=" +
      RACES +
      ", " +
      DUR +
      "s)",
  ...
  md.push(
    RACES === 100 && SEED === 1
      ? `✅ V0 reproduced the known baseline exactly (${TRK.filter(
          (t) => KNOWN4[t.id] != null,
        )
          .map((t) => `${t.id} ${store.V0[t.id].agg.runaway}`)
          .join(", ")}; overall ${pctS(v0a.runawayRate)}).`
  ...
  for (let i = 0; i < ARMS.length; i++) {
    const a = AA[i],
      arm = ARMS[i];
    md.push(
      `| ${a.name} | ${arm.rel ?? "0.97 (default)"} | ${arm.gr ? "ON" : "off"} | ${pctS(a.runawayRate)} | ${pctS(a.maxTrack)} | ${pctS(a.paradeRate)} | ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} | ${pctS(a.b1min)} | ${pctS(a.b2min)} | ${a.holmTracks}/${TRK.length} | ${a.pass ? "✅" : "❌"} | ${pctS(a.p1Swap)} | ${pctS(a.exSwap)} (n=${a.exDenom}) | ${a.leadChangeMean.toFixed(2)} | ${pctS(a.b1Exit)} | ${pctS(a.b2Exit)} |`,
    );
  }
  md.push("");
  md.push(
  ...
  for (const a of AA)
    md.push(
      `| ${a.name} | ` +
        TRK.map((t) => {
          const g = a.perTrack[t.id];
          return `${pctS(g.b1Exit)} / ${pctS(g.b2Exit)}`;
        }).join(" | ") +
        " |",
    );
  md.push("");
  ...
  for (const a of AA)
    md.push(
      `| ${a.name} | ` +
        TRK.map((t) => pctS(a.perTrack[t.id].runawayRate)).join(" | ") +
        " |",
    );
  md.push("");
  ...
  for (const a of AA)
    md.push(
      `| ${a.name} | ` +
        TRK.map((t) => pctS(a.perTrack[t.id].p1Swap)).join(" | ") +
        " |",
    );
  md.push("");
  md.push(
  ...
  for (const a of AA)
    console.log(
      `  ${a.name.padEnd(8)} runaway=${pctS(a.runawayRate)} PASS=${a.pass} p1Swap=${pctS(a.p1Swap)} exSwap=${pctS(a.exSwap)} B1exit=${pctS(a.b1Exit)} B2exit=${pctS(a.b2Exit)}`,
    );
  process.exit(0);
}
...
if (argv.includes("--p1-contest")) {
  ...
  async function runArm(arm) {
    ...
    async function w() {
      while (nx < jobs.length) {
        ...
        store[arm.name][jobs[i].id] = r;
        const rate =
          Object.values(r.bySeed).filter((x) => x.contest === true).length /
          r.n;
        console.log(
          `  ${arm.name.padEnd(7)} ${jobs[i].id.padEnd(15)} runaway=${r.runaway}/${r.n} p1Contest=${(100 * rate).toFixed(0)}%`,
        );
      }
    }
    await Promise.all(Array.from({ length: Math.min(JOBS, jobs.length) }, w));
  ...
  if (RACES === 100 && SEED === 1) {
    for (const t of TRK) {
      if (KNOWN4[t.id] == null) continue;
      if (store.V0[t.id].runaway !== KNOWN4[t.id])
        gateFails.push(
          `${t.id}: V0 runaway ${store.V0[t.id].runaway} (baseline ${KNOWN4[t.id]})`,
        );
    }
  }
  if (gateFails.length) {
    writeFileSync(
      join(OUT_P, "SUMMARY.md"),
      `# P1-contest baseline STOPPED — V0 reproduction gate failed\n\nV0 must reproduce the known per-track runaway baseline EXACTLY; it did not, so the arm below it is not comparable. R97-ON was NOT run.\n\n${gateFails.map((f) => `- ${f}`).join("\n")}\n`,
    );
    console.error(
      "\n❌ STOP GATE FAILED:\n" + gateFails.map((f) => "  " + f).join("\n"),
    );
  ...
  const classOf = (armName, trackId, seed) => {
    ...
    return store[armName][trackId].bySeed[seed]?.runaway
      ? "runaway"
      : "converted";
  };
    ...
    for (const id of trackIds) {
      ...
      for (const s of Object.keys(bs)
        ...
        for (const bucket of ["all", cls]) {
          ...
          if (rec.contest != null) {
            acc.contestKnown++;
            if (rec.contest) acc.contestTrue++;
          }
          if (rec.fb)
            for (const p of PRIMS) {
              const v = rec.fb[p.key];
              if (v != null) acc[p.key].push(v);
            }
        }
      }
    }
    for (const cls of Object.keys(out))
      out[cls].rate = out[cls].contestKnown
        ? out[cls].contestTrue / out[cls].contestKnown
        : null;
    return out;
  };
  ...
  const perTrack = Object.fromEntries(
    ARMS.map((a) => [
      a.name,
      Object.fromEntries(TRK.map((t) => [t.id, sample(a.name, [t.id])])),
    ]),
  );
  ...
  for (const a of ARMS)
    for (const t of TRK) {
      const bs = store[a.name][t.id].bySeed;
      writeFileSync(
        join(OUT_P, `races-${a.name}-${t.id}.csv`),
        [
          RCOLS.join(","),
          ...Object.keys(bs)
            .map(Number)
            ...
            .map((s) => {
              ...
              return [
                ...
                t.id,
                s,
                classOf(a.name, t.id, s),
                rec.runaway ? 1 : 0,
                rec.contest == null ? "" : rec.contest ? 1 : 0,
                ...PRIMS.map((p) => r4(fb[p.key])),
                fb.windowFrames ?? "",
              ].join(",");
            }),
        ].join("\n") + "\n",
  ...
  const fmtQ = (arr, dp) =>
    arr.length
      ? `${q(arr, 0.5).toFixed(dp)} [${q(arr, 0.25).toFixed(dp)}–${q(arr, 0.75).toFixed(dp)}]`
      : "–";
  const ACOLS = [
    "arm",
    "scope",
  ...
  const aggRows = [];
  for (const a of ARMS) {
    for (const [scope, smp] of [
      ["overall", overall[a.name]],
      ...TRK.map((t) => [t.id, perTrack[a.name][t.id]]),
    ]) {
      for (const cls of ["all", "normal", "runaway", "converted"]) {
        const s = smp[cls];
        aggRows.push(
  ...
  writeFileSync(
    join(OUT_P, "per-arm-track-class.csv"),
    [ACOLS.join(","), ...aggRows].join("\n") + "\n",
  );

  // ── SUMMARY.md ────────────────────────────────────────────────────────────────────────────
  const pctS = (x) => (x == null ? "–" : (100 * x).toFixed(1) + "%");
  const anyWindow = Object.values(store.V0)[0]
    ? Object.values(Object.values(store.V0)[0].bySeed)[0]?.windowStart
    : null;
  const md = [];
  md.push(
    `# P1 Contest Score — baseline (V0 vs gap-reroll winner), N=${RACES}, ${DUR}s`,
  );
  md.push("");
  md.push(
  ...
  md.push(
    `- Window W = **[${anyWindow ?? "?"}, first finish]**. The start is the LIVE \`choreoResolveB2\` config value, read at runtime — no hardcoded progress constant. The window CLOSES at the first finish: past it, racers leave the live ordering by finishing and rank 1 is inherited straight down the field (measured to 1.0, every race reports ~40 "distinct leaders").`,
  );
  md.push(
    `- Front group = live racers within **${FRONT_BATTLE_DEFAULTS.nearLen} lengths of P1, INCLUDING P1** (so \`minGroup\` ${FRONT_BATTLE_DEFAULTS.minGroup} = the leader plus two chasers). Note this differs from runaway-parade's \`within3P1At090\`, which counts only the racers BEHIND P1.`,
  );
  md.push(
    `- Lengths via the shared lap-aware path (\`arcT × govLenScale\`) — the same one every other observer uses.`,
  );
  md.push(
    `- REAL P1 ACTION iff **all four**: distinctLeaders ≥ ${FRONT_BATTLE_DEFAULTS.minDistinctLeaders}, leadChangeCount ≥ ${FRONT_BATTLE_DEFAULTS.minLeadChanges}, maxLeadHoldShare ≤ ${FRONT_BATTLE_DEFAULTS.maxLeadHoldShare}, frontContestFraction ≥ ${FRONT_BATTLE_DEFAULTS.minFrontContestFraction}. \`p1LongestMultiSec\` is reported but is NOT a criterion (a seconds quantity, its threshold would be track- and duration-dependent).`,
  );
  md.push("");
  md.push("## STOP gate");
  md.push(
    RACES === 100 && SEED === 1
      ? `✅ V0 reproduced the known runaway baseline exactly (${TRK.filter(
          (t) => KNOWN4[t.id] != null,
        )
          .map((t) => `${t.id} ${store.V0[t.id].runaway}`)
          .join(", ")}).`
      : "SKIPPED (needs --races=100 --seed=1).",
  );
  md.push("");
  ...
  for (const a of ARMS) {
    ...
    md.push(
      `- **${a.name}** — ${pctS(s.rate)} of races contain a real, sustained P1 battle; in the median race the front is contested for ${q(s.frontContestFraction, 0.5) != null ? (100 * q(s.frontContestFraction, 0.5)).toFixed(0) + "%" : "–"} of the window and ${q(s.distinctLeaders, 0.5) ?? "–"} distinct racers lead.`,
    );
  }
  const dV0 = overall.V0.all,
    dR = overall["R97-ON"].all;
  const sgn = (x, dp) =>
    x == null ? "–" : (x >= 0 ? "+" : "") + x.toFixed(dp);
  md.push("");
  md.push(
    `**Delta R97-ON − V0 (what the current winner already contributes):** p1ContestRate ${sgn(dR.rate != null && dV0.rate != null ? 100 * (dR.rate - dV0.rate) : null, 1)} pp` +
      PRIMS.map(
        (p) =>
          `, median ${p.label} ${sgn(q(dR[p.key], 0.5) != null && q(dV0[p.key], 0.5) != null ? q(dR[p.key], 0.5) - q(dV0[p.key], 0.5) : null, p.dp)}`,
      ).join("") +
      ".",
  );
  md.push("");
  ...
  md.push("");
  md.push(
    "| arm | class | races | p1ContestRate | " +
      PRIMS.map((p) => p.label).join(" | ") +
      " |",
  );
  ...
  for (const a of ARMS)
    for (const cls of ["all", "normal", "runaway", "converted"]) {
      ...
      if (!s.n) {
        md.push(
          `| ${a.name} | ${cls} | 0 | – | ${PRIMS.map(() => "–").join(" | ")} |`,
        );
        continue;
      }
      md.push(
        `| ${a.name} | ${cls} | ${s.n} | ${pctS(s.rate)} | ` +
          PRIMS.map((p) => fmtQ(s[p.key], p.dp)).join(" | ") +
          " |",
      );
    }
  md.push("");
  ...
  for (const a of ARMS)
    console.log(
      `  ${a.name.padEnd(7)} p1ContestRate=${pctS(overall[a.name].all.rate)}`,
    );
  process.exit(0);
}
...
if (argv.includes("--p1-criteria")) {
  ...
  const COND = [
    {
      key: "distinctLeaders",
      label: `distinctLeaders < ${P.minDistinctLeaders}`,
      fails: (r) => +r.distinctLeaders < P.minDistinctLeaders,
    },
    {
      key: "leadChangeCount",
      label: `leadChangeCount < ${P.minLeadChanges}`,
      fails: (r) => +r.leadChangeCount < P.minLeadChanges,
    },
    {
      key: "maxLeadHoldShare",
      label: `maxLeadHoldShare > ${P.maxLeadHoldShare}`,
      fails: (r) => +r.maxLeadHoldShare > P.maxLeadHoldShare,
    },
    {
      key: "frontContestFraction",
      label: `frontContestFraction < ${P.minFrontContestFraction}`,
      fails: (r) => +r.frontContestFraction < P.minFrontContestFraction,
    },
  ];
  const readCsv = (path) => {
  ...
  for (const arm of ARM_NAMES) {
    ...
    for (const t of TRACK_IDS) {
      ...
      try {
        ...
        /* track not in this run */
      }
    }
    if (!rows.length) continue;
    const scored = rows.map((r) =>
      COND.filter((c) => c.fails(r)).map((c) => c.key),
    );
    const pass = scored.filter((f) => f.length === 0).length;
    md.push(
      `## ${arm} — ${rows.length} races, ${pass} classified REAL P1 ACTION (${((100 * pass) / rows.length).toFixed(1)}%)`,
    );
    md.push("");
    ...
    for (const c of COND) {
      ...
      const sole = scored.filter(
        (f) => f.length === 1 && f[0] === c.key,
      ).length;
      md.push(
        `| ${c.label} | ${((100 * fail) / rows.length).toFixed(1)}% | ${sole} |`,
      );
      rowsOut.push(
        [
          arm,
  ...
  writeFileSync(
    join(DIR, "criterion-breakdown.csv"),
...
if (argv.includes("--smallg-diag")) {
  ...
  async function runOne(arm, track) {
    ...
    for (const rec of rp.races) {
      ...
      bySeed[rec.seed] = {
        ...
        parade: c.paradeFinish,
        gap090: g,
        within3: rec.runawayParade.within3P1At090,
      };
      if (c.runawayWinner) runaway++;
      if (c.paradeFinish) parade++;
    ...
    return {
      ...
      agg: {
        ...
        runawayRate: runaway / rp.races.length,
        paradeRate: parade / rp.races.length,
        gap090Mean: mean(gaps),
        gap090Med: percentile(gaps, 0.5),
        gap090N: gaps.length,
        windowRolls: nat.gapWindowRolls ?? 0,
        biasedRolls: nat.gapBiasedRolls ?? 0,
        downTilts: nat.gapDownTilts ?? 0,
        upTilts: nat.gapUpTilts ?? 0,
        smokingGun: nat.gapDownAheadGtBehind ?? 0,
        downLeader: nat.gapDownLeader ?? 0,
        downChaser: nat.gapDownChaser ?? 0,
        downPack: nat.gapDownPack ?? 0,
        downGapAheadMean: nat.gapDownGapAheadMean ?? 0,
        downGapBehindMean: nat.gapDownGapBehindMean ?? 0,
      },
    };
  }
  ...
  async function w() {
    while (nx < jobs.length) {
      ...
      dn++;
      console.log(
        `  [${dn}/${jobs.length}] ${a.name.padEnd(5)} ${t.id.padEnd(15)} runaway=${r.agg.runaway}/${r.agg.n} down=${r.agg.downTilts} up=${r.agg.upTilts} smokingGun=${r.agg.smokingGun}`,
      );
    }
  }
  await Promise.all(Array.from({ length: Math.min(JOBS, jobs.length) }, w));
  ...
  for (const t of TRK) {
    ...
    if (got !== KNOWN50[t.id])
      gateFails.push(
        `${t.id}: OFF runaway ${got} (baseline ${KNOWN50[t.id]} on seeds 1–50)`,
      );
  }
  const gateChecked =
    TRK.filter((t) => KNOWN50[t.id] != null).length > 0 &&
    RACES === 50 &&
    SEED === 1;
  ...
  for (const a of ARMS)
    for (const t of TRK) {
      ...
      const rows = Object.keys(bs)
        ...
        .sort((x, y) => x - y);
      writeFileSync(
        join(OUT_D, `races-${a.name}-${t.id}.csv`),
        [
          RCOLS.join(","),
          ...rows.map((s) =>
            [
  ...
  const md = [];
  md.push(
    "# PART B — Chase-suppression at small G (searound + mountainstreet, N=" +
      RACES +
      ", " +
      DUR +
      "s)",
  ...
  md.push("");
  md.push(
    `Arms: OFF (no gapReroll) / G15 (symmetric G=1.5 s=1.0, the confirmed candidate) / G075 (symmetric G=0.75 s=1.0, the owner's slider). Fixed baseline seeds ${SEED}–${SEED + RACES - 1}, default racer per track. Branch-fire counters are pure telemetry — no sim behavior changed.`,
  );
  md.push("");
  md.push("## STOP gate — OFF reproduces the known baselines");
  if (!gateChecked)
    md.push("SKIPPED (needs --races=50 --seed=1 on the known tracks).");
  else if (gateFails.length)
    md.push("❌ FAILED:\n" + gateFails.map((f) => `- ${f}`).join("\n"));
  else
    md.push(
      `✅ PASSED — OFF runaway counts equal the confirm-n200 V0 first-50-seed subset (${TRK.filter(
        (t) => KNOWN50[t.id] != null,
      )
        .map((t) => `${t.id} ${store.OFF[t.id].agg.runaway}`)
        .join(", ")}).`,
    );
  md.push("");
  ...
  for (const t of TRK)
    for (const a of ARMS) {
      ...
      md.push(
        `| ${t.id} | ${a.name} | ${pctS(r.runawayRate)} (${r.runaway}/${r.n}) | ${r.gap090Mean.toFixed(2)} | ${r.gap090Med.toFixed(2)} | ${r.downTilts} | ${r.upTilts} | **${r.smokingGun}** |`,
      );
    }
  md.push("");
  md.push(
  ...
  for (const t of TRK)
    for (const a of ARMS) {
      ...
      md.push(
        `| ${t.id} | ${a.name} | ${r.downTilts} | ${r.downLeader} | ${r.downChaser} | ${r.downPack} | ${r.downGapAheadMean.toFixed(2)} | ${r.downGapBehindMean.toFixed(2)} | **${share}** |`,
      );
    }
  md.push("");
  md.push("## Verdict");
  ...
  md.push(
    `Pooled: suppressed DOWN-tilts G15 = ${sgG15}, G075 = ${sgG075} (ratio ${sgG15 > 0 ? (sgG075 / sgG15).toFixed(1) + "×" : "n/a"}). Mean runawayWinnerRate OFF ${pctS(rwOFF)} / G15 ${pctS(rwG15)} / G075 ${pctS(rwG075)}.`,
  );
  md.push("");
  md.push(
    "Data: `per-arm-track.csv` (aggregates), `races-<arm>-<track>.csv` (per-seed; determinism re-run target).",
  ...
  if (gateChecked && gateFails.length) {
    console.error(
      "❌ STOP GATE FAILED:\n" + gateFails.map((f) => "  " + f).join("\n"),
    );
    process.exit(1);
  ...
  console.log(
    gateChecked
      ? "  STOP gate PASSED (OFF = baseline on both tracks)."
      : "  STOP gate SKIPPED.",
  );
...
if (argv.includes("--gapreroll-phase2b")) {
  ...
  async function runArmTrack(arm, track) {
    ...
    const races = rp.races.map((r) => ({
      runaway: classifyRace(r.runawayParade, D).runawayWinner,
      parade: classifyRace(r.runawayParade, D).paradeFinish,
      within3: r.runawayParade.within3P1At090,
    }));
    const nat = (fd.results || [])[0]?.avgNaturalness || {};
    ...
    return {
      ...
      n: races.length,
      gr: arm.gr,
      runaway: runawayRows.length,
      parade: races.filter((r) => r.parade).length,
  ...
  async function worker() {
    while (next < jobs.length) {
      const i = next++;
      rows[i] = await runArmTrack(jobs[i].a, jobs[i].t);
      done++;
      const r = rows[i];
      console.log(
        `  [${done}/${jobs.length}] ${r.arm.padEnd(14)} ${r.track.padEnd(15)} runaway=${r.runaway}/${r.n} w3run=${r.within3RunMed} winRolls=${r.gapWindowRolls != null ? r.gapWindowRolls.toFixed(0) : "-"} biased=${r.gapBiasedRolls != null ? r.gapBiasedRolls.toFixed(1) : "-"}`,
      );
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(JOBS, jobs.length) }, worker),
  );

  // ── STOP GATE 1: V0 overall runaway must reproduce 22.5% ±2 ──
  const v0Rows = rows.filter((r) => r.arm === "V0");
  const v0Rate =
    v0Rows.reduce((s, r) => s + r.runaway, 0) /
    v0Rows.reduce((s, r) => s + r.n, 0);
  // ── STOP GATE 2: window-eligible rolls per track > 0 on ALL tracks (the fix's target) ──
  const zeroWindow = rows
    .filter((r) => r.gr && (r.gapWindowRolls ?? 0) <= 0)
    .map((r) => `${r.arm}/${r.track}`);
  const stops = [];
  if (RACES === 50 && (v0Rate < 0.205 || v0Rate > 0.245))
    stops.push(
      `V0 overall runaway ${(100 * v0Rate).toFixed(1)}% outside 22.5% ±2`,
    );
  if (zeroWindow.length)
    stops.push(`window-eligible rolls still 0 on: ${zeroWindow.join(", ")}`);
  ...
  if (stops.length) {
    writeFileSync(
      join(OUT_G, "SUMMARY.md"),
      `# Gap-Reroll Phase-2b — STOP GATE FAILED\n\nNOT interpreted. Investigate before trusting results.\n\n${stops.map((s) => `- ${s}`).join("\n")}\n`,
    );
    console.error(
      "\n❌ STOP GATE FAILED:\n" + stops.map((s) => "  " + s).join("\n"),
    );
  ...
  const agg = ARMS.map((a) => {
    ...
    const N = ar.reduce((s, r) => s + r.n, 0),
      ...
      parade = ar.reduce((s, r) => s + r.parade, 0);
    const perTrack = {};
    for (const r of ar) perTrack[r.track] = r.runaway / r.n;
    return {
      name: a.name,
      N,
      runawayRate: runaway / N,
      maxTrackRunaway: Math.max(...ar.map((r) => r.runaway / r.n)),
      paradeRate: parade / N,
      perTrack,
      within3RunMed: med(ar.map((r) => r.within3RunMed)),
      within3AllMed: med(ar.map((r) => r.within3AllMed)),
      b1min: Math.min(...ar.map((r) => r.b1 ?? 0)),
      b2min: Math.min(...ar.map((r) => r.b2 ?? 0)),
      holmTracks: ar.filter((r) => (r.holmUnfair ?? 0) > 0).length,
      action: mean(ar.map((r) => r.top5Action ?? 0)),
      winRolls: mean(ar.map((r) => r.gapWindowRolls ?? 0)),
      biasedRolls: mean(ar.map((r) => r.gapBiasedRolls ?? 0)),
      dutyCycle: mean(ar.map((r) => r.gapLeaderDutyCycle ?? 0)),
    };
  });
  ...
  md.push("");
  md.push(
    `SIM-only, scheduled rolls only, window end on the roll schedule's REALIZED-duration basis (fix ${"`"}9ff3bf3${"`"}). All 4 tracks, **N=${RACES}**, seed=${SEED} (f40a7a6 seeds). Facts only — arm decision is the owner's.`,
  );
  md.push(
    `STOP gates PASSED: V0 overall ${pctS(v0Rate)} (within 22.5% ±2); window-eligible rolls > 0 on all tracks (closed tracks were 0 pre-fix).`,
  );
  md.push("");
  md.push("## Gates");
  md.push(
  ...
  for (const a of agg) {
    const pt = tks
      .map((t) => (a.perTrack[t] != null ? pctS(a.perTrack[t]) : "-"))
      .join(" / ");
    md.push(
      `| ${a.name} | ${pctS(a.runawayRate)} | ${pt} | ${pctS(a.maxTrackRunaway)} | ${a.within3RunMed} / ${a.within3AllMed} | ${pctS(a.paradeRate)} | ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} | ${pctS(a.b1min)} | ${pctS(a.b2min)} | ${a.holmTracks}/4 | ${a.winRolls.toFixed(0)} | ${a.biasedRolls.toFixed(1)} | ${a.dutyCycle.toFixed(2)} | ${a.pass ? "✅" : "❌"} |`,
    );
  }
  md.push("");
  md.push(
    `Per-track column order: ${tks.join(" / ")}. Raw: ${"`"}per-arm-track.csv${"`"}. winRolls = mean window-eligible rolls/race (STOP-gate denominator); biased = mean biased rolls/race; duty = leader duty-cycle (the "held" gauge).`,
  );
  md.push("");
  ...
  for (const a of agg)
    console.log(
      `  ${a.name.padEnd(14)} ${pctS(a.runawayRate).padStart(6)} / w3=${a.within3RunMed} / ${pctS(a.paradeRate).padStart(5)} / ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} / dc=${a.dutyCycle.toFixed(2)} / ${a.pass ? "PASS" : "fail"}`,
    );
  process.exit(0);
}
...
if (argv.includes("--gapreroll-phase2")) {
  ...
  async function worker() {
    while (next < jobs.length) {
      ...
      const r = rows[i];
      console.log(
        `  [${done}/${jobs.length}] ${r.arm.padEnd(9)} ${r.track.padEnd(15)} runaway=${r.runaway}/${r.n} w3run=${r.within3RunMed} biasedRolls=${r.gapBiasedRolls != null ? r.gapBiasedRolls.toFixed(1) : "-"}`,
      );
    }
  }
  await Promise.all(
  ...
  md.push("");
  md.push(
    `SIM-only gap-cap re-roll bias (docs/CONCEPT-COHESION.md), scheduled rolls only. All 4 tracks, **N=${RACES} per track**, seed=${SEED} (same seeds as the f40a7a6 baseline). Strength 0.5 (threshold-first). Facts only — the arm decision is the owner's.`,
  );
  md.push(
    `V0 STOP gate PASSED (V0 overall runaway ${pctS(v0Rate)} within 22.5% ±2).`,
  );
  md.push("");
  md.push("## Gates");
  md.push(
  ...
  for (const a of agg) {
    const pt = tks
      ...
      .join(" / ");
    md.push(
      `| ${a.name} | ${pctS(a.runawayRate)} | ${pt} | ${pctS(a.maxTrackRunaway)} | ${a.within3RunMed} / ${a.within3AllMed} | ${pctS(a.paradeRate)} | ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} | ${pctS(a.b1min)} | ${pctS(a.b2min)} | ${a.holmTracks}/4 | ${a.biasedRolls.toFixed(1)} | ${a.dutyCycle.toFixed(2)} | ${a.pass ? "✅" : "❌"} |`,
    );
  }
  md.push("");
  md.push(
    `Per-track column order: ${tks.join(" / ")}. Raw per-(arm×track): \`per-arm-track.csv\`.`,
  );
  md.push(
  ...
  for (const a of agg)
    console.log(
      `  ${a.name.padEnd(9)} ${pctS(a.runawayRate).padStart(6)} / w3=${a.within3RunMed} / ${pctS(a.paradeRate).padStart(5)} / ${(a.actionDelta >= 0 ? "+" : "") + a.actionDelta.toFixed(2)} / dc=${a.dutyCycle.toFixed(2)} / ${a.pass ? "PASS" : "fail"}`,
    );
  process.exit(0);
}
...
if (argv.includes("--leash-phase1")) {
  ...
  const bandReach = (rawData, bandNum) => {
    ...
    return (
      rows.filter((r) => zoneIdxOf(r.finalRank) === bandNum - 1).length /
      rows.length
    );
  };
  ...
  async function runVT(variant, track) {
    ...
    const args = [
      ...
      `--dur=${DUR}`,
      "--runaway-parade",
      "--hero-map", // hero-map → Holm; fairness-data → band-reach + action
      `--out=${toSimOut(outAbs)}`,
    ];
    if (variant.leash) {
      args.push(
        `--frontLeashMaxLengths=${variant.max}`,
        `--frontLeashGainPct=${variant.gain}`,
      );
    }
    const t0 = Date.now();
    await pExecFile(process.execPath, args, {
    ...
    const runaway = races.filter(
      (r) => classifyRace(r.runawayParade, D).runawayWinner,
    ).length;
    const parade = races.filter(
      (r) => classifyRace(r.runawayParade, D).paradeFinish,
    ).length;
    const nat = (fd.results || [])[0]?.avgNaturalness || {};
    return {
      ...
      runaway,
      parade,
      runawayRate: runaway / races.length,
      paradeRate: parade / races.length,
      b1: bandReach(fd.rawData, 1),
      b2: bandReach(fd.rawData, 2),
      holmUnfair: hm.fairness?.startRowUnfair ?? null,
      top5Action: nat.outcomeTop5SwapsMean ?? null,
  ...
  async function worker() {
    while (next < jobs.length) {
      ...
      console.log(
        `  [${done}/${jobs.length}] ${jobs[i].v.name.padEnd(9)} ${jobs[i].t.id.padEnd(15)} runaway=${(rows[i].runawayRate * 100).toFixed(0)}% parade=${(rows[i].paradeRate * 100).toFixed(0)}% leashF=${rows[i].leashFrames ?? "-"} ${rows[i]._secs.toFixed(0)}s`,
      );
    }
  }
  await Promise.all(
  ...
  const byVariant = VARIANTS.map((v) => {
    ...
    const perTrack = {};
    for (const r of vr) perTrack[r.track] = r.runawayRate;
    const maxTrackRunaway = Math.max(...vr.map((r) => r.runawayRate));
    const b1min = Math.min(...vr.map((r) => r.b1 ?? 0));
    const b2min = Math.min(...vr.map((r) => r.b2 ?? 0));
  ...
  md.push("");
  md.push(
    `Sim-only leash (SIM harness flags), all 4 tracks, **N=${RACES} per track**, seed=${SEED} (same seeds as the f40a7a6 baseline). Facts only — the variant decision is the owner's.`,
  );
  md.push(
  ...
  for (const v of byVariant) {
    ...
    const perTrackStr = tks
      .map((t) => (v.perTrack[t] != null ? pctS(v.perTrack[t]) : "-"))
      .join(" / ");
    md.push(
      `| ${v.name} | ${leashStr} | ${pctS(v.runawayRate)} | ${perTrackStr} | ${pctS(v.maxTrackRunaway)} | ${pctS(v.paradeRate)} | ${(v.action ?? 0).toFixed(2)} (${v.actionDelta >= 0 ? "+" : ""}${v.actionDelta.toFixed(2)}) | ${pctS(v.b1min)} | ${pctS(v.b2min)} | ${v.holmTracks}/4 | ${v.pass ? "✅" : "❌"} |`,
    );
  }
  md.push("");
  ...
  for (const v of byVariant)
    console.log(
      `  ${v.name.padEnd(9)} ${pctS(v.runawayRate).padStart(6)} / ${pctS(v.maxTrackRunaway).padStart(6)} / ${pctS(v.paradeRate).padStart(5)} / ${(v.actionDelta >= 0 ? "+" : "") + v.actionDelta.toFixed(2)} / ${v.pass ? "PASS" : "fail"}`,
    );
  process.exit(0);
}
...
console.log(
  `Tracks ${TRACKS.length} × Races ${RACES} (seed=${SEED}, dur=${DUR}s), jobs=${JOBS}`,
);
console.log(
  `Thresholds: leadLen=${D.leadLen}L challengeLen=${D.challengeLen}L sideBySideLen=${D.sideBySideLen}L ` +
...
async function worker() {
  while (nextIdx < jobList.length) {
    ...
    console.log(
      `  [${doneCount}/${jobList.length}] ${r.track.id.padEnd(15)} ${r.track.racer.padEnd(8)} ` +
        `runaway=${(s.runawayWinnerRate * 100).toFixed(0)}% parade=${(s.paradeFinishRate * 100).toFixed(0)}%  ${r._secs.toFixed(0)}s`,
    );
  }
}
await Promise.all(
  Array.from({ length: Math.min(JOBS, jobList.length) }, worker),
);

// ── Emit CSVs ────────────────────────────────────────────────────────────────
const allRows = trackResults.flatMap((r) => r.rows);
writeFileSync(
  join(OUT_ABS, "runaway-parade-races.csv"),
  toCsv(RACE_COLS, allRows),
);
// Per-track race CSVs (used by the determinism re-run check).
for (const r of trackResults)
  writeFileSync(
    join(OUT_ABS, `races-${r.track.id}.csv`),
    toCsv(RACE_COLS, r.rows),
  );

const summaryRows = trackResults.map((r) =>
  summarize(r.track.id, r.track.closed ? "closed" : "open", r.rows),
);
if (!ONLY) summaryRows.push(summarize("OVERALL", "all", allRows));
writeFileSync(
  join(OUT_ABS, "runaway-parade-summary.csv"),
  toCsv(SUM_COLS, summaryRows),
);
...
md.push(
  `**N=${RACES} deterministic seeded races per track** (seed=${SEED}, dur=${DUR}s), all 4 tracks, each pinned to its seeded default racer.`,
);
md.push("");
...
md.push("|---|---|---|");
md.push(
  `| windowStart | ${D.windowStart} | progress at which the runaway lead is measured; opens the [.,1.0] challenge window |`,
);
md.push(
  `| leadLen | ${D.leadLen} lengths | RUNAWAY (a): rank-1 must lead rank-2 by >= this at windowStart |`,
);
md.push(
  `| challengeLen | ${D.challengeLen} length | RUNAWAY (c): leader→P2 gap must never drop below this in [windowStart, 1.0] |`,
);
md.push(
  `| sideBySideLen | ${D.sideBySideLen} length | PARADE (a): max consecutive gap inside the leading group |`,
);
md.push(
  `| farLen | ${D.farLen} lengths | PARADE (b): min gap from the group's last member to the next racer |`,
);
md.push(
  `| speedWindow | ${D.speedWindow} | final race fraction over which the group's internal speed spread is reported |`,
);
md.push("");
...
for (const s of summaryRows) {
  md.push(
    `| ${s.track} | ${s.type} | ${s.N} | ${pct(s.runawayWinnerRate)} (${s.runawayWinners}) | ${pct(s.paradeFinishRate)} (${s.paradeFinishes}) | ` +
...
md.push("");
md.push(
  "- **runawayWinnerRate** — share of races where the leader is already >= leadLen clear at progress " +
    `${D.windowStart}, goes on to win, and is never challenged (leader→P2 stays >= challengeLen) through the finish.`,
);
md.push(
  "- **paradeFinishRate** — share of races that end with a side-by-side leading group (>= 2, internal gaps " +
    `<= sideBySideLen) detached >= farLen from the rest of the field.`,
);
md.push(
  "- **gap@0.90** distribution — the raw leader→P2 lead at windowStart across ALL races (context for how often " +
    "the runaway clause (a) is even in reach). `≥leadLen@0.90` counts races meeting clause (a) alone.",
);
md.push(
  "- **grp speed-spread** — the parade groups' internal max relative speed delta over the final " +
    `${D.speedWindow * 100}% of the race; near 0 confirms the "same speed" (paced) signature.`,
);
md.push("");
md.push(
  "Data: `runaway-parade-summary.csv` (this table), `runaway-parade-races.csv` (per-race, all tracks), " +
...
for (const s of summaryRows) {
  console.log(
    `  ${s.track.padEnd(15)} ${String(s.type).padEnd(6)} N=${s.N}  runaway=${pct(s.runawayWinnerRate)}  parade=${pct(s.paradeFinishRate)}  gap@0.90 p50=${s.gap090_p50} max=${s.gap090_max}`,
  );
}
