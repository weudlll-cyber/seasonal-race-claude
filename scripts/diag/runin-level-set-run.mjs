// RUNIN-LEVEL-SET-1 sweep driver. MEASURE ONLY. Pool sized from the machine's own cores.
//
...
const OUT = arg("out", `c:/tmp/runin-level/p${PHASE}`);
...
function runOne(c) {
  return new Promise((res) => {
    const p = spawn(
      process.execPath,
      [
        join(ROOT, "scripts", "diag", "runin-level-set.mjs"),
        `--track=${c.track}`,
        `--racers=${c.racers}`,
        `--from=${c.from}`,
        `--to=${c.to}`,
