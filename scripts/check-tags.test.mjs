...
test('check-tags FAILS and names the entry when a DECLARED tag does not exist at origin', () => {
  ...
  const r = runGuard(p['ls-remote.txt'], p['TAGS.md']);
  assert.notEqual(r.status, 0, 'guard must exit non-zero on a declared-but-absent tag');
  assert.match(r.stderr, /pre\/ghost -> declared at/, 'guard must name the phantom tag');
  assert.match(r.stderr, /do NOT exist at origin/);
  assert.doesNotMatch(r.stderr, /pre\/alpha -> declared/, 'a real tag must not be flagged');
});
...
test('a declaration under a RETIRED heading does NOT fail — collapsed tags are history, explicitly', () => {
  ...
  const r = runGuard(p['ls-remote.txt'], p['TAGS.md']);
  assert.equal(r.status, 0, `a retired declaration must not fail; stderr: ${r.stderr}`);
});
...
test('the same exclusion applies to a COLLAPSED heading, and it is the HEADING that decides', () => {
  ...
  const r = runGuard(p['ls-remote.txt'], p['TAGS.md']);
  assert.notEqual(r.status, 0, 'leaving the retired section must re-arm the check');
  assert.match(r.stderr, /pre\/second-ghost -> declared at/);
  assert.doesNotMatch(r.stderr, /pre\/ghost -> declared/, 'the collapsed one must stay excluded');
});
...
test('PROSE MENTIONS ARE NOT DECLARATIONS — a branch name in running text must not fail the build', () => {
  ...
  const r = runGuard(p['ls-remote.txt'], p['TAGS.md']);
  assert.equal(r.status, 0, `prose mentions must not fail; stderr: ${r.stderr}`);
...
test('a declaration without a backticked SHA is not a declaration — the sha is what makes it one', () => {
  ...
  const r = runGuard(p['ls-remote.txt'], p['TAGS.md']);
  assert.equal(r.status, 0, `a bare list mention must not fail; stderr: ${r.stderr}`);
});
