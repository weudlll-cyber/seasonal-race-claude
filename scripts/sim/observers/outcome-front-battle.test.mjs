...
test('a frozen tracker ignores everything afterwards, including a fresh scrap', () => {
  ...
  for (let i = 0; i < 20; i++) {
    t.observe(field([2 + (i % 3), 20 + i], [5, 19 + i]), 0.99, 2000 + i * 100, gapLen);
  }
  const r = t.result();
...
test('distinctLeaders counts every racer that held rank 1, once each', () => {
  ...
  t.observe(field([1, 14], [2, 13]), 0.90, 200, gapLen); // back to 1 — still 2 distinct
...
test('one leader throughout gives distinctLeaders 1 and maxLeadHoldShare 1', () => {
  const t = makeFrontBattleTracker({ windowStart: W });
  for (let i = 0; i < 5; i++) t.observe(field([1, 10 + i], [2, 9 + i]), 0.85, i * 100, gapLen);
  const r = t.result();
  assert.equal(r.distinctLeaders, 1);
...
test('leadChangeCount counts real overtakes, and a finish is never one of them', () => {
  ...
  t.observe(field([1, 14], [2, 13]), 0.90, 200, gapLen);          // taken back    -> 2
...
test('maxLeadHoldShare is the largest single share, not the last leader share', () => {
  ...
  t.observe(field([1, 12], [2, 11]), 0.90, 200, gapLen);
...
test('p1LongestMultiSec is the longest CONTINUOUS stretch, not the total', () => {
  const t = makeFrontBattleTracker({ windowStart: W, nearLen: 3, minGroup: 3 });
  const tight = (ms) => t.observe(field([1, 10], [2, 9], [3, 8]), 0.9, ms, gapLen);
  const loose = (ms) => t.observe(field([1, 10], [2, 9], [3, 1]), 0.9, ms, gapLen);
  tight(0); tight(1000); tight(2000);  // run A: 2s
  loose(3000);                          // break
  tight(4000); tight(5000);             // run B: 1s  (total contested time 3s — must NOT be reported)
...
test('each threshold fails on its own, one step off', () => {
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, distinctLeaders: 2 }), false);
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, leadChangeCount: 2 }), false);
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, maxLeadHoldShare: 0.71 }), false);
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, frontContestFraction: 0.49 }), false);
});

test('comfortably past every threshold still counts', () => {
  assert.equal(classifyFrontBattle({
    ...AT_THRESHOLD, distinctLeaders: 6, leadChangeCount: 11,
    maxLeadHoldShare: 0.31, frontContestFraction: 0.92,
  }), true);
});

test('p1LongestMultiSec is NOT a criterion', () => {
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, p1LongestMultiSec: 0 }), true);
});
...
test('no window frames classifies as null, never as false', () => {
  ...
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, maxLeadHoldShare: null }), null);
});

test('thresholds are overridable and the defaults are the documented ones', () => {
  assert.deepEqual(FRONT_BATTLE_DEFAULTS, {
    nearLen: 3.0, minGroup: 3, minDistinctLeaders: 3, minLeadChanges: 3,
    maxLeadHoldShare: 0.70, minFrontContestFraction: 0.50,
  });
  assert.equal(classifyFrontBattle({ ...AT_THRESHOLD, distinctLeaders: 2 }, { minDistinctLeaders: 2 }), true);
});

// ── end-to-end: tracker -> classifier ────────────────────────────────────────
test('a lonely march classifies false, a genuine scrap classifies true', () => {
  const march = makeFrontBattleTracker({ windowStart: W, nearLen: 3, minGroup: 3 });
  for (let i = 0; i < 10; i++) march.observe(field([1, 100 + i], [2, 50 + i], [3, 40 + i]), 0.9, i * 500, gapLen);
  assert.equal(classifyFrontBattle(march.result()), false);

  const scrap = makeFrontBattleTracker({ windowStart: W, nearLen: 3, minGroup: 3 });
  ...
  order.forEach((lead, i) => {
    ...
    scrap.observe(field([lead, 100 + i], ...others.map((x, k) => [x, 99.5 + i - k * 0.5])), 0.9, i * 500, gapLen);
  });
