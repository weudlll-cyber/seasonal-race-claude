...
**Open-track finish line:** On open tracks `finishT = 1.0 − runoutZone`. The last
...
(at the shipped `reRollIntervalDivisor` and `reRollLastPositionPercent`.)
...
`racePlanBonusStrengthMultiplier` is tunable in Dev Screen → Race Tuning → "Race Plan Bonus" and is persisted in `raceDynamicsConfig`.
...
`modules/racePlanner.js` `createTrajectoryController(racePlan)` returns a `.update()` that runs once per frame and writes **`r.trajectoryMult`** on every racer. It combines authored hero-choreography curves ("choreo") with a servo that steers each racer toward its assigned `targetRank` band. **Heroes are cast at the CHAOS→PULK boundary** (`pulkStart` = config `racePlanPulkStart`, narrowed at COMBO15; note the `DEFAULT_PHASE_FRACTIONS` raw fallback literal is a DIFFERENT, wider value that the shipped config overrides — a live source-vs-source gap, values in `storage/defaults.js`); the rest of the field runs at `choreoPackBandStrictness` (the PACK band-loosening). Pre-OUTCOME, non-hero targets are pinned to 1.0; OUTCOME band-steering begins where PULK ends (`corridorStart := pulkEnd`, derived from `choreoOutcomeStart`). At the finish, heroes are held to `choreoReleaseProgress` then released, and each lower band resolves at `choreoResolveB2..B5`. `r.trajectoryMult` is threaded into the t-update multiplier chain in `screens/RaceScreen/index.jsx` alongside `r.areaBonusMult` and `rowEnvMult`.
...
COMBO15 (`v-ship-combo15`, master `175a475`) promoted the FAIR-ARRIVAL candidate to the default game via two cooperating config keys, both flag-gated so OFF reproduces the pre-combo15 world byte-identically. Since then the shipped world has advanced through two avoidance engine changes (§(a.3)) to the current world (its value: [docs/fingerprints.json](fingerprints.json); its lineage: [SIM.md](SIM.md)). The two fair-arrival keys:
...
Together with the narrower `pulkStart=0.15` chaos window (which keeps the mid-race lively, PULK-SPECTACLE), COMBO15 lifts **absolute band arrival to 85–90%/track** (from 69–83%) while holding the per-row floor and frontContest. The canonical definition (two-layer fairness, arrival headline, the v2 duration-relative pulk watchdog `chaosGap ≤ ship×1.5`, and the documented residuals) is [docs/FAIRNESS.md](FAIRNESS.md); the ship record is [reports/evolution/MERGE-SHIP-1.md](../reports/evolution/MERGE-SHIP-1.md). COMBO15 fingerprint `ded0a126048e4cdb` (OFF invariant `f8f7d9c2fd3283e9`) — **both HISTORICAL, superseded 2026-07-31** by margin hysteresis and then the acceleration cap; the current values are in [docs/fingerprints.json](fingerprints.json). Config (`storage/defaults.js`): `chaosSteer` (true), `chaosSteerGain` (0.06), `bandBias` (true), `bandBiasR` (0.6), `bandBiasGain` (0.1), `racePlanPulkStart` (0.15).

### (a.3) Avoidance feel — margin hysteresis + lateral acceleration cap (shipped 2026-07-31)

Two avoidance-engine changes shipped after COMBO15, each a single flag-gated config key in `storage/defaults.js` on the §4a soft-steering / lateral integrator path; **setting each key to `0` reproduces its predecessor world byte-identically** (parity rule, valid slider position). Because they run in the avoidance code (present with gap-reroll OFF too), each moved both the ON and OFF fingerprints. Full lineage: [docs/SIM.md](SIM.md); reports RACER-FLAPPING-2 / RACER-MOTION-2 and the definitive gate HOLM-300-COMBINED in [reports/evolution/INDEX.md](../reports/evolution/INDEX.md).

- **Margin hysteresis** (`softSteeringObstacleMargin` 0.5, RACER-FLAPPING-2): the §4a incumbent obstacle keeps the steer unless a challenger's force dominates by the relative margin (30%) — per-agent, geometric, NO clock (the timer form synchronised the field and was killed, [LESSONS.md](LESSONS.md) L190). Kills the traffic left-right flap. Set to `0` → restores plain COMBO15 (`ded0a126048e4cdb`); shipped `62400c8e88cdbe59`.
- **Lateral acceleration cap** (`maxLateralAccelPerStep` 0.0005, RACER-MOTION-2): bounds the per-tick CHANGE in a racer's lateral step so an overtake dodge eases in/out instead of snapping to full swerve on the first tick. The hard-separation non-penetration safety is UNTOUCHED (owner decision). Set to `0` → restores COMBO15 + margin hysteresis (`62400c8e88cdbe59`); shipped the current world (value + OFF invariant: [docs/fingerprints.json](fingerprints.json)). Verified fairness-neutral at the definitive N=300 Holm (changes the Holm verdict on 0/4 tracks, HOLM-300-COMBINED).

### (b) PulkLeadRotation — the PULK-phase front contest

`modules/raceGovernor.js` `applyPulkLeadRotation(racers, finishT, phaseCtx, cfg)` stages the front fight during the PULK phase by writing **`r.governorMult`** — this function is the **only writer** of `governorMult`. It is **rank-blind**: a live leader-brake, a challenger-boost on the racer catching P1, an ex-leader drop-depth release, and a permanent outsider slot. It is **faded to EXACTLY 1.0 by OUTCOME** via `governorPhaseWeight(progress, pulkEndFrac, corrStartFrac)` (`raceGovernor.js` → `governorPhaseWeight`) and lives inside the **PULK realism envelope**: an outer `±pulkEnvelopeMaxEffect` clamp (±0.12) and a `pulkEnvelopeMaxStepPerFrame` slew limit (0.01), with an optional `pulkCeilingCap` that caps a boosted racer's resulting speed at the natural band max. Outside the PULK phase every `governorMult` is pinned to exactly 1.0 (`raceGovernor.js` → `governorMult`).

Config keys (values live in `storage/defaults.js`): `pulkLeaderBrake`, `pulkChallengerBoost`, `pulkFrontPool`, `pulkBoostHeadroom`, `pulkCeilingCap`, `pulkEnvelopeMaxEffect`, `pulkEnvelopeMaxStepPerFrame`, and the `pulkLeadRotation*` sub-knobs: `AttackerSlots`, `DropDepthLengths`, `OutsiderMaxReachLengths`, `DeadlockTimeoutMs`, `MinHoldMs`.
...
The `speedBrakeTMultiplier` replaces an earlier fixed absolute threshold (`speedBrakeTThreshold`) that was not calibrated to sprite size or path length. As of feat/open-track-overlap (reports 43+45), both speed-brake axes are body-based: the longitudinal zone uses `(bodyContactLength / pathLength) × speedBrakeTMultiplier` (lead-time: it fires a fixed multiple of the contact distance early); the lateral zone uses the body contact width as a same-lane filter (no multiplier — either in the same lane or not). `avoidanceDistance` is retired from the browser gate (body-based geometric gate replaces it entirely); it remains in `defaults.js` and sim scripts for backward compat. `speedBrakeYThreshold` is still read by the browser as a same-lane fallback when track width is unavailable (`raceBehavior.js`); it remains in `defaults.js` for both browser and sim use.
