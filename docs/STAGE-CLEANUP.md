...
| 2 | (this commit) | `pulkSurge*` selection + update pass + config keys + `pulkBrakeExemptStrength`, `surgeRacerIds`, `r.pulkSurgeMult*` state + t-update factor, surge telemetry, sim surge path, config validation + DevScreen "PULK Surge" SubCard, surge test cases | `72cfbdb4…` ✓ | 3031 ✓ | `computePulkBiasedTarget` + `pulkBiasGain` PROMOTED from surge-off fallback to the always-on PULK cohesion mechanism (gate `!pulkSurgeEnabled` removed → unconditional) |
