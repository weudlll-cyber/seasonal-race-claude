...
const OUT_ABS = (() => {
  const r = argVal("out", "reports/speed-candidates");
...
function verdict(a, base) {
  const notes = [];
  if (a.bandReachPooled == null || a.bandReachPooled < 0.7)
    notes.push(`band-reach ${pct(a.bandReachPooled)} < 70%`);
  if (a.holmFlaggedTracks > 2)
    notes.push(`${a.holmFlaggedTracks}/4 Holm-unfair tracks`);
  if (base && a.deadRate > base.deadRate + 0.08)
    notes.push(
      `dead finales ${pct(a.deadRate)} (+${((a.deadRate - base.deadRate) * 100).toFixed(1)}pp vs v225)`,
    );
  if (base && a.runawayRate > base.runawayRate + 0.06)
    notes.push(
      `runaway ${pct(a.runawayRate)} (+${((a.runawayRate - base.runawayRate) * 100).toFixed(1)}pp vs v225)`,
    );
  if (a.slowdownTracks > 0)
    notes.push(
      `${a.slowdownTracks} open track(s) forced into uniform slowdown`,
    );
...
console.log(
  `tracks ${TRACK_IDS.join(", ")} | N=${RACES}/arm/track => ${RACES * TRACKS.length} races/arm | racers ${RACERS_CLOSED} closed / ${RACERS_OPEN} open | canonical per-track defaults | paired seeds`,
);
...
async function worker() {
  while (true) {
    ...
    console.log(
      `  ${out[i].arm.padEnd(6)} ${out[i].track.id.padEnd(15)} band=${pct(out[i].bandReach)} dur=${out[i].durSec.toFixed(1)}s${out[i].slowdown ? " (slow)" : ""} (${out[i]._secs.toFixed(0)}s)`,
    );
  }
}
const t0 = Date.now();
...
console.log(`\n── POOLED (per arm) ──`);
console.log(
  "arm    speed  band-reach  holm   dead    front@line  runaway  parade   duo    escDepthMed  escDepthP90  durMean  VERDICT",
);
const verdicts = {};
for (const a of armAgg) {
  ...
  verdicts[a.arm] = v;
  console.log(
    `${a.arm.padEnd(6)} ${String(a.normalSpeed).padStart(5)}  ${pct(a.bandReachPooled).padStart(9)}  ${(a.holmFlaggedTracks + "/" + a.holmTracksTotal).padStart(4)}  ${pct(a.deadRate).padStart(6)} ${a.frontGroupAtLine.toFixed(2).padStart(11)} ${pct(a.runawayRate).padStart(8)} ${pct(a.paradeRate).padStart(7)} ${pct(a.duoRate).padStart(6)} ${String(r3(a.escapeDepthMed)).padStart(12)} ${String(r3(a.escapeDepthP90)).padStart(12)} ${a.durMean.toFixed(1).padStart(8)}  ${v.safe ? "VIABLE" : "CONCERNS: " + v.notes.join("; ")}`,
  );
}

console.log(`\n── PER TRACK (band-reach + derived duration) ──`);
console.log(
  "arm    track            band-reach  holmUnfair  minPHolm  durSec   paceScale",
);
for (const o of out)
  console.log(
    `${o.arm.padEnd(6)} ${o.track.id.padEnd(15)}  ${pct(o.bandReach).padStart(9)}  ${String(o.startRowUnfair).padStart(9)}  ${String(r3(o.startRowMinPHolm)).padStart(7)}  ${o.durSec.toFixed(1).padStart(6)}   ${r3(o.paceScale)}`,
  );
...
writeFileSync(
  join(OUT_ABS, "speed-arm-track.csv"),
  [
    "arm,normalSpeed,track,bandReach,startRowUnfair,startRowMinPHolm,durSec,paceScale,slowdown",
    ...out.map((o) =>
      [
        o.arm,
        ARMS.find((a) => a.label === o.arm).normalSpeed,
        o.track.id,
        r3(o.bandReach),
        o.startRowUnfair,
...
writeFileSync(
  join(OUT_ABS, "speed-summary.json"),
  JSON.stringify(
    {
      arms: ARMS,
      tracks: TRACK_IDS,
      ...
      perTrack: out.map((o) => ({
        arm: o.arm,
        track: o.track.id,
        bandReach: o.bandReach,
        startRowUnfair: o.startRowUnfair,
        durSec: o.durSec,
