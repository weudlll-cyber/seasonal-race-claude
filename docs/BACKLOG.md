...
| ✅ **L.6-VIS** | PR #44 | Track-Editor Sichtbarkeits-Verbesserung: A1 — halbtransparenter schwarzer Overlay (35% Opacity) zwischen Hintergrund und Track-Linien. A2 — Linien dicker: Width-Boundaries 1→1.5, Center-Line + Boundary-Curves 2→3. A3 — Kontrollpunkte: Radius 6→7, Füllung weiß (#fff), Rand dunkel (#222), Strichstärke 1.5→2; Inaktive Boundary-Punkte Radius 4→5 + weiß; Selektierter Ring Radius 10→12, Strich 2→2.5. `drawStaticScene` in `trackEditorDraw.js` ausgelagert (Testbarkeit). +11 Tests. |
