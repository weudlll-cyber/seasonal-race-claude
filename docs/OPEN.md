...
| ★★ **Three production-arm specs FAIL — `arrival-shape`, `comeback-precedence`, `garden-path-finishes`** | unknown — not investigated | found while MEASURING per-spec cost (BROWSER-GATE-COVERAGE-1), not while investigating; they PRE-DATE the branch that found them and sit OUTSIDE the gate's curated set (`client/package.json:54`), which is why nothing has been reporting them. No cause guessed at. ★ Must not be added to any gate while red. → [BACKLOG.md](BACKLOG.md), "Three production-arm specs fail" |
...
| ~~**No browser test runs in CI, dev or production**~~ — ★★ **CLOSED 2026-09-23 (DELIVERY-BROWSER-GATE-1), and NARROWED rather than deleted** | A browser now runs automatically: `.github/workflows/browser-gate.yml` builds the client and runs the PRODUCTION arm's curated fast set on every push to master, daily, and on demand — proven stable first (5 runs on an unchanged tree, 5 of 5 green, 82 tests, no retries) and therefore allowed to BLOCK a pushed commit. **WHAT REMAINS, stated so the closure is not read as wider than it is: the gate covers 7 specs of the full suite, not all of it**. ★★ **2026-09-25 — moving the widened set BEFORE the merge was established to be impossible as a wiring job: the pre-merge gate runs NO Playwright specs at all** (`verify.mjs:257` `GATE_GUARD = "viewer-invariants"`, spawned at `:428`; no reference to Playwright or `e2e/` anywhere in `verify.mjs`), and the curated set's only consumer is `.github/workflows/browser-gate.yml:177`, which runs on push-to-master, a daily cron and dispatch — never on a branch or a pull request. Nothing was wired. → [BROWSER-GATE-PREMERGE-1.md](../reports/evolution/BROWSER-GATE-PREMERGE-1.md); the rest stay night work by R12a. It does not run on `pull_request` or on feature branches, so a browser regression is caught at master rather than before it. | run `35903843784`, attempts 1–5 | **narrowed to: the gate is a subset** |
