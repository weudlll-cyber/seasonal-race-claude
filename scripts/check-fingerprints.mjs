...
export const GUARD = {
  ...
  covers:
    "a current fingerprint value appearing anywhere but its one home",
  blind: [
    "it does not check that the values in the record are CORRECT - it never runs an instrument",
    "a fingerprint written with different spacing or split across lines",
  ],
  dirs: [],
  files: ["docs/fingerprints.json"],
  reach: [],
  // ALWAYS-ON: a stray copy can appear in a file of any kind, so no subset of paths would be
  // safe to skip. An EMPTY diff still selects nothing, which is coherent.
  everything: true,
