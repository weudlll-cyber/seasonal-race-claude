...
import { makeRaceRng, createRacePlan, createTrajectoryController } from '../../client/src/modules/racePlanner.js';
import { computeEvenRowLayout, computeRacerLayout } from '../../client/src/modules/rowLayout.js';
import { deriveRaceDuration, normalSpeedFrom, trackDefaultLaps } from '../../client/src/modules/durationModel.js';
import { DEFAULT_RACE_DYNAMICS_CONFIG as DYN, DEFAULT_RACE_BEHAVIOR_CONFIG as BEH, DEFAULT_BASE_SPEED_CONFIG } from '../../client/src/modules/storage/defaults.js';
...
const model = deriveRaceDuration({ isOpen, pathLengthPx, laps: LAPS, normalSpeedPxPerSec: V, speedMultiplier: spd, runoutZone: BEH.runoutZone });
const totalRows = computeRacerLayout(geometricTrackWidth * BEH.startSpreadRange, N, displaySize, DEFAULT_AUTO_SCALE_CONFIG).rowCount;

const planConfig = () => ({
  bonusStrengthMultiplier: DYN.racePlanBonusStrengthMultiplier, phaseSplitBonusEnabled: DYN.phaseSplitBonusEnabled,
...
for (const seed of [1, 7, 42]) {
  ...
  const planRacers = rowLayout.assignments.map((a)=>({index:a.racerIndex,startRowIndex:a.rowIndex})).sort((x,y)=>x.index-y.index);
  const plan = createRacePlan(planRacers, model.finishT, model.realizedDurationSec*1000, planConfig(), seed);
  const result = runSingleRace({
    shape, pathLengthPx, geometricTrackWidth, isOpen, speedMultiplier: spd,
    displaySize, bodyFillX, bodyFillY, laps: LAPS, normalSpeedPxPerSec: V,
    seed, nRacers: N, raceRng, rowLayout, behaviorConfigOverrides: { isOpen },
    racePlanController: createTrajectoryController(plan), racerTargetRankMap: plan._racerTargetRank,
  });
  const rows = [...result].sort((a,b)=>a.finalRank-b.finalRank);
  console.log(`── seed ${seed} ──`);
  const t0 = rows[0].finishTime;
  console.log(rows.slice(0,10).map((r,i)=>`${String(i+1).padStart(2)}. ${NAMES[r.racerIndex].padEnd(9)} ${(r.finishTime-t0>=0?'+':'')}${(r.finishTime-t0).toFixed(2)}s`).join('\n'));
  console.log(`   ... full order: ${rows.map(r=>NAMES[r.racerIndex]).join(', ')}`);
  console.log(`   winner margin: ${(rows[1].finishTime-rows[0].finishTime).toFixed(3)}s\n`);
}
