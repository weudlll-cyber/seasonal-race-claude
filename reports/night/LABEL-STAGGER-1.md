...
> **RECORDED NEGATIVE RESULT — THE MECHANISM IS NOT SHIPPED AND NEVER WAS.** Landed on master as
> history by CLEANUP-BEFORE-NUMBERS-1, after the owner chose the race-number design, which takes
> names off racers and removes the problem this block was solving. Nothing described below runs in
> the game.
>
> **What is worth reading:** the measurement that a stagger CREATES as many overlaps as it removes,
> and why — start rows are not monotonic in screen y, so shifting a whole row walks the collision
> into the next row. Four variants, none reaching zero. That is a durable fact about the geometry,
> not about one mechanism.
>
> The overlap TRIGGER this block produced was dropped in the same cleanup: with 2-3 character race
> numbers the condition it detects is effectively unreachable, and it existed only to gate
> mechanisms that are now all gone. Its reasoning survives here.
