...
| ✅ **D7c-fix** | #39 | Bug: `trackWidth`-Metadata (140 px, kalibriert für 1280px-Welt) gab `racersPerRow=1` auf großen Welten (6000px) → alle 20 Racer in Einzelreihen → einzelne vertikale Linie. Fix: `EditorShape.getActualTrackWidth()` misst die echte geometrische Breite (Median aus 20 Stichproben, gecached). `computeRacersPerRow(geometricTrackWidthPx, bsX, minTargetScreenPx)` ersetzt `trackWidthPx/pixelsPerRacer`. `pixelsPerRacer` aus Config, UI und Defaults entfernt (Migration: bestehende Werte werden ignoriert). `computeRowLayout` + `computeMaxRacersDefault` auf neue Signaturen umgestellt. TrackManager und SetupScreen nutzen EditorShape für Hints. |
