...
const OUT_ABS = (() => {
  const r = argVal("out", "reports/parity/gs-screen-data");
...
async function runTrack(track, { G, s }) {
  ...
  try {
    ssRate = servoSaturatedRate(
      JSON.parse(readFileSync(join(outAbs, "speed-source.json"), "utf8")),
    );
  } catch {
    /* optional */
  }
  ...
  rp.races.forEach((rec, i) => {
    rows[i].escapeDepthLen =
      (elByIdx.get(rec.raceIdx) ?? {}).escapeDepthLen ?? null;
  });
  return {
    track,
    G,
...
function aggregateArm(runs) {
  ...
  return {
    ...
    perTrack: runs.map((r) => ({
      ...
      holm: r.startRowUnfair,
      holmP: r.startRowMinPHolm,
      secs: r._secs,
    })),
  };
}
...
async function runAxis(arms) {
  ...
  for (const arm of arms)
    for (const t of TRACKS) tasks.push(() => runTrack(t, arm));
...
function printArmTable(title, results) {
  console.log(`\n${title}`);
  console.log(
    `arm                 bandReach  Holm  dead   front@line  runaway  escDepMed  escDepP90  servoSat`,
  );
  for (const { arm, agg } of results) {
    console.log(
      `${arm.label.padEnd(18)} ${pct(agg.bandReachPooled).padStart(9)}  ${String(agg.holmFlaggedTracks + "/" + agg.holmTracksTotal).padStart(4)}  ${pct(agg.deadRate).padStart(5)} ${agg.frontGroupAtLine.toFixed(2).padStart(11)} ${pct(agg.runawayRate).padStart(8)} ${String(r3(agg.escapeDepthMed)).padStart(10)} ${String(r3(agg.escapeDepthP90)).padStart(10)} ${pct(agg.servoSatPooled).padStart(9)}`,
    );
  }
}
...
console.log(
  `tracks: ${OPEN_ID} (open, Holm-flagged) + ${CLOSED_ID} (closed) | N=${RACES}/arm/track | paired seeds | racers ${RACERS_CLOSED}/${RACERS_OPEN}`,
);
console.log(
  `shipped knobs: G=0.75 (gapRerollThresholdLengths), s=0.5 (gapRerollStrength)`,
);
...
printArmTable(
  "── AXIS A: strength at G=0.75 (paired seeds) ──",
  strengthResults,
);
...
const cleanStrength = strengthResults.filter(
  ({ arm, agg }) => arm.s === 0.5 || guardrailDamage(agg).length === 0,
);
const bestStrength = cleanStrength.reduce(
  (b, x) => (x.agg.bandReachPooled > b.agg.bandReachPooled ? x : b),
  strengthResults[0],
);
console.log(
...
const beatsShip =
  bestStrength.agg.bandReachPooled - shipReach > 0.015 &&
  bestStrength.arm.s !== 0.5;
const stillShort = (bestStrength.agg.bandReachPooled ?? 0) < 0.7;
const openGaxis = stillShort || !beatsShip;
console.log(
  `decision: strength axis ${beatsShip ? "FOUND a clean winner" : "inconclusive"}; band-reach ${stillShort ? "STILL SHORT of 70%" : "clears 70%"} → ${openGaxis ? "OPEN the G axis" : "STOP (strength axis decisive)"}`,
);
...
if (openGaxis) {
  ...
  const G_ARMS = [
    { G: 0.5, s: bestS, label: `G0.5 s${bestS}` },
    {
      G: 0.75,
      s: bestS,
      label: `G0.75 s${bestS}${bestS === 0.5 ? " (SHIP)" : ""}`,
    },
    { G: 1.0, s: bestS, label: `G1.0 s${bestS}` },
  ];
  gResults = await runAxis(G_ARMS);
  printArmTable(
    `── AXIS B: G axis at best s=${bestS} (paired seeds) ──`,
    gResults,
  );
}
...
const seen = new Set();
const uniq = allArms.filter(({ arm }) => {
  const k = `${arm.G}_${arm.s}`;
...
console.log(
  `arm                 bandReach  Δreach   dead   front@line  runaway  servoSat   guardrail`,
);
const ranked = [...uniq].sort(
  (a, b) => (b.agg.bandReachPooled ?? 0) - (a.agg.bandReachPooled ?? 0),
);
for (const { arm, agg } of ranked) {
  const dmg =
    arm.s === 0.5 && arm.G === 0.75 ? ["—(ship)"] : guardrailDamage(agg);
  const dR = (agg.bandReachPooled - shipReach) * 100;
  console.log(
    `${arm.label.padEnd(18)} ${pct(agg.bandReachPooled).padStart(9)}  ${(dR >= 0 ? "+" : "") + dR.toFixed(1) + "pp"}`.padEnd(
      42,
    ) +
      ` ${pct(agg.deadRate).padStart(5)} ${agg.frontGroupAtLine.toFixed(2).padStart(11)} ${pct(agg.runawayRate).padStart(8)} ${pct(agg.servoSatPooled).padStart(9)}   ${dmg.length ? dmg.join(", ") : "clean"}`,
  );
}
...
const challengers = uniq
  ...
  .filter(
    ({ agg }) =>
      agg.bandReachPooled - shipReach > 0.015 &&
      guardrailDamage(agg).length === 0,
  )
...
if (winner) {
  console.log(
    `CHANGE → G=${winner.arm.G}, s=${winner.arm.s}: band-reach ${pct(winner.agg.bandReachPooled)} (+${((winner.agg.bandReachPooled - shipReach) * 100).toFixed(1)}pp vs ship ${pct(shipReach)}), no guardrail damage.`,
  );
} else {
  console.log(
    `SHIPPED (G=0.75, s=0.5) IS OPTIMAL: no arm beats it on band-reach by >1.5pp without deadening the finale. Ship band-reach ${pct(shipReach)}.`,
  );
}

writeFileSync(
  join(OUT_ABS, "gs-screen-150.json"),
  JSON.stringify(
    {
      normalSpeed: NORMAL_SPEED,
      races: RACES,
      ...
      gAxis: gResults,
      ranked: ranked.map(({ arm, agg }) => ({ arm, agg })),
      recommendation: winner
        ? {
            change: true,
            G: winner.arm.G,
            s: winner.arm.s,
            bandReach: winner.agg.bandReachPooled,
          }
        : { change: false },
    },
    null,
