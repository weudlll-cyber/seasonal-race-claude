...
// Path:        client/src/racer-types/SpriteRacerType.test.js
// Project:     RaceArena
// Created:     2026-04-26
...
describe('SpriteRacerType — _drawBody', () => {
  ...
  it('calls tintSprite (multiply) when base sprite is loaded but variants cache is cold', () => {
    ...
    expect(tintSprite).toHaveBeenCalledWith(mockBase, '#ff0000', 'multiply', 'solid');
  ...
  it('passes tintMode "screen" to tintSprite in lazy path', () => {
    ...
    const ctx = makeCtx();
    rt._drawBody(ctx, { ...MOCK_RACER, coatId: 'red' }, 0);
    expect(tintSprite).toHaveBeenCalledWith(mockBase, '#ff0000', 'screen', 'solid');
  });
