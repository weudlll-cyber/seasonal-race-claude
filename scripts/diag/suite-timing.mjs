// GATE-SERIAL-BCRYPT-1 — wall clock and worst-per-test margin for the server suite, under a named
...
for (let i = 1; i <= RUNS; i++) {
  ...
  const r = spawnSync(
    process.execPath,
    [join(SERVER, "node_modules", "vitest", "vitest.mjs"), "run", ...EXTRA, "--reporter=json", `--outputFile=${jsonPath}`],
    { cwd: SERVER, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
  );
  if (r.status === null)
    process.stdout.write(`  run ${i}: SPAWN FAILED — ${r.error?.message ?? "no exit code"}
`);
  ...
  runs.push({ i, wallMs, exit: r.status, tests: tests.length, failed, worst, all: tests });
...
process.stdout.write(`  exit codes         ${runs.map((r) => r.exit).join(", ")}\n`);
process.stdout.write(`  failed tests       ${runs.reduce((s, r) => s + r.failed, 0)}\n`);
if (worstAll)
  process.stdout.write(
