...
export const SPEED_SOURCE_SAMPLES = [0.7, 0.75, 0.8, 0.85, 0.9, 0.95];
...
export const FORMATION_DEFAULTS = {
  ...
  windowEnd: 0.9, // the sustained-window end (matches the runaway-winner measurement point)
  sample1: 0.3, // boundary sample A (mid-chaos)
  sample2: 0.6, // boundary sample B (PULK→OUTCOME handoff)
};
...
export function makeFormationTracker(D = FORMATION_DEFAULTS) {
  let firstCross15 = null,
    sustained15 = null;
  let firstCross30 = null,
    sustained30 = null;
  ...
  return {
    observe(gap, progress, leaderIdx) {
      if (!done030 && progress >= D.sample1) {
        gapAt030 = gap;
        done030 = true;
      }
      if (!done060 && progress >= D.sample2) {
        gapAt060 = gap;
        done060 = true;
      }
      if (firstCross15 === null) {
        if (gap >= D.t15) {
          firstCross15 = progress;
          sustained15 = true;
        }
      ...
      if (firstCross30 === null) {
        if (gap >= D.t30) {
          firstCross30 = progress;
          sustained30 = true;
          leaderIdxAtCross30 = leaderIdx;
        }
...
export function formationBucket(firstCross) {
  ...
  if (firstCross < 0.3) return "lt030";
  if (firstCross < 0.6) return "030to060";
  if (firstCross < 0.75) return "060to075";
  if (firstCross < 0.9) return "075to090";
...
export function classifyRace(raw, D = RUNAWAY_PARADE_DEFAULTS) {
  ...
  // ── RUNAWAY_WINNER ──
  const winnerIsLeaderAt090 =
    winnerIdx != null && raw.leaderIdxAt090 === winnerIdx;
  const runawayWinner =
...
export function leadingGroup(line, D = RUNAWAY_PARADE_DEFAULTS) {
  ...
  return {
    size: isParade ? size : size >= 2 ? size : 0,
    members,
    nextGapLen,
    isParade,
  };
