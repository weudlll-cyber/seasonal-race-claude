...
| ✅ **D7b-avoidance-fix** | TBD | Root-Cause-Fix für Racer-Overlap (Hypothesis B+A). Algebraische Diagnose (avoidance-diagnose.md): Gleichgewichtslücke 3.4 px ↔ Sprite ~24 px durch (1) `1e-6`-Skip blockierte Avoidance bei gleicher physicalY, (2) asymmetrische Avoidance — Leader konvergierte zur Centerline (Deadlock). Fix: `lateralForce` 0.01→0.04, `symmetricAvoidance=true`, Epsilon-Tie-Breaking ersetzt 1e-6-Skip. Exponiert 4 hard-coded Konstanten als tunable Parameter: `minLateralEpsilon`, `crowdNormalizationExponent`, `draftingMaxTargets`, `avoidanceStrictness`. Start-Phase-Detection via dynamischem Field-Spread: `avoidanceFactor` / `homeForceFactor` skaliert während Startphase. Block 10 "Avoidance Advanced" im Dev-Screen (7 neue Slider). 16 neue Unit-Tests (47 total). Commit `78efb81`. |
