...
const BUNCHED_FINISH = [10.0, 10.05, 10.1, 10.15, 10.2];
...
test("RANK-space metrics are IDENTICAL between bunched and strung-out (blind to the gap)", () => {
  ...
  const frontCount = (race) =>
    race.filter((r) => reachedFront(r.finalRank)).length;
...
test("GAP-space metrics DIFFER between bunched and strung-out (they see the dead race)", () => {
  ...
  assert.ok(
    gS.leaderGapToP2 > gB.leaderGapToP2,
    "strung leader→P2 must be larger",
  );
  assert.ok(
    gS.top5Spread > gB.top5Spread,
    "strung top-5 spread must be larger",
  );
  ...
  const sbB = secondsBehindLeader(
    0.5,
    leaderTrace(BUNCHED_FINISH),
    BUNCHED_FINISH[4] * 1000,
  );
  const sbS = secondsBehindLeader(
    0.5,
    leaderTrace(STRUNG_FINISH),
    STRUNG_FINISH[4] * 1000,
  );
...
test("visibleComeback: a rank<=5 finish is NOT automatically a comeback", () => {
  ...
  assert.equal(
    visibleComeback(/*maxBehind*/ 8.0, /*finalBehind*/ 8.0, Y, Z),
    false,
  );
...
test("deadRaceFlag fires on the strung race and not the bunched race", () => {
  const { deadRaceGapSec: thr, deadRaceMajorityFrac: maj } =
    PROPOSED_THRESHOLDS; // 3.0, 0.5
...
const lenBehind = (posArr) =>
  posArr.map((t) => lengthsBehindLeader(t, LEADER_T, false, LEN_SCALE));
...
test("LENGTHS: bunched vs strung-out differ in racer lengths (identical ranks stay blind)", () => {
  ...
  assert.ok(
    Math.max(...ls) > Math.max(...lb),
    "strung field is more lengths behind",
  );
  assert.ok(
    fieldSpreadP10P90(ls) > fieldSpreadP10P90(lb),
    "strung field p10–p90 spread is larger (lengths)",
  );
...
test("LENGTHS: a rank<=5 finish is NOT automatically a comeback (lengths thresholds)", () => {
  ...
  assert.equal(
    visibleComeback(/*maxBehindLen*/ 8.0, /*finalBehindLen*/ 8.0, Y, Z),
    false,
  ); // 8 lengths back at the line
...
test("LENGTHS: deadRaceFlag / front-gap fires on the strung race, not the bunched (lengths)", () => {
  const { deadRaceGapLen: thr, deadRaceMajorityFrac: maj } =
    PROPOSED_THRESHOLDS; // 5, 0.5
