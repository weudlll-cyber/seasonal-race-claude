...
//
// BEHAVIOURAL IDENTITY NOTE. This driver needs read-only instrumentation (--escape-latency, the
// episode observer, lateDistinctLeaders). It was authored on the analysis branch and PORTED to master
// with that instrumentation in the greenfield wrap (2026-07-23), so it now runs against master
// directly. The instrumentation is fingerprint-verified NOT to alter behaviour — both worlds were
// re-proven byte-identical on master after the port (ON e93ffa70dad562a1, OFF 72c3360fb75225ef) —
// so the race outcomes measured here are master's race outcomes; only the observability differs.
//
// NOTE ON THE NUMBERS THIS PRODUCED. The committed gate results under reports/greenfield/gate-retune/
// were measured on the branch, where the defaults were still G=1.5 / strength=1.0 and the arms were
// passed as flags. Both arms are flag-driven, so the run is reproducible here unchanged; but master's
// DEFAULTS are now the candidate (G=0.75 / s=0.5), so a flagless run is no longer the "current" arm.
//
...
const DURATIONS = argVal("durations", "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map(Number);
...
const OUT_ABS = (() => {
  const r = argVal("out", "reports/greenfield/gate-retune");
...
const RACERS_CLOSED = 40,
  RACERS_OPEN = 60; // owner-standard field sizes
...
async function runArmTrack(arm, track, races, durationSec, tag) {
  ...
  const outAbs = join(
    TMP_ABS,
    `${tag}-${arm.label}-${track.id}-${durationSec}`,
  );
  const args = [
    ...
    // No --carouselEnabled here: the mechanism was deleted in the dead-mechanisms cleanup, and the
    // flag no longer exists. The branch runs that produced the committed numbers passed it as
    // =false, i.e. exactly the behaviour master has unconditionally now — so the arms are unchanged.
...
function aggregate(runs, allRowsForThresholds) {
  ...
  const bandReachPooled = wSum
    ...
    : null;
  const TIGHT = pctl(
    allRowsForThresholds.map((r) => r.gapP1P2).filter((x) => x != null),
    25,
  );
  const FAR = pctl(
    allRowsForThresholds.map((r) => r.gapP2P3).filter((x) => x != null),
    75,
  );
  ...
  return {
    ...
    tiltSaturatedRate: fracs.length
      ? fracs.filter((f) => f >= 0.999).length / fracs.length
      : null,
    ...
    _rawCounts: {
      ...
      runaway: rows.filter((r) => r.runaway).length,
      parade: rows.filter((r) => r.parade).length,
      duo,
    },
  };
}
...
console.log(
  `arms: ${ARMS.map((a) => `${a.label}(G=${a.G},s=${a.S})`).join("  vs  ")}`,
);
console.log(
  `tracks ${TRACK_IDS.join(", ")} | N=${RACES}/arm/track => ${RACES * TRACKS.length} races/arm | racers ${RACERS_CLOSED} closed / ${RACERS_OPEN} open | dur ${DUR}s | paired seeds`,
);
...
async function worker() {
  while (true) {
    ...
    console.log(
      `  ${out[i].arm.padEnd(20)} ${out[i].track.id.padEnd(15)} band=${pct(out[i].bandReach)} holmUnfair=${out[i].startRowUnfair} (${out[i]._secs.toFixed(0)}s)`,
    );
  }
}
const t0 = Date.now();
...
const byArm = new Map();
for (const o of out) {
  if (!byArm.has(o.arm)) byArm.set(o.arm, []);
  byArm.get(o.arm).push(o);
}
const armAgg = ARMS.map((a) => ({
  arm: a.label,
  G: a.G,
  S: a.S,
  ...aggregate(byArm.get(a.label) ?? [], allRows),
}));
...
for (const o of out)
  console.log(
    `  ${o.arm.padEnd(20)} ${o.track.id.padEnd(15)} band=${pct(o.bandReach)} holmUnfair=${o.startRowUnfair} minPHolm=${o.startRowMinPHolm}`,
  );

// ── STOP RULE evaluation ─────────────────────────────────────────────────────────────────────
const bandOK = CAND.bandReachPooled >= 0.7;
...
console.log(`\n── STOP RULE ──`);
console.log(
  `  band-reach >= 70%: ${bandOK ? "PASS" : "FAIL"} (${pct(CAND.bandReachPooled)})`,
);
console.log(
  `  dead finales  candidate ${pct(CAND.deadRate)} vs current ${pct(CUR.deadRate)}  delta ${(deadDelta * 100).toFixed(1)}pp  (screen expected candidate BETTER/lower)`,
);
console.log(
  `  front@line    candidate ${CAND.frontGroupAtLine.toFixed(2)} vs current ${CUR.frontGroupAtLine.toFixed(2)}  delta ${frontDelta.toFixed(2)}`,
);
console.log(
  `  => ${bandOK ? "GATE PRIMARY PASSES" : "GATE PRIMARY FAILS — DO NOT FLIP DEFAULTS"}`,
);
...
writeFileSync(
  join(OUT_ABS, "gate-arms.csv"),
  [
    AC.join(","),
    ...armAgg.map((a) =>
      AC.map((c) => (typeof a[c] === "number" ? r3(a[c]) : (a[c] ?? ""))).join(
        ",",
      ),
    ),
  ].join("\n") + "\n",
...
writeFileSync(
  join(OUT_ABS, "gate-arm-track.csv"),
  [
    "arm,track,dur,bandReach,startRowUnfair,startRowMinPHolm,nRacerRows",
    ...out.map((o) =>
      [
        o.arm,
        o.track.id,
...
writeFileSync(
  join(OUT_ABS, "gate-per-seed.csv"),
  [
    RC.join(","),
    ...allRows.map((r) =>
      RC.map((c) => (typeof r[c] === "number" ? r3(r[c]) : (r[c] ?? ""))).join(
        ",",
      ),
    ),
  ].join("\n") + "\n",
...
if (DURATIONS.length) {
  ...
  for (const d of DURATIONS)
    for (const arm of ARMS) for (const t of TRACKS) djobs.push({ d, arm, t });
  ...
  for (const d of DURATIONS) {
    for (const arm of ARMS) {
      ...
      const a = aggregate(
        runs,
        runs.flatMap((r) => r.rows),
      );
      durRows.push({
        dur: d,
        arm: arm.label,
        bandReachPooled: a.bandReachPooled,
        deadRate: a.deadRate,
        frontGroupAtLine: a.frontGroupAtLine,
        runawayRate: a.runawayRate,
        tiltSaturatedRate: a.tiltSaturatedRate,
      });
      console.log(
        `  ${String(d).padStart(4)}s ${arm.label.padEnd(20)} band=${pct(a.bandReachPooled)} dead=${pct(a.deadRate)} front@line=${a.frontGroupAtLine.toFixed(2)} runaway=${pct(a.runawayRate)} tiltSat=${pct(a.tiltSaturatedRate)}`,
      );
    }
  }
  writeFileSync(
    join(OUT_ABS, "gate-duration-scaling.csv"),
    [
      "dur,arm,bandReachPooled,deadRate,frontGroupAtLine,runawayRate,tiltSaturatedRate",
      ...durRows.map((r) =>
        [
          r.dur,
          r.arm,
          r3(r.bandReachPooled),
          r3(r.deadRate),
          r3(r.frontGroupAtLine),
          r3(r.runawayRate),
          r3(r.tiltSaturatedRate),
        ].join(","),
      ),
    ].join("\n") + "\n",
  );
  ...
  JSON.stringify(
    {
      ...
      dur: DUR,
      seed: SEED,
      racersClosed: RACERS_CLOSED,
      racersOpen: RACERS_OPEN,
      durations: DURATIONS,
      durRaces: DUR_RACES,
      wallClockSec: wall,
    },
