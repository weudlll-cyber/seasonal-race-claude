...
15b. ✅ **Phase 3A — Race Plan + Bereichs-Bonus** (feat/phase-3a, 2026-05-19) — `racePlanner.js` (B1–B5-Bereichszuordnung, P-Controller trajectoryMult [0.85,1.10], seeded PRNG), `bereichsBonusMult` in Physics-Loop (fade nach OUTCOME), symmetrische Startreihen (bottom-up), dynamische Ziellinie Open Tracks (ssf-basiert), 5 HUD-Overlays (RP DIAG), `racePlanBonusStrengthMultiplier` DevPanel + Sim. Defaults: avoidanceDistance=0.15, bonusMult=2.0. Sim-Smoke 120s: χ²=0.3–0.6 ✅. User-validiert.
