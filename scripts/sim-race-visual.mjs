...
function fillCircle(buf, W, H, cx, cy, radius, r, g, b) {
  ...
  for (let dy = -radius; dy <= radius; dy++) {
    for (let dx = -radius; dx <= radius; dx++) {
      if (dx * dx + dy * dy <= r2)
        setPixel(buf, W, Math.round(cx + dx), Math.round(cy + dy), r, g, b);
    }
  }
}
...
function renderFrame(shape, racers, transform, IMG_W, IMG_H, frameLabel) {
  ...
  for (let i = 0; i < N_SEG; i++) {
    ...
    const idx0 = shape._idx(t0),
      idx1 = shape._idx(t1);
    const o0 = shape._outer[idx0],
      o1 = shape._outer[idx1];
    const inn0 = shape._inner[idx0],
      inn1 = shape._inner[idx1];
    drawLine(
      buf,
      IMG_W,
      IMG_H,
...
function runSimulation(track, seed, burstIndex, burstLen) {
  ...
  const visualPaceSpeed = paceSpeedPxPerSec(
    normalSpeedFrom(),
  ...
  const rollInterval =
    ((dynamicsConfig.reRollLastPositionPercent / 100) * targetDuration * 1000) /
    rollCount;
    ...
    for (const r of racers) {
      if (!r.finished) {
        if (raceTs >= r.nextRollTime && raceTs < lastRollDeadline) {
          ...
          const halfWidth =
            spreadRange * (dynamicsConfig.reRollVariationPercent / 100);
        ...
        if (elapsed < r.transitionDuration) {
          ...
          r.spreadFactor =
            r.spreadFactorPrev +
            (r.spreadFactorTarget - r.spreadFactorPrev) * easeInOutCubic(prog);
...
console.log(
  `Bursts  : ${SEEDS.length} (one per seed), ${BURST_LEN} frames each`,
);
...
for (const trackId of TRACKS) {
  ...
  console.log(
    `   closed=${!track.closed ? "false (open)" : "true"} | pathLengthPx=${Math.round(track.pathLengthPx)} | worldW=${track.worldWidth}`,
  );
  ...
  for (let bi = 0; bi < SEEDS.length; bi++) {
    ...
    console.log(
      ` done (race-frames ${captureStart}–${captureStart + BURST_LEN - 1}, t=${startSec}s)`,
    );
  }

  console.log(
    `   → ${SEEDS.length * BURST_LEN * 2} PNGs (${SEEDS.length * BURST_LEN}×world + ${SEEDS.length * BURST_LEN}×camera) in ${trackDir}\n`,
  );
}
