...
- **B4 — camera foresight (consume the authored cameraPlan)** 🔜 *(added 2026-07-14 reconciliation)* — `heroCurveGenerator.buildCameraPlan` already emits a forward-looking plan (hero cast + roles: comebacker / sovereign-lead / faller / pack + beat timing), but `CameraDirector` never reads it — it still detects comebacks REACTIVELY via the `updateRacePlan(b1Indices)` hook / `_b1Indices` (CameraDirector.js:206-207, 494-544). **Done =** the camera consumes the plan and PRE-POSITIONS for authored drama (knows in advance a comebacker is coming). Extension of the existing `updateRacePlan` channel, not a new subsystem; the lateral rule is never weakened. Owner-approved step.
- **E2 — lead GROWS during PULK from a small start (unmeasured)** 🔜 *(added 2026-07-14 reconciliation)* — distinct from the chaos-runaway (a leader ALREADY uncatchable at PULK-start; see RACE-ACTION.md limitations + E1). Here a leader starts with a SMALL lead and GROWS it through PULK (the "Swift race" the owner observed). Never measured; root not pinned. Suspected "boost-herds-not-passes" dynamic — but that was under the removed PulkRaceDirector, so re-confirm against the current `applyPulkLeadRotation` (raceGovernor.js). **First step = measurement** (a sim metric for lead-delta across the PULK window), not a fix.
- **E3 — PULK→OUTCOME speed differential (accept-as-design or log)** 🔜 *(added 2026-07-14 reconciliation)* — verified design: in PULK the pack's `trajectoryMult` is pinned to 1.0 and rowBonus is 0 (racePlanner.js:474); in OUTCOME both return (trajectory rises via the P-controller gain 2.0, rowBonus full). Racers are genuinely faster in OUTCOME than in PULK — a real speed step at the boundary, not an onset artifact. **Owner decision:** intended dramaturgy (accept + document as design) or a seam to smooth? Log the decision either way.
