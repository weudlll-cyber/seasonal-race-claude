...
const VALID_OVERRIDE = {
  ...
  label: 'Turbo Mud',
  generatorId: 'splash',
  config: { color: '#3b1a00', count: 6, sizeMin: 2, sizeMax: 6, lifetimeFrames: 30, spawnProbability: 0.6, gravity: 0.2, spreadAngle: 1.5 },
  isOverride: true,
};

afterAll(async () => {
  for (const id of createdIds) {
    await request(app).delete(`/api/surface-classes/${id}`);
  }
});
...
describe('GET /api/surface-classes/:id', () => {
  it('returns 404 for unknown id', async () => {
    const res = await request(app).get('/api/surface-classes/nonexistent-xyz');
...
describe('POST /api/surface-classes', () => {
  it('creates a custom class and returns 201', async () => {
    const res = await request(app).post('/api/surface-classes').send(VALID_CUSTOM);
  ...
  it('created class appears in GET list', async () => {
    ...
    const listRes = await request(app).get('/api/surface-classes');
  ...
  it('returns 400 when id is missing', async () => {
    ...
    const res = await request(app).post('/api/surface-classes').send(noId);
  ...
  it('returns 400 when id contains uppercase letters', async () => {
    const res = await request(app)
      .post('/api/surface-classes')
      .send({ ...VALID_CUSTOM, id: 'InvalidID' });
    expect(res.status).toBe(400);
  });
  ...
  it('returns 400 when config is missing', async () => {
    ...
    const res = await request(app)
      .post('/api/surface-classes')
      .send({ ...noConfig, id: 'test-noconfig' });
    expect(res.status).toBe(400);
  });
...
describe('GET /api/surface-classes/:id after creation', () => {
  it('returns the created class', async () => {
    ...
    const getRes = await request(app).get(`/api/surface-classes/test-get-single`);
...
describe('DELETE /api/surface-classes/:id', () => {
  it('deletes an existing class and returns 204', async () => {
    ...
    const deleteRes = await request(app).delete('/api/surface-classes/test-delete');
    expect(deleteRes.status).toBe(204);

    const getRes = await request(app).get('/api/surface-classes/test-delete');
    expect(getRes.status).toBe(404);
  });

  it('returns 404 when class does not exist', async () => {
    const res = await request(app).delete('/api/surface-classes/nonexistent-xyz');
    expect(res.status).toBe(404);
  });
  ...
  it('deleted class no longer appears in GET list', async () => {
    ...
    await request(app).delete('/api/surface-classes/test-delete-list');
