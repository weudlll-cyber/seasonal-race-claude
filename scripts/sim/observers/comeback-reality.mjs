...
export function filterComebackers(perHero) {
  return (perHero ?? []).filter(
    (h) => heroRole(h) === 'comeback' && h.anchorRank != null && h.targetRank != null && h.finalRank != null
  );
}
...
export function comebackerMetrics(h) {
  ...
  const climbFracOfPlan = climbPlanned > 0 ? +(climbAchieved / climbPlanned).toFixed(4) : null;
  const nearTarget = Math.abs(rankDeltaToTarget) <= NEAR_TARGET_RANKS;
  return {
    index: h.index, anchorRank, targetRank, finalRank,
    climbPlanned, climbAchieved, targetReached, rankDeltaToTarget, climbFracOfPlan, nearTarget,
  };
}

const mean = (a) => (a.length ? +(a.reduce((s, v) => s + v, 0) / a.length).toFixed(3) : null);
const rate = (a) => (a.length ? +(a.filter(Boolean).length / a.length).toFixed(3) : null);
...
export function summarizeComebacker(perHero) {
  ...
  return {
    ...
    meanClimbFracOfPlan: mean(recs.map((r) => r.climbFracOfPlan).filter((v) => v != null)),
...
export function reliabilityStats(races) {
  let nRacesWithComebacker = 0, totalComebackers = 0;
  let topClimberHits = 0, top3ClimberHits = 0, halfClimbHits = 0;
  for (const race of races ?? []) {
    ...
    const climbers = (race.heroObs ?? [])
      ...
      .map((h) => ({ index: h.index, climb: Math.max(0, h.anchorRank - h.finalRank) }))
      .sort((a, b) => b.climb - a.climb);
    const topIdx = climbers[0]?.index;
    const top3 = new Set(climbers.slice(0, 3).map((x) => x.index));
    const bestCb = [...cbs].sort((a, b) => b.climbAchieved - a.climbAchieved)[0];
    if (bestCb.index === topIdx) topClimberHits++;
    if (top3.has(bestCb.index)) top3ClimberHits++;
    if (cbs.some((c) => c.climbFracOfPlan != null && c.climbFracOfPlan >= HALF_CLIMB_FRAC)) halfClimbHits++;
  }
  const r = (n) => (nRacesWithComebacker ? +(n / nRacesWithComebacker).toFixed(3) : null);
  return {
    nRacesWithComebacker, totalComebackers,
...
export function perTrackReport(trackId, isOpen, races) {
  ...
  return {
    trackId, isOpen, nRaces: (races ?? []).length,
...
export function renderMarkdown(perTrack, meta = {}) {
  ...
  L.push('');
  L.push(`*Generated ${meta.date ?? '(date)'} · seed=${meta.seed ?? '?'} · races/track=${meta.racesPerTrack ?? '?'} · racer=${meta.racer ?? '?'} · dur=${meta.dur ?? '?'}s · world=${meta.world ?? '?'}*`);
  L.push('');
  L.push('Empirical check for camera-foresight B4: does a hero cast as a **comebacker** actually climb close');
  L.push('to its authored `finalRank`, and is that designation a reliable pointer to real climbing?');
  L.push('');
  L.push('> **Scope caveat:** `heroObs` records only the cast (heroes), so the reliability "top climber" ranking');
  L.push('> is **among the cast**, not against the full field (full-field would need per-racer anchor ranks the');
  L.push('> sim does not collect). Read the reliability rates as "within the cast".');
  L.push('');
  L.push('## Per-track summary');
  L.push('');
  L.push('| Track | open | races | comebackers | meanPlanned | meanAchieved | achieved/plan | targetReach | Δ→target | nearTarget(±3) | top3-climber | ≥50%-climb |');
  L.push('|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|');
  for (const t of perTrack) {
    const s = t.summary, r = t.reliability;
    L.push(`| ${t.trackId} | ${t.isOpen ? 'open' : 'closed'} | ${t.nRaces} | ${s.nComebackers} | ${s.meanClimbPlanned ?? '—'} | ${s.meanClimbAchieved ?? '—'} | ${s.meanClimbFracOfPlan ?? '—'} | ${fmtPct(s.targetReachRate)} | ${s.meanRankDeltaToTarget ?? '—'} | ${fmtPct(s.nearTargetRate)} | ${fmtPct(r.top3ClimberRate)} | ${fmtPct(r.halfClimbRate)} |`);
  }
  // Pooled overall
  const allRaces = perTrack.flatMap((t) => t._races ?? []);
  if (allRaces.length) {
    const flat = allRaces.flatMap((rr) => rr.heroObs ?? []);
    const s = summarizeComebacker(flat), r = reliabilityStats(allRaces);
    L.push(`| **ALL** | — | ${allRaces.length} | ${s.nComebackers} | ${s.meanClimbPlanned ?? '—'} | ${s.meanClimbAchieved ?? '—'} | ${s.meanClimbFracOfPlan ?? '—'} | ${fmtPct(s.targetReachRate)} | ${s.meanRankDeltaToTarget ?? '—'} | ${fmtPct(s.nearTargetRate)} | ${fmtPct(r.top3ClimberRate)} | ${fmtPct(r.halfClimbRate)} |`);
  }
  L.push('');
  L.push('## Acceptance thresholds (from the sweep spec)');
  L.push('- **Plan soundness:** comebacker `finalRank` lands within **mean ±2–3** of `targetRank` (col Δ→target).');
  L.push('  If |Δ→target| ≥ 5, the plan is unreliable.');
  L.push('- **Reliability:** designated comebacker is a **top-3 climber (in the cast) in ≥60%** of races.');
  ...
  for (const t of perTrack) {
    ...
    L.push(`- **${t.trackId}**: ${keys.length ? keys.map((k) => `${k}×${h[k]}`).join(', ') : '(no comebackers)'}`);
  }
  L.push('');
  return L.join('\n');
}
