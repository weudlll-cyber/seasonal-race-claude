...
The random draw happens once in `SetupScreen/quickTestSeed.js` (`resolveQuickTestSeed`), **before** the race — so the race itself stays a pure function of the seed. A drawn seed is never written back into the field, which is what keeps the next race random instead of pinning it to the first drawn value.
...
| `scripts/camera-fingerprint.mjs` | The CAMERA's change detector — every director decision on every frame across all 10 tracks. Current **`ab731df15724ab5d`** |
| `scripts/render-fingerprint.mjs` | The RENDER path's change detector — the draw-call SEQUENCE at sixteen fixed frames, reaching the finish. Current **`1da1a5b392879293`** |
