...
| 2026-05-29 | **2334 unit** | feat(coats): expand STANDARD_COAT_PALETTE from 11 to 20 colors (Buggy/F1/Luge/Motorbike/Plane/Rocket — vehicle types). 9 new colors filling largest hue gaps; CSS lightness 40–57% for multiply/screen compatibility. `standardCoats.test.js` (6 tests). `racer-types.integration.test.js` updated: vehicle=20, animal=11 coat count assertion. Branch: `feat/extended-palette`. Backup tag: `backup/pre-extended-palette`. |
| 2026-05-29 | **2334 unit** | fix(luge): `tintMode: 'multiply'` → `tintMode: 'screen'` — red base suit caused red cast with multiply blending; screen mode correct for dark-on-light sprites. Test updated. No new tests. |
| 2026-05-30 | **2353 unit** | feat(coats): coat patterns infrastructure — `PATTERN_IDS`, tile builders (`_buildStripeTile`, `_buildDotsTile`), `_patternedVariantCache`, `getPatternedVariant` (lazy-baked on first call), `tintSprite` extended with `patternId` arg. `assignPattern` in `coatAssignment.js` (Fibonacci-salt XOR for decorrelation). `SpriteRacerType._drawBody` reads `racer.patternId`, uses fast-path via `getPatternedVariant`. `RaceScreen` wires `assignPattern` to racer init. 15 test mocks updated. +19 new tests (PATTERN_IDS, tintSprite patterns, getPatternedVariant lazy baking, assignPattern). Branch: `feat/coat-patterns`. Backup tag: `backup/pre-coat-patterns`. |
| 2026-05-30 | **2349 unit** | fix(coats): disable stripe/dot patterns — `assignPattern` always returns `'solid'`. Patterns too visually dominant at current sprite size; 20 solid colors sufficient. Infrastructure preserved for future re-enablement. 4 distribution/uncorrelated tests replaced by solid-only assertions. |
| 2026-05-30 | **2349 unit** | assets(luge): replace `luge-slide.png` with helmet-darkened version — helmet area (top ~58 px) darkened to near-black so screen blending does not color it; sled runners darkened to max brightness 130. No config changes. |
| 2026-05-31 | **2352 unit** | feat(lateral-velocity): physicalYVelocity system + 8-param sim sweep (feat/lateral-velocity → master, squash). `physicalYVelocity` state variable in `applyRacerBehavior` — lateral movement now has inertia governed by `lateralDamping` (0.25); `lateralForce` (0.012) drives avoidance/free-lane impulse; `yFreeLaneDeltas` normalized by `sqrt(overlapNeighborCount)` (N-scaling). 8-parameter sim sweep via Latin Hypercube Sampling: 1000 Phase 1 combos × 3 tracks × 10 races + top-10 Phase 2 × 100 races. Targeted 2D sweep `lateralDamping × lateralForce` (5×5 grid): winner d=0.25 f=0.012 — −37% lateralSpeedScore, −44% zigzagScore vs. baseline. New sim metric `computeZoneSuccessRate(raceEntries)` — per-zone (B1–B5) success rate using real racePlanner.js zone boundaries. Zone comparison (100 races × 3 tracks): +0.3pp overall — lateral change is a motion quality win, not a race-plan accuracy win. DevScreen: amber warning banner extended to all 8 interdependent params in BehaviorTuningSection. Final defaults: `lateralForce=0.012`, `lateralDamping=0.25`, `homeForceReductionOnOverlap=0.30`, `avoidanceDistance=0.15`, `speedBrakeFactor=0.95`, `speedBrakeTThreshold=0.015`, `speedBrakeYThreshold=0.20`. +3 tests (2349→2352). |

**Master-HEAD:** pending push (feat/lateral-velocity squash-merge)
**Tests:** 2352 total (2352 passing)
