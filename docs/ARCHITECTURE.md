...
`modules/racePlanner.js` `createTrajectoryController(racePlan)` returns a `.update()` that runs once per frame and writes **`r.trajectoryMult`** on every racer. It combines authored hero-choreography curves ("choreo") with a servo that steers each racer toward its assigned `targetRank` band. **Heroes are cast at the CHAOS→PULK boundary** (`pulkStart` = config `racePlanPulkStart`, shipped default `0.15` since COMBO15 — the narrower chaos window; the `DEFAULT_PHASE_FRACTIONS` raw fallback literal remains `0.25`, overridden by the shipped config); the rest of the field runs at `choreoPackBandStrictness` (the PACK band-loosening). Pre-OUTCOME, non-hero targets are pinned to 1.0; OUTCOME band-steering begins where PULK ends (`corridorStart := pulkEnd`, derived from `choreoOutcomeStart`). At the finish, heroes are held to `choreoReleaseProgress` then released, and each lower band resolves at `choreoResolveB2..B5`. `r.trajectoryMult` is threaded into the t-update multiplier chain in `screens/RaceScreen/index.jsx` alongside `r.areaBonusMult` and `rowEnvMult`.
