...
for (const a of advisory) console.log(`  advisory (${a.sev}): ${a.ghsa || a.url} — ${a.title}`);
for (const a of allowed)
  console.log(`  ALLOWLISTED (${a.sev}): ${a.ghsa} — ${a.title}\n    reason: ${ALLOWLIST[a.ghsa]}`);
for (const b of blocking) console.error(`  BLOCKING (${b.sev}): ${b.ghsa || b.url} — ${b.title}`);
