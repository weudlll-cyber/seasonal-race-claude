 ...
 *
 * Single shared function for both the server-load path (index.js) and the
 * lazy-draw fallback (SpriteRacerType) so the mask/non-mask logic lives in
 * exactly one place. Mask mode loads the base sprite, the shared maskUrl (if
 * any), and all per-coat patternMask / borderMask URLs.
 *
