...
| 2026-05-24 | **Phase 3C — spriteScale-Migration** (chore/sprite-scale-relative → master, squash `6a9dcfc`): `spritePx` → `spriteScale` (relativer Zoom-Faktor, Schema v14). `spriteScale=1.0` = natürliche dichte-skalierte Größe; racer-count-unabhängig (L82). Defaults: OVERVIEW 1.00, LEADER 1.81, BATTLE 2.81, COMEBACK 1.39, LEAD_CHANGE 1.81. `FALLBACK_REFERENCE_SPRITE_SIZE = 36 px` als Ankerpunkt. Neben-Fix: LEAD_CHANGE-Eintrag in `CameraStateHUD.STATE_CONFIG` ergänzt — fehlender Eintrag ließ Fallback `?? OVERVIEW` das falsche Badge zeigen (L87). CameraZoomTuningSection.test.jsx auf v14-Defaults aktualisiert. 2048/2048 Tests ✅. |
| 2026-05-25 | **Phase 3D — FINISH_OVERVIEW + BATTLE/COMEBACK-Fixes** (master, squash `bcdedb8`): FINISH_OVERVIEW: neuer finishMode auf OVERVIEW-State — sanfter Zoom-Out + Pan zum Lookback-Punkt in Weltpixeln (`finishOverviewLookbackPx: 300`, L88); Leader am Bildrand sichtbar; wartet auf letzten Finisher. Smooth-Pan-Fix: `_camT` bleibt bei winner.t, `_transitionTargetT = lookbackT`; eigener `else if`-Branch für T-Lerp parallel zum Zoom-Out (L89). BATTLE: P2-Drift-Exit, Rank-Span-Limit (max 5), Top-10-Anforderung, Isolation-Default 0→300px. COMEBACK: `outcomePhaseThreshold` 0.75→0.65, `comebackMinStartGap` 0.40→0.25, `comebackMaxCurrentRankPct` 0.10→0.20; DIAG mit gainOk/startGapOk/currentRankOk, Phase-Gate + leaderProgress/isOutcomePhaseActive sichtbar. Endgame-Threshold 85%→90%. Same-State-Repeat sofort unterbrechbar. LEAD_CHANGE: Pan-Snap-Fix bei Entry. DevScreen: CameraZoomTuningSection + CameraStateHudSection → CameraAdvancedSection (konsolidiert). OVERVIEW spriteScale auf Open Tracks wirksam. 2091/2091 Tests ✅. |
