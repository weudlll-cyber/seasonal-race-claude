...
> **BUILT, MEASURED, AND REJECTED BY THE OWNER AT THE PICTURE. NOT SHIPPED.** Landed on master as
> history by CLEANUP-BEFORE-NUMBERS-1. The mechanism below worked on its own terms — zero overlaps
> at every field size — and he judged the result unacceptable to look at. Nothing here runs in the
> game.
>
> **What is worth reading:** that a rule can pass every number it was given and still be wrong, and
> the two findings that outlive it — the render-fingerprint harness draws NAMELESS racers, so it
> cannot see this class of change; and requirement (f), stability between adjacent field sizes,
> cannot hold together with "smallest factor that clears" while the start grid is a staircase.
>
> **Salvaged from this branch:** the label-box geometry unification (box height, offset and padding
> had two homes). That part is arithmetically identical and is on master.
