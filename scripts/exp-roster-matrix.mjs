...
const JOBS = Number(argVal('jobs', String(Math.max(1, (cpus().length || 4) - 2))));
...
const SURFACES = {
  beetle: ['asphalt', 'cobble', 'earth'], boarder: ['asphalt', 'cobble', 'earth'], buggy: ['sand', 'earth', 'mud'],
  dolphin: ['water'], dragon: ['air', 'asphalt', 'earth', 'water'], duck: ['water', 'grass'],
  elephant: ['sand', 'earth', 'grass'], f1: ['asphalt'], giraffe: ['sand', 'earth', 'grass'],
  horse: ['sand', 'earth', 'grass', 'asphalt', 'snow', 'mud'], koi: ['water'], luge: ['ice', 'snow'],
  manta: ['water'], motorbike: ['asphalt', 'earth'], plane: ['air'], rocket: ['air', 'water'],
  snail: ['grass'], snake: ['sand', 'earth', 'grass'], snowmobile: ['snow', 'ice', 'earth'], turtle: ['water'],
};
...
const seed = (id) => JSON.parse(readFileSync(join(ROOT, 'server/seeds/tracks', `${id}.json`), 'utf8'));
const TRACKS = (argVal('tracks', TEN.join(',')).split(',').map((s) => s.trim()).filter(Boolean)).map((id) => {
  const s = seed(id);
  return { id, surf: s.surfaceClasses, closed: !!s.closed, racers: s.closed ? 40 : 60 };
});
...
function pExec(file, args) {
  return new Promise((res, rej) => {
    execFile(file, args, { cwd: ROOT, maxBuffer: 512 * 1024 * 1024 }, (err) => (err ? rej(err) : res()));
  });
}
...
async function runCell(track, type) {
  ...
  await pExec(process.execPath, [
    'scripts/sim-fairness.mjs', `--track=${track.id}`, `--racer=${type}`,
    '--seed=1', `--races=${RACES}`, `--racers=${track.racers}`, `--dur=${DUR}`,
    '--hero-map', '--runaway-parade', `--out=${out}`,
  ]);
  ...
  try {
    ...
    runaway = rp.length ? rp.filter((r) => r.runawayParade?.runaway).length / rp.length : null;
...
const plan = TRACKS.map((t) => ({ track: t, types: Object.keys(SURFACES).filter((ty) => intersects(SURFACES[ty], t.surf)) }))
  .filter((g) => g.types.length > 0);
const totalCells = plan.reduce((n, g) => n + g.types.length, 0);
console.log(`ROSTER-MATRIX-1: ${plan.length} tracks, ${totalCells} eligible cells, N=${RACES} dur=${DUR}s jobs=${JOBS}`);
...
for (const g of plan) results[g.track.id] = { track: g.track.id, closed: g.track.closed, surfaces: g.track.surf, expected: g.types.length, cells: [] };
const queue = [];
for (const g of plan) for (const ty of g.types) queue.push({ track: g.track, type: ty });
...
async function worker() {
  while (queue.length) {
    ...
    try {
      ...
      console.log(`[${++done}/${totalCells}] ${track.id}/${type}: arrival=${r.bandReach == null ? '?' : (r.bandReach * 100).toFixed(1) + '%'} rowMin=${(r.rowMin * 100).toFixed(0)}% ${r.holm} runaway=${r.runaway == null ? '?' : (r.runaway * 100).toFixed(0) + '%'} (${((Date.now() - t0) / 1000).toFixed(0)}s)`);
    } catch (e) {
      results[track.id].cells.push({ type, error: String(e).slice(0, 200) });
      console.log(`[${++done}/${totalCells}] ${track.id}/${type}: ERROR ${String(e).slice(0, 120)}`);
    }
    ...
    if (R.cells.length === R.expected) {
      ...
      if (!NO_COMMIT) {
        try {
          ...
          execFileSync('git', ['commit', '-q', '-m', `data(roster-matrix): ${track.id} eligible-cell arrivals (ROSTER-MATRIX-1)`], { cwd: ROOT });
          console.log(`  committed ${track.id}.json (${R.cells.length} cells)`);
        } catch (e) { console.log(`  commit skipped: ${String(e).slice(0, 80)}`); }
      }
    }
  }
}
