...
function buildReport(allResults, rawData, runDate, nRaces, nRacers, worldStamp, rowLayoutConfig) {
  const lines = [];

  lines.push('# RaceArena — Fairness Simulation Report');
  lines.push('');
  lines.push(`**world:** ${worldStamp.worldHash} (schema v${worldStamp.schemaVersion})${worldStamp.provisional ? ' — ⚠️ PROVISIONAL (ASSUMED-DEFAULTS, no --config; may not describe the owner\'s race)' : ''}  `);
  ...
  lines.push(`**Catch-Up (speedBonusFactor):** ${rowLayoutConfig.speedBonusFactor}  `);
